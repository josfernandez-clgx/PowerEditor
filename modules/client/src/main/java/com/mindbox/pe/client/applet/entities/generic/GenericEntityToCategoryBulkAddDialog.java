...
class GenericEntityToCategoryBulkAddDialog extends JPanel {
	...
	private boolean validate(GenericCategory[] categories) {
		if (!entityCanBelongToMultipleCategories && categories.length > 1) {
			ClientUtil.getInstance().showErrorDialog("msg.error.addmultiplecategories.overlaps", new Object[] { entity.getType().getName(), entity.getName() });
			return false;
		}
		...
		for (int i = 0; i < categories.length; i++) {
			MutableTimedAssociationKey newkey = new DefaultMutableTimedAssociationKey(categories[i].getId(), effDateEntryField.getValue(), expDateEntryField.getValue());
			...
			for (Iterator<MutableTimedAssociationKey> iter = entity.getCategoryIterator(); iter.hasNext();) {
				...
				if (newkey.overlapsWith(existingKey)) {
					if (newkey.getAssociableID() == existingKey.getAssociableID()) {
						ClientUtil.getInstance().showErrorDialog("msg.error.entitycannot.overlap.parent", new Object[] { entity.getName(), categories[i].getName() });

						return false;
					}
					else if (!entityCanBelongToMultipleCategories) {
						GenericCategory overlappingCategory = EntityModelCacheFactory.getInstance().getGenericCategory(entity.getType().getCategoryType(), existingKey.getAssociableID());
						ClientUtil.getInstance().showErrorDialog("msg.error.addentitycategory.overlaps", new Object[] { entity.getType().getName(), entity.getName(), overlappingCategory.getName() });
						return false;
					}
				}
			}
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (validate(categories)) {
				if ((categories != null) && (categories.length > 0)) {
					...
					for (int i = 0; i < categories.length; i++) {
						MutableTimedAssociationKey key = new DefaultMutableTimedAssociationKey(categories[i].getId(), effDateEntryField.getValue(), expDateEntryField.getValue());
						keys.add(key);
					}
				}
				else {
