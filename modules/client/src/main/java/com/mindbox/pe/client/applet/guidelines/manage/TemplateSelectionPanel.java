...
public final class TemplateSelectionPanel extends JPanel implements GuidelineContextProvider {
	...
	private final class SetFullContextL extends AbstractThreadedActionAdapter {
		...
		public void performAction(ActionEvent e) {
			if (guidelinesTable.getSelectedTemplateID() > 0) {
				try {
					GuidelineContext[] fullContext = ClientUtil.getCommunicator().fetchFullContext(guidelinesTable.getSelectedTemplateID(), contexts);
					if (fullContext == null) {
	...
	public TemplateSelectionPanel(boolean readOnly) {
		...
		this.editButton = UIFactory.createButton(ClientUtil.getInstance().getLabel("button.edit"), "image.btn.small.edit", new EditL(), null);
		this.viewButton = UIFactory.createButton(ClientUtil.getInstance().getLabel("button.view"), "image.btn.small.view", new ViewL(), null);
		this.newButton = UIFactory.createButton(ClientUtil.getInstance().getLabel("button.new"), "image.btn.small.new", new NewL(), null);
		this.setFullContextButton = UIFactory.createButton(ClientUtil.getInstance().getLabel("button.reset.full.context"), null, new SetFullContextL(), null);
	...
	private void initPanel() {
		...
		ButtonPanel buttonPanel = new ButtonPanel(
				(readOnly ? new JButton[] { viewButton, setFullContextButton } : new JButton[] { newButton, viewButton, editButton, setFullContextButton }),
				FlowLayout.LEFT);
	...
	public synchronized void setTemplates(TemplateUsageType usageType, GuidelineContext[] contexts, List<GridSummary> templateList) {
		if (usageType != null) {
			...
			this.contextLabel.setText("<html><body>" + ClientUtil.getInstance().getLabel("label.guideline.type") + ": <b>" + usageType + "</b></body></html>");
		}
		else {
