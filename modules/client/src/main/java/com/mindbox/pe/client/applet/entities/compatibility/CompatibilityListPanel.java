	...
	private class AddL extends AbstractThreadedActionAdapter {
		...
		public void performAction(ActionEvent event) throws Exception {
			if (prevType1 != null && prevType2 != null) {
				...
				if (data != null) {
					String type1CellVal = EntityModelCacheFactory.getInstance().getGenericEntityName(data.getSourceType(), data.getSourceID(), null);
					String type2CellVal = EntityModelCacheFactory.getInstance().getGenericEntityName(data.getGenericEntityType(), data.getAssociableID(), null);
