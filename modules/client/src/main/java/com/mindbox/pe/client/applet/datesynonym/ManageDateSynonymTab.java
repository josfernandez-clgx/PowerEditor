...
public class ManageDateSynonymTab extends PanelBase implements ChangeListener {
	private class AddL extends AbstractThreadedActionAdapter {
		public void performAction(ActionEvent event) throws Exception {
			DateSynonym dateSynonym = DateSynonymEditDialog.newDateSynonym(
		...
		public void performAction(ActionEvent event) throws Exception {
			...
			if (userSelection.length > 0) {
				...
				try {
					...
					else if (ClientUtil.getInstance().showConfirmation("msg.question.delete.dateSynonym", new Object[] { dateSynonym.getName() })) {
						ClientUtil.getCommunicator().delete(dateSynonym.getID(), PeDataType.DATE_SYNONYM);
	...
	private void editSelectedSynonym() {
		if (!readOnly) {
			...
			if (userSelection.length > 0) {
				...
				try {
					ClientUtil.getCommunicator().lock(synonymID, PeDataType.DATE_SYNONYM);
				}
				catch (ServerException ex) {
					ClientUtil.getInstance().showErrorDialog("msg.error.failure.lock", new Object[] { dateSynonym.getName(), ClientUtil.getInstance().getErrorMessage(ex) });
					return;
				}
				try {
				...
				try {
					ClientUtil.getCommunicator().unlock(synonymID, PeDataType.DATE_SYNONYM);
				}
				catch (ServerException ex) {
