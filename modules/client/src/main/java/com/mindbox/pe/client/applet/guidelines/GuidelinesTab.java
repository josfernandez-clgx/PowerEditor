...
public class GuidelinesTab extends PowerEditorTab implements ChangeListener {
	...
	public GuidelinesTab(final List<GuidelineTab> tabConfigs, TemplateManagementTab templateManagementTab, boolean readOnly) throws ServerException {
		super();
		...
		if (ClientUtil.checkPermissionByPrivilegeName(PrivilegeConstants.PRIV_MANAGE_GUIDELINE_ACTIONS)) {
			...
			addTab(
					...
					ClientUtil.getInstance().makeImageIcon("image.blank"),
					new EntityManagementPanel<ActionTypeDefinition, EntityManagementButtonPanel<ActionTypeDefinition>>(PeDataType.GUIDELINE_ACTION, new FunctionFilterPanel<ActionTypeDefinition>(
							selectionPanel,
							PeDataType.GUIDELINE_ACTION), selectionPanel, detailPanel),
		...
		if (ClientUtil.checkPermissionByPrivilegeName(PrivilegeConstants.PRIV_MANAGE_GUIDELINE_ACTIONS)) {
			...
			addTab(
					...
					ClientUtil.getInstance().makeImageIcon("image.blank"),
					new EntityManagementPanel<TestTypeDefinition, EntityManagementButtonPanel<TestTypeDefinition>>(PeDataType.GUIDELINE_TEST_CONDITION, new FunctionFilterPanel<TestTypeDefinition>(
							selectionPanel,
							PeDataType.GUIDELINE_TEST_CONDITION), selectionPanel, detailPanel),
