 ...
 *
 */
public class CBRCaseBaseManagementPanel extends JPanel implements PowerEditorTabPanel {
	...
	private void _removeEntity() {
		if (ClientUtil.getInstance().showConfirmation("msg.question.remove.entity", new Object[] { "Case-Base" })) {
			try {
				if (caseBase.getID() != CBRCaseBase.UNASSIGNED_ID) {
					ClientUtil.getCommunicator().delete(caseBase.getID(), PeDataType.CBR_CASE_BASE);
				}
			}
			catch (Exception x) {
				// what to do here???
	...
	private void addButtonPanel() {
		JButton newButton = UIFactory.createButton(ClientUtil.getInstance().getLabel("button.new"), "image.btn.small.new", new NewL(), null);

		// This is a copy button as of PE 4.2.0.
		JButton cloneButton = UIFactory.createButton(ClientUtil.getInstance().getLabel("button.copy"), "image.btn.small.copy", new CloneL(), null);
		JButton removeButton = UIFactory.createButton(ClientUtil.getInstance().getLabel("button.remove"), "image.btn.small.delete", new RemoveL(), null);
