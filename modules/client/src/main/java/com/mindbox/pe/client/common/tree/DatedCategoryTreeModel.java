...
public class DatedCategoryTreeModel extends DefaultTreeModel {
	...
	public DatedCategoryTreeModel(GenericCategoryNode root, Date initialDate, DataProvider dataProvider, boolean showEntities, boolean sort) {
		super(root);
	...
	public synchronized void editGenericCategory(GenericCategory category) {
		...
		if (nodeToMove != null) {
			if (nodeToMove.getGenericCategory().isRoot()) {
				...
				GenericCategoryNode newParentNode = (category.getParentID(date) == -1 ? null : findGenericCategoryNode(category.getParentID(date)));
