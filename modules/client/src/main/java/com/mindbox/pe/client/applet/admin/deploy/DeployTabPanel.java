...
public class DeployTabPanel extends PanelBase {
	...
	private class DeployActionAdapter extends AbstractThreadedActionAdapter {
		...
		public void performAction(ActionEvent e) {
			...
			try {
				if (!dataFilterPanel.hasSelection()) {
					...
					final GuidelineReportFilter filter = dataFilterPanel.getFilter();
					...
					if (checkDeployPermission(specifiedStatus)) {
						...
						if (asOfDateField != null) {
							filter.setDeployAsOfDate(asOfDateField.getDate());
						}
		...
		private void processDeployment(GuidelineReportFilter filter, boolean exportPolicies) {
			final DeployResponse deployResponse = deploy(filter, exportPolicies);
			if (deployResponse != null) {
				if (deployResponse.getGenerateRunId() > 0) {
					reset();

					boolean flag = monitorCheckBox.isSelected();
					if (flag) {
						MonitorThread monitorthread = new MonitorThread(deployResponse.getGenerateRunId());
						monitorthread.start();
					}
					printInfo("RunId = " + deployResponse.getGenerateRunId());
				}
				else if (deployResponse.getGenerateRunId() < 0) {
					JOptionPane.showMessageDialog(getApplet(), getInstance().getMessage(DeployTabPanel.DEPLOYMENT_ALREADY_IN_PROGRESS_MSG), getInstance().getMessage("ServerErrorMsgTitle"), 0);
					return;
				}
			}
		}
	...
	private class MonitorThread extends Thread {
		...
		private GenerateStats generateStats;
		...
		public MonitorThread(int i) {
			...
			monitorRunnable = new Runnable() {
				...
				public void run() {
					deployDirField.setText(generateStats == null ? "" : generateStats.getDeployDir());
					deployProgressBar.setValue(generateStats == null ? 0 : generateStats.getPercentComplete());
					numRulesField.setText(Integer.toString(generateStats == null ? 0 : generateStats.getNumRulesGenerated()));
					numObjectsField.setText(Integer.toString(generateStats == null ? 0 : generateStats.getNumObjectsGenerated()));
					numErrorsField.setText(Integer.toString(generateStats == null ? 0 : generateStats.getNumErrorsGenerated()));
				}
		...
		private void reportStatus() {
			if (generateStats.getNumErrorsGenerated() == 0) {
			...
			try {
				...
				while (flag) {
					try {
						...
						Thread.sleep(2000L);
						generateStats = monitor(runID);
						flag = (generateStats != null && generateStats.isRunning());
	...
	private final MDateDateField asOfDateField;
	private final JButton showErrorButton;
	private int runID = 0;

	public DeployTabPanel(final boolean showDeployAsOfDate) {
		if (showDeployAsOfDate) {
			asOfDateField = new MDateDateField(true, true, true);
		}
		else {
			asOfDateField = null;
		}
		...
		showErrorButton = UIFactory.createButton(getInstance().getLabel("button.display.error"), "image.btn.small.view", new ShowErrorL(), null);
		showErrorButton.setVisible(false);
	...
	private void addDeployComponents(final JPanel jpanel) {
		...
		if (asOfDateField != null) {
			final JPanel fieldPanel = UIFactory.createJPanel(new BorderLayout(4, 4));
			fieldPanel.add(UIFactory.createLabel("label.deploy.as.of.date"), BorderLayout.WEST);
			fieldPanel.add(asOfDateField, BorderLayout.CENTER);

			final JPanel containerPanel = UIFactory.createJPanel(new BorderLayout());
			containerPanel.add(fieldPanel, BorderLayout.NORTH);
			containerPanel.add(guidelineFilterPanel, BorderLayout.CENTER);
			splitPane = UIFactory.createSplitPane(JSplitPane.HORIZONTAL_SPLIT, dataFilterPanel, containerPanel);
		}
		else {
			splitPane = UIFactory.createSplitPane(JSplitPane.HORIZONTAL_SPLIT, dataFilterPanel, guidelineFilterPanel);
		}
	...
	private GenerateStats monitor(int i) throws ServerException {
