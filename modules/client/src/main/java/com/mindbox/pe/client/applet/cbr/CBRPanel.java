...
public class CBRPanel extends JPanel implements PowerEditorTabPanel, ChangeListener {
	...
	public CBRPanel(boolean readOnly) {
		...
		newButton = UIFactory.createButton(ClientUtil.getInstance().getLabel("button.casebase.new"), "image.btn.small.new", new NewL(), null);
	...
	private List<CBRCaseBase> getCaseBasesFromServer() {
		...
		try {
			caseBaseList = ClientUtil.getCommunicator().search(new AllSearchFilter<CBRCaseBase>(PeDataType.CBR_CASE_BASE));
		}
