 ...
 */
public class CBRCaseDetailPanel extends AbstractDetailPanel<CBRCase, EntityManagementButtonPanel<CBRCase>> implements ActionListener, ValueChangeListener, ChangeListener {
	/**
	 * 
	 */
	...
	protected void validateFields() throws InputValidationException {
		...
		try {
			List<?> caseList = ClientUtil.getCommunicator().search(new CBRExactNameSearchFilter(PeDataType.CBR_CASE, nameField.getText(), caseBase.getID()));
		...
		while (it.hasNext()) {
			...
			while (it2.hasNext()) {
				...
				if (av2.getAttribute() != null && av2.getName() != null && av2 != av && av2.getAttribute().equals(av.getAttribute())
					...
					throw new InputValidationException("Two Attribute Value pairs in the same case may not have the same Attribute and Value.");
			}
		}
