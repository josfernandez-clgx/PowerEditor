...
public final class EntityManagementButtonPanel<T extends IDNameObject> extends ButtonPanel {
	...
	private EntityManagementButtonPanel(boolean readOnly, final AbstractSelectionPanel<T, EntityManagementButtonPanel<T>> selectionPanel, GenericEntityType genericEntityType,
		...
		if (genericEntityType != null) {
			CategoryType catDef = ClientUtil.getEntityConfigHelper().getCategoryDefinition(genericEntityType);
			if (catDef != null && UtilBase.asBoolean(catDef.isShowInSelectionTable(), false)) {
		...
		if (verifyUnlockPrevious()) {
			...
			if (values != null && values.size() > 0) {
				try {
					...
					ClientUtil.getInstance().showErrorDialog("msg.error.failure.lock", new Object[] { entityName, ClientUtil.getInstance().getErrorMessage(ex) });
				}
			}
			else {
			...
			if (values != null && values.size() > 0) {
				if (ClientUtil.getInstance().showConfirmation("msg.question.remove.entity", new Object[] { entityName })) {
					try {
						...
						ClientUtil.getInstance().showErrorDialog("msg.error.failure.remove", new Object[] { entityName, ClientUtil.getInstance().getErrorMessage(ex) });
					}
				}
			}
			else {
			...
			for (Iterator<ParameterGrid> it = paramGridList.iterator(); it.hasNext();) {
				...
				if (ClientUtil.isHighestStatus(grid.getStatus())) {
					...
					if (ids != null && ids.length > 0 && UtilBase.contains(new int[] { entityID }, ids)) {
						ClientUtil.getInstance().showErrorDialog("msg.error.entityusedinproduction.parameter", new Object[] { ClientUtil.getHighestStatusDisplayLabel() });
						return true;
					}
				}
			}
				...
				if (detailPanel.getCurrentEntityID() != -1 && hasUnsavedChanges) {
					...
					if (entityType != null) {
						...
						try {
							...
							ClientUtil.getLogger().error("Failed to unlock " + genericEntityType + "  - " + detailPanel.getCurrentEntityID(), ex);
						}
					}
					hasUnsavedChanges = false;
				}
