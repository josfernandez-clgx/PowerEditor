...
public class ConditionEditDialog extends JPanel implements ChangeListener {
	...
	private static final String[] OPERATORS_EQUALITY = new String[] { OPERATORS_ALL[0], OPERATORS_ALL[1], OPERATORS_ALL[10], OPERATORS_ALL[11] };
	...
	private final JComboBox<EnumValue> enumCombo;
	private final JComboBox<String> booleanCombo, booleanNullCombo;
	...
	private final JComboBox<String> calcOpCombo;
	...
	private ConditionEditDialog(GridTemplate template, JDialog dialog, Condition condition, boolean allowValueChangeOnly, boolean allowNull, int columnNo) {
		this.dialog = dialog;
		...
		calcOpCombo = new JComboBox<String>(OPERATORS_MATH_ONLY);
		...
		booleanNullCombo = new JComboBox<String>();
		...
		opCombo = new JComboBox<String>();
		commentField = new JTextField(10);
		enumCombo = new JComboBox<EnumValue>();
		...
		if (deployType != null) {
			...
			if (colRef != null) {
				...
				if (col == null || opCombo.getSelectedItem() == null)
					...
					String message = DataTypeCompatibilityValidator.isValid(attRef, op, colRefVal, template, DomainModel.getInstance(), true);
					if (message != null) {
						columnRefField.setValue(null);
						JOptionPane.showMessageDialog(this, message);
					}
	...
	private void initPanel() {
		...
		if (allowValueChangeOnly && condition != null && condition.getValue() instanceof MathExpressionValue && ((MathExpressionValue) condition.getValue()).getValue() != null) {
			MathExpressionValue mathExpValue = (MathExpressionValue) condition.getValue();
			vpanelWrapper.add(new JLabel(" " + mathExpValue.getOperator() + "  " + mathExpValue.getAttributeReference().toString()), BorderLayout.SOUTH);
		}
	...
	private boolean isForEnumMultiSelect() {
		...
		if (columnNo > 0 && template != null && template.getColumn(columnNo) != null) {
			...
			return (opCombo.getSelectedItem() != null && (opCombo.getSelectedItem().equals(Condition.OPSTR_IN) || opCombo.getSelectedItem().equals(Condition.OPSTR_NOT_IN)));
		}
	}

	private boolean isForEnumSingleSelect() {
		return enumValueList != null && (columnNo < 1 || template.getColumn(columnNo).getColumnDataSpecDigest().getType().equals(ColumnDataSpecDigest.TYPE_ENUM_LIST));
	}

	private boolean isForSymbol() {
		return deployType == DeployType.CODE
				|| deployType == DeployType.SYMBOL && (columnNo < 1 || template.getColumn(columnNo).getColumnDataSpecDigest().getType().equals(ColumnDataSpecDigest.TYPE_SYMBOL));
	}

	private boolean isOpBetween() {
		return opCombo.getSelectedItem() != null && (opCombo.getSelectedItem().equals(Condition.OPSTR_BETWEEN) || opCombo.getSelectedItem().equals(Condition.OPSTR_NOT_BETWEEN));
	}

	private boolean isOpComparison() {
		return opCombo.getSelectedItem() != null && (opCombo.getSelectedItem().equals(Condition.OPSTR_GREATER) || opCombo.getSelectedItem().equals(Condition.OPSTR_GREATER_EQUAL)
				|| opCombo.getSelectedItem().equals(Condition.OPSTR_LESS) || opCombo.getSelectedItem().equals(Condition.OPSTR_LESS_EQUAL));
	}

	private boolean isOpEntityMatchFunc() {
		return opCombo.getSelectedItem() != null
				&& (opCombo.getSelectedItem().equals(Condition.OPSTR_ENTITY_MATCH_FUNC) || opCombo.getSelectedItem().equals(Condition.OPSTR_NOT_ENTITY_MATCH_FUNC));
	}

	private boolean isOpEquality() {
		return opCombo.getSelectedItem() != null && (opCombo.getSelectedItem().equals(Condition.OPSTR_EQUAL) || opCombo.getSelectedItem().equals(Condition.OPSTR_NOT_EQUAL));
	}

	private boolean isOpUnary() {
		return opCombo.getSelectedItem() != null && (opCombo.getSelectedItem().equals(Condition.OPSTR_IS_EMPTY) || opCombo.getSelectedItem().equals(Condition.OPSTR_IS_NOT_EMPTY)
				|| (opCombo.getSelectedItem().equals(Condition.OPSTR_ANY_VALUE)));
	}
	...
	private void refreshValueEnabling() {
		if (deployType == null) {
			...
			setAttributeEnabled(true);
			setColumnEnabled(true);
			boolean calcAllowed = UtilBase.isMember(DataTypeCompatibilityValidator.getGenericDataType(deployType), DataTypeCompatibilityValidator.DATA_TYPES_FOR_CALC);
	...
	private void setValueString(String value) {
		if (isOpUnary()) {
			...
			for (int j = 0; j < enumCombo.getItemCount(); j++) {
				EnumValue ev = enumCombo.getItemAt(j);
			...
			if (deployType == null) {
				...
				if (doRange) {
					...
					try {
						dateField.setValue((UtilBase.isEmpty(value) ? null : Constants.THREADLOCAL_FORMAT_DATE.get().parse(value)));
					}
					catch (Exception e) {
						e.printStackTrace();
					}
	...
	private boolean updateCondition() {
		...
		if ((valueRadioButton.isSelected() && (getValueString() == null || getValueString().length() == 0)) || (attrRefRadioButton.isSelected() && !attrRefField.hasValue())
		...
		if (isOpUnary()) {
			...
			condition.setValue(RuleElementFactory.getInstance().createValue(RuleElementFactory.getInstance().createReference(attrRefField.getValue())));
		}
		else if (columnRefRadioButton.isSelected()) {
