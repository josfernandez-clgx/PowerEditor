...
public class ConditionEditDialog extends JPanel implements ChangeListener {
	...
	private void resetEnumIfNecessary(int columnNo) {
		if (template != null && template.getColumn(columnNo) != null && template.getColumn(columnNo).getColumnDataSpecDigest().getEnumSourceType() == EnumSourceType.DOMAIN_ATTRIBUTE
	...
	private boolean isForSymbol() {
		return deployType == DeployType.CODE || deployType == DeployType.SYMBOL
				&& (columnNo < 1 || template.getColumn(columnNo).getColumnDataSpecDigest().getType().equals(ColumnDataSpecDigest.TYPE_SYMBOL));
	}
	...
	private boolean isOpEntityMatchFunc() {
		return opCombo.getSelectedItem() != null && (opCombo.getSelectedItem().equals(Condition.OPSTR_ENTITY_MATCH_FUNC) || opCombo.getSelectedItem().equals(Condition.OPSTR_NOT_ENTITY_MATCH_FUNC));
	}

	private boolean isOpUnary() {
		return opCombo.getSelectedItem() != null
				&& (opCombo.getSelectedItem().equals(Condition.OPSTR_IS_EMPTY) || opCombo.getSelectedItem().equals(Condition.OPSTR_IS_NOT_EMPTY) || (opCombo.getSelectedItem().equals(Condition.OPSTR_ANY_VALUE)));
	}

	private boolean isOpComparison() {
		return opCombo.getSelectedItem() != null
				&& (opCombo.getSelectedItem().equals(Condition.OPSTR_GREATER) || opCombo.getSelectedItem().equals(Condition.OPSTR_GREATER_EQUAL)
