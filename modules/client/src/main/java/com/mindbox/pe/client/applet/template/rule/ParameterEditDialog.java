...
class ParameterEditDialog extends JPanel {
	 ...
	 * 
	 */
	private ParameterEditDialog(GridTemplate template, JDialog dialog, FunctionParameterDefinition paramDef, FunctionParameter parameter) {
		this.dialog = dialog;
	...
	private void setValueString(String value) {
		...
		if (deployType == DeployType.BOOLEAN) {
			...
			try {
				dateField.setDate(Constants.THREADLOCAL_FORMAT_DATE.get().parse(value));
			}
			catch (Exception e) {
