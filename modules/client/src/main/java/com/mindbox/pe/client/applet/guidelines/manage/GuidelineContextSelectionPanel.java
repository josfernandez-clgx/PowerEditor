	...
	private void initPanel() {
		...
		for (final EntityType entityType : ClientUtil.getEntityConfigHelper().getEntityTypeDefinitions()) {
			if (ConfigUtil.isUseInContext(entityType)) {
				final GenericEntityType type = GenericEntityType.forID(entityType.getTypeID().intValue());
				...
				if (categoryDef != null) {
					tabPane.add(ClientUtil.getInstance().getLabel(ClientUtil.getEntityConfigHelper().getCategoryDefinition(type)), createCategoryContextPanel(type));
				}
			}
		}
