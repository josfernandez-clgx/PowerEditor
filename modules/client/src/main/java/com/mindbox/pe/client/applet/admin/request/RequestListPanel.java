...
class RequestListPanel extends IDNameDescriptionObjectSelectionPanel<ProcessRequest, ButtonPanel> {
	...
	public static RequestListPanel createInstance(boolean readOnly) {
		RequestListPanel panel = new RequestListPanel(ClientUtil.getInstance().getLabel("label.request"), new IDNameDescriptionObjectSelectionTable<RequestSelectionTableModel, ProcessRequest>(
	...
	private class DeleteL extends AbstractThreadedActionAdapter {
		public void performAction(ActionEvent arg0) {
			...
			if (request != null && ClientUtil.getInstance().showConfirmation("msg.question.delete.request", new Object[] { request.getDisplayName() })) {
				// delete request
				try {
					ClientUtil.getCommunicator().delete(request.getID(), PeDataType.PROCESS_REQUEST);
	...
	private class LoadL extends AbstractThreadedActionAdapter {
		public void performAction(ActionEvent arg0) {
			try {
				selectionTable.setDataList(ClientUtil.getCommunicator().search(new AllSearchFilter<ProcessRequest>(PeDataType.PROCESS_REQUEST)));
			}
			catch (Exception ex) {
				ClientUtil.handleRuntimeException(ex);
			}
	...
	private RequestListPanel(String title, IDNameDescriptionObjectSelectionTable<RequestSelectionTableModel, ProcessRequest> selectionTable, boolean readOnly) {
