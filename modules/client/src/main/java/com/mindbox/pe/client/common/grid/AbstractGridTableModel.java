...
public abstract class AbstractGridTableModel<T extends AbstractTemplateCore<?>> extends DefaultTableModel {
	...
	private Object getDefaultValue(int i) throws ServerException {
		try {
			...
			if (dataSpecDigest.getType().equals(ColumnDataSpecDigest.TYPE_FLOAT_RANGE) || dataSpecDigest.getType().equals(ColumnDataSpecDigest.TYPE_CURRENCY_RANGE)) {
			...
			if (ExcelAdapter.isNullEmptyString(obj)) { // new value is empty
				...
				if (columnDataSpecDigest.getType().equals(ColumnDataSpecDigest.TYPE_ENTITY)) {
					...
					if (obj instanceof Date) {
						...
						final ThreadLocal<DateFormat> formatterThreadLocal = columnDataSpecDigest.getType().equals(ColumnDataSpecDigest.TYPE_DATE_TIME)
								? Constants.THREADLOCAL_FORMAT_DATE_TIME_MIN
								: Constants.THREADLOCAL_FORMAT_DATE;
					...
					if (obj instanceof FloatRange) {
						...
						if (!CellValidator.validateValue(obj.toString(), columnDataSpecDigest)) {
							...
							ClientUtil.getInstance().showErrorDialog("msg.warning.invalid.value.cell", new Object[] { obj, String.valueOf(col + 1) });
							return false;
						}
					...
					if (!CellValidator.validateValue(f, columnDataSpecDigest)) {
						...
						ClientUtil.getInstance().showErrorDialog("msg.warning.invalid.value.cell", new Object[] { String.valueOf(f), String.valueOf(col + 1) });
						return false;
					}
					objToSet = number;
				}
				else if (columnDataSpecDigest.getType().equals(ColumnDataSpecDigest.TYPE_INTEGER)) {
					Integer integer = obj instanceof Integer ? (Integer) obj : (obj instanceof Number ? new Integer(((Number) obj).intValue()) : Integer.parseInt(obj.toString()));
					...
					if (!CellValidator.validateValue(i, columnDataSpecDigest)) {
						...
						ClientUtil.getInstance().showErrorDialog("msg.warning.invalid.value.cell", new Object[] { String.valueOf(i), String.valueOf(col + 1) });
						return false;
					}
	...
	private boolean validateValue(Object obj, int col) {
		...
		try {
			...
			else if (columnDataSpecDigest.getType().equals(ColumnDataSpecDigest.TYPE_FLOAT) || columnDataSpecDigest.getType().equals(ColumnDataSpecDigest.TYPE_PERCENT)) {
