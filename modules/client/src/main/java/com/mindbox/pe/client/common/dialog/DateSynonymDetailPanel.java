...
final class DateSynonymDetailPanel extends JPanel {
		...
		public void actionPerformed(ActionEvent arg0) {
			try {
				if (DateSynonymDetailPanel.this.getDateSynonym() == null || DateSynonymDetailPanel.this.getDateSynonym().getDate() == null) {
					...
					DateSynonymDetailPanel.this.nameField.setText(DateFormat.getDateTimeInstance(DateFormat.LONG, DateFormat.LONG).format(DateSynonymDetailPanel.this.getDateSynonym().getDate()));
				}
			}
	...
	private void updateFromFields() throws InputValidationException {
		if (dateField.getDate() == null) throw new InputValidationException(ClientUtil.getInstance().getMessage("msg.warning.invalid.date", new Object[] { ClientUtil.dateFormatter.toPattern() }));
		if (dateSynonym == null) {
