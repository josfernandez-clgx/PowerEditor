...
public class VersionChecker {
    ...
    public void run() {
        try {
            ...
            // Removed by comment --> Runtime.getRuntime().exec(cmdList.toArray(new String[0])); // Veracode Flaw ID 1 CWE ID ('OS Command Injection')
            System.out.print(".");

            // spawn timeout thread
            // new TimeoutThread().start(); // removed due to removal of runtime call above for Veracode Flaw ID 1 CWE ID ('OS Command Injection')
            System.out.print(".");
