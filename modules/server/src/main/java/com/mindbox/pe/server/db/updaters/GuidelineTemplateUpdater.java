...
public class GuidelineTemplateUpdater extends AbstractUpdater {
	...
	private static final String Q_INSERT_TEMPLATE_COLUMN_PROP = "insert into MB_TEMPLATE_COLUMN_PROP" + " (template_id,column_no,property_name,property_value) values (?,?,?,?)";

	private static final String Q_DELETE_TEMPLATE_COLUMN_PROP = "delete from MB_TEMPLATE_COLUMN_PROP" + " where template_id=? and column_no=?";

	private static final String Q_INSERT_TEMPLATE = "insert into MB_TEMPLATE" + " (template_id,name,usage_type,status,max_row,parent_id,comp_cols,consist_cols,fit_screen,description,comments)"
			+ " values (?,?,?,?,?,?,?,?,?,?,?)";

	private static final String Q_UPDATE_TEMPLATE = "update MB_TEMPLATE" + " set name=?,usage_type=?,status=?,max_row=?,parent_id=?,comp_cols=?,consist_cols=?,fit_screen=?,description=?,comments=?"
			+ " where template_id=?";

	private static final String Q_DELETE_TEMPLATE = "delete from MB_TEMPLATE where template_id=?";

	private static final String Q_INSERT_TEMPLATE_COLUMN = "insert into MB_TEMPLATE_COLUMN" + " (template_id,column_no,column_name,description,attribute_map,title,font,color,width,data_type,"
			+ " multi_select,allow_blank,sort_enum,show_lhs_attr,min_value,max_value,precision_val)" + " values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";

	private static final String Q_UPDATE_TEMPLATE_COLUMN = "update MB_TEMPLATE_COLUMN" + " set column_name=?,description=?,attribute_map=?,title=?,font=?,color=?,width=?,data_type=?,"
			+ " multi_select=?,allow_blank=?,sort_enum=?,show_lhs_attr=?,min_value=?,max_value=?,precision_val=?" + " where template_id=? and column_no=?";
	...
	private static final String Q_INSERT_TEMPLATE_COLUMN_ENUM = "insert into MB_TEMPLATE_COLUMN_ENUM" + " (template_id,column_no,enum_value) values (?,?,?)";
	...
	private static final String Q_INSERT_TEMPLATE_COLUMN_ATTR_ITEM = "insert into MB_TEMPLATE_COLUMN_ATTR_ITEM" + " (template_id,column_no,name,display_value) values (?,?,?,?)";
	...
	private static final String Q_INSERT_TEMPLATE_COLUMN_MESSAGE_FRAGMENT = "insert into MB_TEMPLATE_MESSAGE_FRAGMENT"
			+ " (template_id,column_no,message_text,type,cell_selection,enum_delim,enum_final_delim,enum_prefix,range_style)" + " values (?,?,?,?,?,?,?,?,?)";
	...
	private static final String Q_INSERT_TEMPLATE_MESSAGE = "insert into MB_TEMPLATE_MESSAGE" + " (template_id,column_no,entity_id,cond_delim,cond_final_delim,message_text)" + " values (?,?,?,?,?,?)";
	...
	private void setTemplateMessages(Connection conn, int templateID, int columnNo, List<TemplateMessageDigest> messageDigestList, boolean deleteFirst) throws SQLException {
			...
			if (!messageDigestList.isEmpty()) {
				...
				for (Iterator<TemplateMessageDigest> iter = messageDigestList.iterator(); iter.hasNext();) {
					...
					if (conditionalFinalDelimiter != null && conditionalFinalDelimiter.indexOf(" ") > -1) {
						conditionalFinalDelimiter = conditionalFinalDelimiter.replaceAll(" ", ColumnMessageFragmentDigest.SPACE_PLACE_HOLDER);
					}
					ps.setInt(1, templateID);
					ps.setInt(2, columnNo);
					...
					count = ps.executeUpdate();
					if (count < 1) {
						throw new SQLException("Failed to insert template message for " + templateID + "," + columnNo + " - entityID:" + element.getEntityID());
					}
				}
			}
	...
	private void setColumnAttributeItems(Connection conn, int templateID, GridTemplateColumn column, boolean deleteFirst) throws SQLException {
		...
		try {
			...
			if (column.getColumnDataSpecDigest().hasAttributeItem()) {
				...
				for (Iterator<ColumnAttributeItemDigest> iter = column.getColumnDataSpecDigest().getAllAttributeItems().iterator(); iter.hasNext();) {
					...
					count = ps.executeUpdate();
					if (count < 1) {
						throw new SQLException("Failed to insert column attr item for " + templateID + "," + column.getColumnNumber() + " - " + digest.getName());
					}
				}
			}
		...
		try {
			...
			if (column.getColumnDataSpecDigest().hasEnumValue()) {
				...
				for (Iterator<String> iter = column.getColumnDataSpecDigest().getAllColumnEnumValues().iterator(); iter.hasNext();) {
					...
					count = ps.executeUpdate();
					if (count < 1) {
						throw new SQLException("Failed to insert column enum value for " + templateID + "," + column.getColumnNumber() + " - " + value);
					}
				}
			}
	...
	private void setColumnMessageFragments(Connection conn, int templateID, GridTemplateColumn column, boolean deleteFirst) throws SQLException {
		logDebug(logger, ">>> setColumnMessageFragments: %s, %s, %b", templateID, column, deleteFirst);
		PreparedStatement ps = null;
		try {
			int count = 0;
			if (deleteFirst) {
				...
				ps.close();
				ps = null;
				logDebug(logger, "setColumnMessageFragments: removed %s message fragments", count);
			}
			...
			if (column.hasMessageFragmentDigest()) {
				...
				for (ColumnMessageFragmentDigest digest : column.getAllMessageFragmentDigests()) {
					logDebug(logger, "inserting %s...", digest);
					...
					ps.setString(4, digest.getType().toString());
					ps.setString(5, (digest.getCellSelection() == null ? null : digest.getCellSelection().toString()));
					ps.setString(6, enumDelimiter);
					ps.setString(7, enumFinalDelimiter);
					ps.setString(8, enumPrefix);
					ps.setString(9, (digest.getRangeStyle() == null ? null : digest.getRangeStyle().toString()));
					count = ps.executeUpdate();
					if (count < 1) {
						throw new SQLException("Failed to insert column message fragment for " + templateID + "," + column.getColumnNumber() + " - " + digest);
					}
				}
			}
			else {
				logDebug(logger, "column has no column messages");
			}
	...
	private Map<String, String> getColumnPropertyMap(GridTemplateColumn column) {
		...
		if (column.getColumnDataSpecDigest().getType().equals(ColumnDataSpecDigest.TYPE_ENTITY)) {
			...
			if (!UtilBase.isEmpty(column.getColumnDataSpecDigest().getEnumSelectorColumnName())) {
				map.put(GuidelineTemplateLoader.COLUMN_PROP_ENUM_SELECTOR_COLUMN, column.getColumnDataSpecDigest().getEnumSelectorColumnName());
			}
		}
		return map;
	}
