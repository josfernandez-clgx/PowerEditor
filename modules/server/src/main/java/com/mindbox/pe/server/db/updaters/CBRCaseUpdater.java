...
public class CBRCaseUpdater extends DateSynonymReferenceUpdater {
	...
	private static final String Q_INSERT_CBR_CASE = "insert into MB_CBR_CASE (case_id, case_base_id, display_name, notes) "
			+ "values (?,?,?,?)";
	...
	private static final String Q_REPLACE_ALL_CBR_CASE_EFFECTIVE_DATE_SYNONYM = "update MB_CBR_CASE_DATE_SYNONYM set effective_synonym_id=? where effective_synonym_id in (" + IN_CLAUSE_LIST_HOLDER + ")";

	private static final String Q_REPLACE_ALL_CBR_CASE_EXPIRATION_DATE_SYNONYM = "update MB_CBR_CASE_DATE_SYNONYM set expiration_synonym_id=? where expiration_synonym_id in (" + IN_CLAUSE_LIST_HOLDER + ")";
	...
	public void deleteCBRCase(int caseID) throws SQLException {
		...
		try {
			...
			if (count < 1) { throw new SQLException("no row was deleted from the case table"); }
			ps.close();
	...
	public void updateCBRCase(CBRCase cbrCase) throws SQLException {
		...
		try {
			...
			while (it.hasNext()) {
				...
				count = ps.executeUpdate();
				if (count < 1) { throw new SQLException("no CBR_ATTRIBUTE_VALUE row was inserted"); }
			}
			ps.close();
			...
			while (it2.hasNext()) {
				...
				count = ps.executeUpdate();
				if (count < 1) { throw new SQLException("no CBR_CASE_ACTIONS_MAPPING row was inserted"); }
			}
			ps.close();
			...
			count = ps.executeUpdate();
			if (count < 1) { throw new SQLException("no CBR_CASE_LABEL_MAPPING row was inserted"); }
			ps.close();
			ps = null;
	...
	public void replaceDateSynonymReferences(DateSynonym[] toBeReplaced, DateSynonym replacement) throws SQLException {
		replaceDateSynonymReferencesInIntersectionTable(toBeReplaced, replacement, Q_REPLACE_ALL_CBR_CASE_EFFECTIVE_DATE_SYNONYM, 
				Q_REPLACE_ALL_CBR_CASE_EXPIRATION_DATE_SYNONYM);
	}
}