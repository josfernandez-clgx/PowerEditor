...
public class GuidelineTemplateLoader extends AbstractLoader {
	...
	private static final String Q_LOAD_COLUMNS = "select column_no,column_name,description,attribute_map," + " title,font,color,width,data_type,"
			+ " multi_select,allow_blank,sort_enum,show_lhs_attr,min_value,max_value,precision_val" + " from MB_TEMPLATE_COLUMN where template_id=? order by column_no";
	...
	private static final String Q_LOAD_TEMPLATE_MESSAGES = "select template_id,column_no,entity_id,cond_delim,cond_final_delim,message_text" + " from MB_TEMPLATE_MESSAGE order by template_id";
	...
	public void load(KnowledgeBaseFilter knowledgeBaseFilterConfig) throws ParseException, SapphireException, SQLException, SAXException, IOException, ParserConfigurationException {
	...
	private void loadTemplates() throws SQLException {
		...
		try {
			...
			while (rs.next()) {
				...
				logger.info("Template: " + id + ",name=" + name + ",parent=" + parentID + ",desc=" + desc + ",status=" + status + ",fitScreen=" + fitToScreen);
			...
			while (rs.next()) {
				...
				if (template == null) {
					...
					if (Util.isEmpty(version)) {
						...
						logger.warn("Template-Version: version empty: set to " + GridTemplate.DEFAULT_VERSION + "; " + templateID + ",v=" + version);
					}
					else {
		...
		try {
			...
			for (Iterator<GridTemplate> iter = GuidelineTemplateManager.getInstance().getAllTemplates().iterator(); iter.hasNext();) {
				...
				while (rs.next()) {
					...
					if (columnNo <= 0) {
						logger.warn("Template_Column: ignored - invalid column no: col=" + columnNo + ",title=" + title + ",attrMap=" + attrMap + ",dataType=" + dataType + "," + ",allowBlank?="
								+ allowBlank);
					}
					else {
						...
						logInfo(logger, "Template_Column: added - col=%s,title=%s,attrMap=%s,dataType=%s,allowBlank?=%b", columnNo, title, attrMap, dataType, allowBlank);
					}
				}
			}
		}
	...
	private void loadColumnMessageFragments(int templateID, int columnNo, GridTemplateColumn column) {
		logDebug(logger, "loadColumnMessageFragments: template=%s, column=%s", templateID, columnNo);
		Connection connection = null;
		ResultSet rs = null;
		PreparedStatement ps = null;
		try {
			...
			while (rs.next()) {
				...
				String type = UtilBase.trim(rs.getString(2)).toLowerCase();
				...
				logDebug(
						logger,
						"loadColumnMessageFragments: found type=%s,cellSel=%s,rangeStyle=%s,enumPrefix=%s,enumDelim=%s,enumFinalDelim=%s,text=%s",
						type,
						cellSelection,
						rangeStyle,
						enumPrefix,
				...
				if (!UtilBase.isEmpty(cellSelection)) {
					if (cellSelection.equalsIgnoreCase("default")) {
						digest.setCellSelection(CellSelectionType.DEFAULT);
					}
					else {
						digest.setCellSelection(CellSelectionType.fromValue(cellSelection));
					}
				}
				digest.setEnumDelimiter(enumDelimiter);
				digest.setEnumFinalDelimiter(enumFinalDelimiter);
				digest.setEnumPrefix(enumPrefix);
				if (!UtilBase.isEmpty(rangeStyle)) {
					digest.setRangeStyle(RangeStyleType.fromValue(rangeStyle.toLowerCase()));
				}
				digest.setText(messageText);
				digest.setType(MessageConfigType.fromValue(type));

				column.addColumnMessageFragment(digest);
				logInfo(logger, "Added Column Message Fragment: template=%s,col=%s,type=%s", templateID, columnNo, type);
			}
			rs.close();
			rs = null;
		}
		catch (Exception ex) {
			logError(logger, ex, "Failed to get column message fragments for template %s,col %s", templateID, columnNo);
		}
		finally {
			DBUtil.closeLocallyManagedResources(rs, ps);
			DBConnectionManager.getInstance().freeConnection(connection);
	...
	private void loadTemplateMessages() throws SQLException {
		final EntityType etDef = ConfigurationManager.getInstance().getEntityTypeForMessageContext();
		final GenericEntityType entityTypeForMessage = (etDef == null ? null : GenericEntityType.forID(etDef.getTypeID().intValue()));
		...
		try {
			...
			while (rs.next()) {
				...
				GridTemplate template = GuidelineTemplateManager.getInstance().getTemplate(templateID);
				if (template == null) {
					logger.warn("Template-Message: ignored - template not found: temp=" + templateID + ",col=" + columnNo + ",entityID=" + entityID);
				}
				else if (columnNo > template.getNumColumns()) {
					logger.warn("Template-Message: ignored - invalid column no: temp=" + templateID + ",col=" + columnNo + ",entityID=" + entityID);
				}
				else {
	...
	private void loadTemplateDeployRules() throws SQLException, SAXException, IOException, ParserConfigurationException {
		...
		for (GuidelineRuleInfo ruleInfo : ruleInfoList) {
			...
			try {
				...
				if (template == null) {
					logger.warn("Template-Rule: ignored - template not found: temp=" + ruleInfo.getTemplateID() + ",col=" + ruleInfo.getColumnNo());
				}
				else if (ruleInfo.getColumnNo() > template.getNumColumns()) {
					logger.warn("Template-Rule: ignored - invalid column no: temp=" + ruleInfo.getTemplateID() + ",col=" + ruleInfo.getColumnNo());
				}
				else {
					...
					logger.info("Template-Rule: added - temp=" + ruleInfo.getTemplateID() + ",col=" + ruleInfo.getColumnNo() + ",strSize=" + (ruleDefStr == null ? 0 : ruleDefStr.length()));
				}
			}
			catch (SAXException ex) {
				logger.error("Failed to load template (XML Error): template=" + ruleInfo.getTemplateID() + ", column=" + ruleInfo.getColumnNo(), ex);
