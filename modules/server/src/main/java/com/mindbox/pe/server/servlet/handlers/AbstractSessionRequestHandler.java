...
public abstract class AbstractSessionRequestHandler<T extends SessionRequest<?>> extends AbstractActionRequestHandler<T> {
	...
	private ErrorResponse authenticate(T request, HttpServletRequest httpServletRequest) {
		try {
			...
			return new ErrorResponse(ErrorResponse.SERVER_ERROR, "msg.error.generic.service", new Object[] { "Failed to authenticate session: " + ex.getMessage() });
		}
	}
	...
	private ErrorResponse authorize(T request) {
		...
		try {
			...
			logger.error(ex.getMessage(), ex);
			return new ErrorResponse(ErrorResponse.SERVER_ERROR, "msg.error.generic.service", new Object[] { "Failed to authorize session: " + ex.getMessage() });
		}
	}
