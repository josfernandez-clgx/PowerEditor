...
public class GridManager extends AbstractCacheManager {
	...
	private static boolean isContainedIn(GuidelineContext[] contexts, ProductGrid grid) {
		...
		for (int i = 0; i < contexts.length; i++) {
			...
			if (genericEntityType != null) {
				...
				genericEntityType = ConfigurationManager.getInstance().getEntityConfigHelper().findEntityTypeForCategoryType(contexts[i].getGenericCategoryType());
	...
	// TT-21 determine if the spcified cell value matches the specified filter value
	private static boolean matchesAttributeValue(final Object cellValue, final String value, final boolean ignoreCase) {
		if (UtilBase.isEmpty(value)) {
			return cellValue != null;
		}
		else {
			if (EnumValues.class.isInstance(cellValue)) {
				EnumValues<?> enumValues = EnumValues.class.cast(cellValue);
				if (!enumValues.isEmpty()) {
					final boolean containsValue = enumValues.containsStringValueForDisplayLabel(value, ignoreCase);
					return (enumValues.isSelectionExclusion() && !containsValue) || (!enumValues.isSelectionExclusion() && containsValue);
				}
				else {
					return false;
				}
				...
				return cellValue != null && cellValue.toString().indexOf(value) >= 0;
			}
		}
	}
	...
	private static GuidelineReportData toGuidelineReportData(ProductGrid prodGrid, String userId) throws ServerException {
		...
		GuidelineReportData data = new GuidelineReportData(
				templateID,
				prodGrid.getID(),
				template.getName(),
				template.getUsageType(),
				template.getVersion(),
				prodGrid.extractGuidelineContext(),
				prodGrid.getCreationDate(),
	...
	public int getGridID(int templateID, GuidelineContext[] context, DateSynonym effDate, DateSynonym expDate) {
		...
		for (Iterator<ProductGrid> iter = list.iterator(); iter.hasNext();) {
			ProductGrid element = iter.next();
			if (ServerContextUtil.hasSameContext(element, context) && Util.isSame(element.getEffectiveDate(), effDate) && Util.isSame(element.getExpirationDate(), expDate)) {
	...
	private List<GuidelineReportData> getGuidelineReportData(final String className, final String attrName, final String value, final String username) throws ServerException {
		if (className == null || attrName == null) {
			return getAllGuidelineReportData(username);
		}

		final List<GuidelineReportData> resultList = new ArrayList<GuidelineReportData>();
		final List<GridTemplate> templateList = GuidelineTemplateManager.getInstance().getTemplates(className, attrName);
		for (final GridTemplate element : templateList) {
			final int col = GuidelineTemplateManager.getInstance().findColumnForAttribReference(element, className, attrName);
			if (col > 0) {
				final List<ProductGrid> gridList = gridTemplateMap.get(new Integer(element.getID()));
				if (gridList != null) {
					final List<ProductGrid> tempList = new ArrayList<ProductGrid>();
					for (final ProductGrid grid : gridList) {
						// add if grid contains a value in the right column
						boolean gridAdded = false;
						for (int row = 1; row <= grid.getNumRows() && !gridAdded; row++) {
							try {
								final Object cellValue = grid.getCellValueObject(row, col, null);
								logDebug(logger, "getConsolidatedGuidelineReportData: cell-value=%s", cellValue);
								// check matching ignoring case
								if (matchesAttributeValue(cellValue, value, true)) {
									tempList.add(grid);
									gridAdded = true;
								}
							}
							catch (InvalidDataException ex) {
								logWarn(logger, ex, "getConsolidatedGuidelineReportData: ignored %s", grid);
							}
						}
					}
			...
			if (entityIDs != null && entityIDs.length > 0) {
				for (Iterator<GuidelineReportData> iter = result.iterator(); iter.hasNext();) {
					...
					if (catIDs != null && catIDs.length > 0) {
						GenericEntityIdentity[] entities = EntityManager.getInstance().getGenericEntitiesInCategorySetAtAnyTime(types[i].getCategoryType(), catIDs, true); // true to indicate that we don't care about dates on entity-category associations
	...
	public synchronized boolean updateCache(AbstractGuidelineGrid cachedGrid, AbstractGuidelineGrid abstractgrid1) {
		...
		DateSynonym cachedEffDate = (abstractgrid1.getEffectiveDate() == null ? null : DateSynonymManager.getInstance().getDateSynonym(abstractgrid1.getEffectiveDate().getID()));
		DateSynonym cachedExpDate = (abstractgrid1.getExpirationDate() == null ? null : DateSynonymManager.getInstance().getDateSynonym(abstractgrid1.getExpirationDate().getID()));
