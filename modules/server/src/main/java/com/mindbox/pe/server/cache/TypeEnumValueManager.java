...
public class TypeEnumValueManager extends AbstractCacheManager {
	...
	private final Map<String, List<TypeEnumValue>> typeEnumMap = new HashMap<String, List<TypeEnumValue>>();
	...
	public boolean hasEnumValue(String typeKey, int enumID) {
		return getEnumValue(typeKey, enumID) != null;
	}
	...
	public void update(String typeKey, TypeEnumValue enumValue) {
		synchronized (typeEnumMap) {
			final List<TypeEnumValue> list = getEnumValueList(typeKey);
			list.remove(getEnumValue(typeKey, enumValue.getId()));
			list.add(enumValue);
		}
	}
