...
public class DomainManager extends AbstractCacheManager implements DomainClassProvider {
	...
	private static DomainManager singleton = null;
	...
	public synchronized void addDomainClass(DomainClass domainClass) {
		...
		if (domainClass.getName() == null || domainClass.getName().length() == 0) throw new IllegalArgumentException("Cannot add a domain class with no name");
		...
		for (DomainClass dc : dcList) {
			if (dc != null) {
				for (DomainAttribute da : dc.getDomainAttributes()) {
					if (da.getContextlessLabel().equals(contextlessLabel)) {
						return da;
					}
				}
			}
	...
	public DomainAttribute findDomainAttributeByContextlessLabel(String className, String contextlessLabel) {
		...
		if (dc != null) {
			...
			for (Iterator<DomainAttribute> iter = dc.getDomainAttributes().iterator(); iter.hasNext();) {
				DomainAttribute da = iter.next();
				if (da.getName().equals(contextlessLabel)) {
					return da;
				}
			}
		}
		...
		for (DomainClass dc : dcList) {
			if (dc != null) {
				for (DomainTranslation dt : dc.getDomainTranslations()) {
					if (dt.getContextlessLabel().equals(contextlessLabel)) {
						return dt;
					}
				}
			}
	...
	private DomainClassLink findLinkToParent(List<DomainClassLink> parentLinkList, String parent) {
		for (DomainClassLink domainclasslink : parentLinkList) {
			if (domainclasslink.getParentName().equalsIgnoreCase(parent)) {
	...
	public synchronized void finishLoading() throws InvalidDataException {
		...
		for (final TemplateUsageType usageType : TemplateUsageType.getAllInstances()) {
			logger.debug("... finishLoading: checking " + usageType);
			RuleGenerationConfigHelper ruleConfig = ConfigurationManager.getInstance().getRuleGenerationConfigHelper(usageType);
			ControlPatternConfigHelper controlPatternConfig = ruleConfig.getControlPatternConfig();
			logger.debug("... finishLoading: ruleConfig=" + ruleConfig + " , patternConfig=" + controlPatternConfig);
			DomainClass domainClass = getDomainClass(controlPatternConfig.getPattern().getClazz());
			// Check if a domain class for the "control" Pattern in PowerEditorConfiguration.xml exists.
			// If it does not, throw an exception.
			if (domainClass == null) {
				throw new IllegalStateException("No " + controlPatternConfig.getPattern().getClazz() + " domain class present in domain xml file.");
			}
			Map<String, String> configAttributes = ((ControlPatternConfigHelper) controlPatternConfig).getAttributes();
			for (Iterator<String> iter = configAttributes.keySet().iterator(); iter.hasNext();) {
				...
				if (domainAttr == null) {
					throw new IllegalStateException("No matching domain attribute found for " + key);
				}
			}
		}
		...
		for (DomainClass domainClass : domainClassMap.values()) {
			...
			for (DomainClassLink link : domainClass.getDomainClassLinks()) {
				...
				if (parentClass == null) {
					throw new IllegalStateException(link + " contains invalid parent class name: class " + parentName + " not found");
				}
				...
				if (attribute.hasEnumValue()) {
					...
					for (int i = 0; i < values.length; i++) {
						...
						if (!values[i].isValidForDomainEnumValue()) {
							throw new IllegalStateException("Illegal EnumValue configuration for " + values[i]);
						}
						...
						// TODO Determine why this is needed (add enum value to UIConfiguration)
						//		Constants.addEnumValue(key, values[i]);
					}

					// add to deployment manager
					DeploymentManager.getInstance().addEnumValueMap(domainClass.getName(), attribute.getName(), enumValueList);
				}
			} // for
