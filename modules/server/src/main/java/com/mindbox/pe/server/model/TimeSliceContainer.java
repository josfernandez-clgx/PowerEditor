...
public class TimeSliceContainer {
	...
	public synchronized void add(TimeSlice timeSlice) {
		...
		if (frozen) {
			throw new IllegalStateException("This is frozen. No changes can be made");
		}
		if (!tsSet.add(timeSlice)) {
			throw new IllegalArgumentException("Time slice " + timeSlice.getName() + " was already added");
		}
	}
	...
	public List<TimeSlice[]> generateTimeSliceGroups(GenerationParams generationParams, EntityDataHolder entityDataHolder) {
		...
		if (generationParams.hasGenericCategoryAsCellValue()) {
			...
			for (final DateSynonym dateSynonym : dateSynonymSet) {
				timeSliceList = getApplicableTimeSlices_internal(startDS, dateSynonym);
