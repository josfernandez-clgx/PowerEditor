...
public class MessageProcessor extends ObjectDepthFirst {
	...
	private static String[] getRangeValueStrings(IRange range, ColumnDataSpecDigest colDataSpec) {
		...
		if (range instanceof DateRange) {
			...
			DateFormat formatter = ConfigurationManager.getInstance().getDefaultRuleGenerationConfigHelper().getMessageDateRangeFormat();
	...
	public static void writeCategoryOrEntityValue(StringBuilder buffer, CategoryOrEntityValue value) {
		if (value != null) {
			if (value.isForEntity()) {
				...
				GenericCategory category = EntityManager.getInstance().getGenericCategory(value.getEntityType().getCategoryType(), value.getId());
				if (category == null) {
	...
	public static void writeCategoryOrEntityValues(MessageConfiguration msgConfig, StringBuilder buffer, CategoryOrEntityValues value) {
		if (value != null) {
	...
	private static void writeDate(StringBuilder buffer, Date date) {
		buffer.append(ConfigurationManager.getInstance().getDefaultRuleGenerationConfigHelper().formatAsMessageDate(date));
	}
	...
	public static void writeEnum(AbstractTemplateColumn column, EnumValues<?> enumvalues, StringBuilder buffer, MessageConfiguration msgConfig) {
		...
		for (int i = 0; i < numEnums; i++) {
			...
			if (ev instanceof EnumValue) {
				...
				enumValue = DeploymentManager.getInstance().getEnumDisplayValue(column.getMAClassName(), column.getMAAttributeName(), dispValue, false);
			}
	...
	public static void writeRange(IRange range, StringBuilder buffer, ColumnDataSpecDigest colDataSpec, MessageConfiguration msgConfig) {
		...
		if (msgConfig.isRangeStyleBracketed()) {
			if (isEqual)
				buffer.append(floor);
			else if (range.getFloor() != null && range.getCeiling() != null && range.getFloor().doubleValue() == range.getCeiling().doubleValue()) {
				buffer.append("= ");
	 ...
	 */
	public static void writeVal(AbstractTemplateColumn gridColumn, Object obj, StringBuilder buffer, MessageConfiguration msgConfig, String defaultWriteValue) {
		if (obj == null) {
			// handle boolean
		...
		Logger.getLogger(MessageProcessor.class).debug("... writeVal: " + obj + " (" + obj.getClass().getName() + ')' + ", col=" + gridColumn);
		...
		if (obj instanceof IRange) {
			...
			if (gridColumn.getColumnDataSpecDigest().getType().equals(ColumnDataSpecDigest.TYPE_ENUM_LIST) && gridColumn.getColumnDataSpecDigest().getMappedAttribute() != null) {
				DomainAttribute attribute = DomainManager.getInstance().getDomainAttribute(gridColumn.getMAClassName(), gridColumn.getMAAttributeName());
				if (attribute != null) {
					String value = DeploymentManager.getInstance().getEnumDisplayValue(gridColumn.getMAClassName(), gridColumn.getMAAttributeName(), obj.toString(), false);
	...
	public synchronized String process(Message messageObj, AbstractGenerateParms generateParams, TemplateMessageDigest messageDigest) {
		...
		String messageStr = process_internal(messageObj, generateParams);

		StringBuilder resultBuffer = new StringBuilder();
		resultBuffer.append("(");
		resultBuffer.append(ConfigurationManager.getInstance().getDefaultRuleGenerationConfigHelper().getMessageFormatConversionFunction());
		resultBuffer.append(" \"");
		resultBuffer.append(RuleGeneratorHelper.formatForSprintf(messageStr));
		resultBuffer.append("\"");
	 ...
	 * This does not format the string for AE sprintf function.
	 * 
	 * NOTE: The returned value must be encoded using {@link RuleGeneratorHelper#formatForSprintf(String)} method. 
	 */
	...
	public Object visit(ColumnMessagesLiteral n, Object argu) {
		...
		logDebug(logger, ">>> visit(ColumnMessageLiteral) for %s", argu);
		...
		if (conditionalDelimiter == null) {
			conditionalDelimiter = Constants.DEFAULT_CONDITIONAL_DELIMITER;
		}
		String conditionalFinalDelimiter = getConditionalFinalDelimiterTouse(generateParams);
		if (conditionalFinalDelimiter == null) {
			conditionalFinalDelimiter = Constants.DEFAULT_CONDITIONAL_FINAL_DELIMITER;
		}
		for (int i = 0; i < intArray.length; i++) {
			...
			logDebug(logger, "    visit(ColumnMessageLiteral): column no=%s", colNum);
			...
			logDebug(logger, "    visit(ColumnMessageLiteral): cell value=%s", cellValue);
			...
			if (parsedMessage == null || cmfd == null) {
				logDebug(logger, "    visit(ColumnMessageLiteral): no column message found - adding column value: %s", generateParams.getColumnValue(colNum));
				...
				logDebug(logger, "    visit(ColumnMessageLiteral): column message processed = %s", columnMessageStr);
