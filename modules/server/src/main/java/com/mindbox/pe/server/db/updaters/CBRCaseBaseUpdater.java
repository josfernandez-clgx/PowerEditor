...
public class CBRCaseBaseUpdater extends DateSynonymReferenceUpdater {
	...
	private static final String Q_REPLACE_ALL_CBR_CASE_BASE_EFFECTIVE_DATE_SYNONYM = "update MB_CBR_CASE_BASE_DATE_SYN set effective_synonym_id=? where effective_synonym_id in ("
			+ IN_CLAUSE_LIST_HOLDER + ")";

	private static final String Q_REPLACE_ALL_CBR_CASE_BASE_EXPIRATION_DATE_SYNONYM = "update MB_CBR_CASE_BASE_DATE_SYN set expiration_synonym_id=? where expiration_synonym_id in ("
			+ IN_CLAUSE_LIST_HOLDER + ")";
	...
	public void deleteCBRCaseBase(int caseBaseID) throws SQLException {
		...
		try {
			...
			if (count < 1) {
				throw new SQLException("no row was deleted from the case base table");
			}
			ps.close();
			ps = null;
	...
	public void replaceDateSynonymReferences(DateSynonym[] toBeReplaced, DateSynonym replacement) throws SQLException {
		replaceDateSynonymReferencesInIntersectionTable(
				toBeReplaced,
				replacement,
				Q_REPLACE_ALL_CBR_CASE_BASE_EFFECTIVE_DATE_SYNONYM,
	...
	public void updateCBRCaseBase(CBRCaseBase caseBase) throws SQLException {
		...
		try {
			...
			logger.debug(" updateCBRCaseBase: updating date synonym " + caseBase.getEffectiveDate() + "," + caseBase.getExpirationDate());
			...
			if (count < 1) {
				...
				count = ps.executeUpdate();
				if (count < 1) {
					throw new SQLException("Failed to update or insert the casebase-date synonym row");
				}
			}
