...
public class FetchAllEnumerationValuesRequestHandler extends AbstractSessionRequestHandler<FetchAllEnumerationValuesRequest> {
	...
	@Override
	protected ResponseComm handleRequest(FetchAllEnumerationValuesRequest requestcomm, HttpServletRequest httpservletrequest) throws DataValidationFailedException, LockException,
			ServletActionException, ServerException {
		List<EnumValue> list = new ArrayList<EnumValue>();
		list.addAll(ConfigurationManager.getInstance().getEnumerationSourceConfigHelper().getAllEnumValues(requestcomm.getSourceName()));
