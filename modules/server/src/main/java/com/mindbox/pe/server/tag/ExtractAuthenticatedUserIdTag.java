...
public class ExtractAuthenticatedUserIdTag extends AbstractVarTag {
	...
	public int doStartTag() throws JspException {
		...
		if (!isEmptyAfterTrim(ConfigurationManager.getInstance().getPowerEditorConfiguration().getServer().getSession().getUserIDCookie())) {
			...
			userName = extractUserNameFromHeader(request, ConfigurationManager.getInstance().getPowerEditorConfiguration().getServer().getSession().getUserIDCookie());
			if (isEmptyAfterTrim(userName)) {
				userName = extractUserNameFromCookie(request, ConfigurationManager.getInstance().getPowerEditorConfiguration().getServer().getSession().getUserIDCookie());
			}
		}
	...
	private String extractUserNameFromCookie(HttpServletRequest request, String cookieName) {
		...
		if (cookies != null) {
			for (Cookie cookie : cookies) {
				logger.debug(String.format("Checking cookie: name=%s,value=%s,path=%s,domain=%s", cookie.getName(), cookie.getValue(), cookie.getPath(), cookie.getDomain()));
