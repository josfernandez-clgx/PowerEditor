...
public class TemplateUtil {
	...
	public static void generateAndSetLHSRuleDefinition(GridTemplate template) {
		...
		for (; columnNo <= template.getNumColumns(); columnNo++) {
			...
			if (column != null) {
				...
				condition.setValue(RuleElementFactory.getInstance().createValue(RuleElementFactory.getInstance().createColumnReference(column.getColumnNumber())));
	...
	public static GridTemplateColumn generateColumnsFromAttribute(DomainClassProvider domainClassProvider, TemplateUsageType usage, String reference, int columnNo) {
		return generateColumnsFromAttribute(domainClassProvider, usage, reference, Condition.OP_EQUAL, columnNo, false);
	}

	public static GridTemplateColumn generateColumnsFromAttribute(DomainClassProvider domainClassProvider, TemplateUsageType usage, String reference, int op, int columnNo) {
		return generateColumnsFromAttribute(domainClassProvider, usage, reference, op, columnNo, true);
	}


	private static GridTemplateColumn generateColumnsFromAttribute(DomainClassProvider domainClassProvider, TemplateUsageType usage, String reference, int op, int columnNo,
			boolean checkForRange) {
		...
		if (dc == null) {
			throw new IllegalArgumentException("Invalid reference - class not found: " + reference);
		}
		DomainAttribute attribute = dc.getDomainAttribute(strs[1]);
		if (attribute == null) {
			throw new IllegalArgumentException("Invalid reference - attribute not found: " + reference);
		}
		...
		else if (attribute.getDeployType() == DeployType.CODE || attribute.getDeployType() == DeployType.STRING) {
			digest.setType("String");
			digest.setIsBlankAllowed(true);
		}
	...
	private static void generateRuleAction(GridTemplate template, RuleDefinition ruleDef, ActionTypeDefinition actionType) {
		...
		for (int i = 0; i < paramDefs.length; i++) {
			...
			ruleAction.add(
					RuleElementFactory.getInstance().createFunctionParameter(
							i + 1,
	...
	private static void regenerateRuleAction(RuleDefinition rd, ActionTypeDefinition newActionType, ActionTypeDefinition oldActionType) {
