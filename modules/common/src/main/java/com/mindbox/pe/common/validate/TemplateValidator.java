...
public class TemplateValidator {
	...
	private static void checkForIncompleteElements(CompoundLHSElement element, String ruleName, WarningConsumer warningConsumer) {
		if (element.isEmpty()) {
			warningConsumer.addWarning(
					WarningInfo.WARNING,
					"No pattern generated for the empty node; there is a " + toCompoundTypeString(element.getType()) + " node " + " that is empty or only contains empty elements.",
					ruleName);
		}
		else {
	 ...
	 * Use this to perform complete validation of a rule.
	 * @param ruleDef the rule definition
	 * @param template the template
