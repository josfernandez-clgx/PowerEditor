...
public class CategoryOrEntityValue implements Serializable, GridCellValue {
	...
	public static CategoryOrEntityValue valueOf(String strValue, String entityType, boolean allowEntity, boolean allowCategory) {
		...
		if (!allowCategory && !allowEntity) throw new IllegalArgumentException("at least one of allowEntity or allowCategory must be true");
		...
		try {
			...
			Logger.getLogger(CategoryOrEntityValue.class).warn("Failed to parse for CategoryEntityValue: " + strValue, exception);
			return null;
		}
	}
