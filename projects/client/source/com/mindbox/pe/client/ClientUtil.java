...
public final class ClientUtil extends UtilBase implements GuidelineActionProvider {
	...
	public static boolean hasProductionRestrictions(String status) {
		return !ClientUtil.checkPermissionByPrivilegeName(PrivilegeConstants.PRIV_EDIT_PRODUCTION_DATA)
		...
		if (context != null) {
			for (int i = 0; i < context.length; i++) {
				if (context[i].getGenericEntityType() != null) {
					...
					grid.setGenericCategoryIDs(
							getEntityConfiguration().findEntityTypeForCategoryType(context[i].getGenericCategoryType()),
							context[i].getIDs());
				}
			}
		}
	...
	/**
	 * Calls {@link com.mindbox.pe.client.MainApplication#checkViewOrEditGuidelinePermission(com.mindbox.pe.common.config.GuidelineTabConfig)}
	 * @param gtConfig
	 * @return boolean
	 */
	public static boolean checkViewOrEditGuidelinePermission(GuidelineTabConfig gtConfig) {
		return instance.parent.checkViewOrEditGuidelinePermission(gtConfig);
	}

	/**
	 * Calls {@link com.mindbox.pe.client.MainApplication#checkViewOrEditTemplatePermission(com.mindbox.pe.common.config.GuidelineTabConfig)}
	 * @param gtConfig
	 * @return boolean
	 */
	public static boolean checkViewOrEditTemplatePermission(GuidelineTabConfig gtConfig) {
	...
	public static boolean checkViewOrEditAnyTemplatePermission() {
		...
		for (Iterator<Privilege> iter1 = privSet.iterator(); iter1.hasNext();) {
			...
			if (priv.getPrivilegeType() == PrivilegeConstants.USAGE_TYPE_PRIV
					&& priv.getName().indexOf(PrivilegeConstants.VIEW_AND_EDIT_USAGE_TEMPLATE_PRIV_NAME_SUFFIX) >= 0) {
				return true;
			}
		}
		return false;
	 ...
	 * @since 5.0.0
	 */
	//TODO Vineet...check for visibility like we check in guidelines?
	 ...
	 * @since 5.0.0
	 */
	public static boolean checkViewOrEditEntityPermission(EntityTypeDefinition entityTypeDef) {
	 ...
	 * @since 5.0.0
	 */
	public static boolean checkEditEntityPermission(EntityTypeDefinition entityTypeDef) {
		String editPrivilegeOnCurrentEntity = PrivilegeConstants.EDIT_PRIV_NAME_PREFIX + entityTypeDef.getName();
	...
	public static EntityConfiguration getEntityConfiguration() {
		return instance.parent.getEntityConfiguration();
	}

	public static GenericEntityType getEntityTypeForMessageContext() {
		return instance.parent.getEntityConfiguration().getEntityTypeForMessageContext() == null
				? null
				: GenericEntityType.forID(instance.parent.getEntityConfiguration().getEntityTypeForMessageContext().getTypeID());
	}
	...
	public static final Logger getLogger() {
		if (logger == null) {
			...
			PatternLayout defaultLayout = new PatternLayout("%d{yyyy-MM-dd HH:mm:ss} %c{4} [%-5p]: %m%n");
			...
			logger = Logger.getLogger("com.mindbox.PowerEditor");
		}
		return logger;
	}
	...
	public static DateSynonym fetchDateSynonym(int dsID) throws ServerException {
		return (DateSynonym) getCommunicator().fetch(dsID, EntityType.DATE_SYNONYM, false);
	}

	public static CBRCaseClass[] fetchAllCBRCaseClasses() throws ServerException {
		List<CBRCaseClass> list = getCommunicator().search(new AllSearchFilter<CBRCaseClass>(EntityType.CBR_CASE_CLASS));
	...
	public static CBRCaseClass fetchCBRCaseClass(int caseClassID) throws ServerException {
		return (CBRCaseClass) getCommunicator().fetch(caseClassID, EntityType.CBR_CASE_CLASS, false);
	}

	public static CBRScoringFunction[] fetchAllCBRScoringFunctions() throws ServerException {
		List<CBRScoringFunction> list = getCommunicator().search(new AllSearchFilter<CBRScoringFunction>(EntityType.CBR_SCORING_FUNCTION));
	...
	public static CBRScoringFunction fetchCBRScoringFunction(int scoringFunctionID) throws ServerException {
		return (CBRScoringFunction) getCommunicator().fetch(scoringFunctionID, EntityType.CBR_SCORING_FUNCTION, false);
	}

	public static CBRValueRange[] fetchAllCBRValueRanges() throws ServerException {
		List<CBRValueRange> list = getCommunicator().search(new AllSearchFilter<CBRValueRange>(EntityType.CBR_VALUE_RANGE));
	...
	public static CBRValueRange fetchCBRValueRange(int valueRangeID) throws ServerException {
		return (CBRValueRange) getCommunicator().fetch(valueRangeID, EntityType.CBR_VALUE_RANGE, false);
	}

	public static CBRCaseAction[] fetchAllCBRCaseActions() throws ServerException {
		List<CBRCaseAction> list = getCommunicator().search(new AllSearchFilter<CBRCaseAction>(EntityType.CBR_CASE_ACTION));
	...
	public static CBRCaseAction fetchCBRCaseAction(int caseActionID) throws ServerException {
		return (CBRCaseAction) getCommunicator().fetch(caseActionID, EntityType.CBR_CASE_ACTION, false);
	}

	public static CBRAttributeType[] fetchAllCBRAttributeTypes() throws ServerException {
		List<CBRAttributeType> list = getCommunicator().search(new AllSearchFilter<CBRAttributeType>(EntityType.CBR_ATTRIBUTE_TYPE));
	...
	public static CBRAttributeType fetchCBRAttributeType(int AttributeTypeID) throws ServerException {
		return (CBRAttributeType) getCommunicator().fetch(AttributeTypeID, EntityType.CBR_ATTRIBUTE_TYPE, false);
	}

	public static ActionTypeDefinition fetchActionTypeDefinition(int actionID) throws ServerException {
		return (ActionTypeDefinition) getCommunicator().fetch(actionID, EntityType.GUIDELINE_ACTION, false);
	}

	public static GridTemplate fetchGuidelineTemplateDefinition(int templateID) throws ServerException {
		return (GridTemplate) getCommunicator().fetch(templateID, EntityType.TEMPLATE, false);
	}
	...
	public static List<TestTypeDefinition> getTestTypes() throws ServerException {
		AllSearchFilter<TestTypeDefinition> filter = new AllSearchFilter<TestTypeDefinition>(EntityType.GUIDELINE_TEST_CONDITION);
	...
	private ClientUtil() {
		...
		ClientUtil.getLogger().debug("... locale " + locale);
	...
	public ActionTypeDefinition getActionType(int id) {
		try {
			return (ActionTypeDefinition) getCommunicator().fetch(id, EntityType.GUIDELINE_ACTION, false);
		}
		catch (Exception ex) {
	...
	public TestTypeDefinition getTestType(int id) {
		try {
			return (TestTypeDefinition) getCommunicator().fetch(id, EntityType.GUIDELINE_TEST_CONDITION, false);
		}
		catch (Exception ex) {
			handleRuntimeException(ex);
			return null;
	...
	private ImageIcon makeImageIcon(JApplet japplet, String s) {
		ImageIcon imageicon = iconMap.get(s);

		if (imageicon == null) {
			URL url = getURL(japplet, "../" + getGIF(s)); // PE Applet's base URL is /powereditor/auth
	...
	public void showText(String titleKey, String text) {
		...
		JOptionPane.showMessageDialog(
				...
				ClientUtil.getInstance().getLabel(titleKey),
	...
	private String generateDetailMessage(ErrorResponse cause) {
		if (cause.hasMessageResource()) {
			try {
				String message = cause.getErrorResourceParams() == null ? getMessage(cause.getErrorResourceKey()) : getMessage(
						cause.getErrorResourceKey(),
						cause.getErrorResourceParams());
				return message;
			}
			catch (Exception ex) {
	...
	public String getLabel(CategoryTypeDefinition categoryDef) {
		return getLabel("label.category." + categoryDef.getName(), categoryDef.getName());
	}

	public String getLabel(GenericEntityType type, CategoryTypeDefinition categoryDef) {
