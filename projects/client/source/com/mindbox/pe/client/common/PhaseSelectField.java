...
public class PhaseSelectField extends AbstractDropSelectField {
	...
	private void initPhaseCombo() {
		if (phaseList == null) {
			...
			phaseList.setSelectionMode((forMultiSelect
					? ListSelectionModel.MULTIPLE_INTERVAL_SELECTION
					: ListSelectionModel.SINGLE_SELECTION));

			// populate phase
			try {
				List<Phase> phases = ClientUtil.getCommunicator().search(new AllSearchFilter<Phase>(EntityType.PROCESS_PHASE));
	...
	private int findPhaseIndex(int phaseID) {
		for (int i = 0; i < phaseList.getModel().getSize(); i++) {
			if (phaseID == ((Phase) phaseList.getModel().getElementAt(i)).getID()) { return i; }
		}
		return -1;
	}
