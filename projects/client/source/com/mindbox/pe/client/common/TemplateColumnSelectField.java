...
public final class TemplateColumnSelectField extends JPanel {
			...
			try {
				...
				for (int i = 1; i <= template.getNumColumns(); i++) {
					...
					if (!forLHS || !column.getColumnDataSpecDigest().isRuleIDType()) {
						...
						if (colOK) {
							if (forEntityTestFunctionOperator
									&& column.getColumnDataSpecDigest().getType().equals(ColumnDataSpecDigest.TYPE_ENTITY)) {
								colVals.add(i + " " + template.getColumn(i).getTitle());
							}
							else if (!forMembershipOperator
	...
	public TemplateColumnSelectField(GridTemplate template, boolean forLHS) {
		...
		findColumnButton = UIFactory.createButton("",
				"image.btn.find.column", new FindColL(), null);
	...
	private void refreshText() {
		columnField.setText(reference == null ? "" : 
			"Column " + reference.getColumnNo() + ": " + template.getColumn(reference.getColumnNo()).getTitle());
	}
