...
public final class DataFilterPanel extends PanelBase {
		...
		public void performAction(ActionEvent e) {
			if (((JCheckBox) e.getSource()).isSelected()) {
				...
				if (guidelineFilterPanel != null) guidelineFilterPanel.setEnabledPanel(true);
					 ...
					 !(templateCheckBoxField.isVisible() && templateCheckBox.isSelected()) && 
	...
	private void initPanel(boolean useBorder) {
		...
		hasParameterFeature = ClientUtil.getUserSession().getFeatureConfiguration().isFeatureEnabled(FeatureConfiguration.PARAMETER_FEATURE);
		hasCbrFeature = ClientUtil.getUserSession().getFeatureConfiguration().isFeatureEnabled(FeatureConfiguration.CBR_FEATURE);
		hasProcessDataFeature = ClientUtil.getUserSession().getFeatureConfiguration().isFeatureEnabled(FeatureConfiguration.PHASE_FEATURE);
	...
	public boolean hasSelection() {
		...
		if (filter.isIncludeGuidelines() || filter.isIncludeTemplates() || filter.isIncludeParameters() || filter.isIncludeEntities()
				|| filter.isIncludeGuidelineActions() || filter.isIncludeTestConditions() || filter.isIncludeDateSynonyms()
				|| filter.isIncludeSecurityData() || filter.isIncludeCBR() || filter.isIncludeProcessData())
			return true;
		else
			return false;
	}
	...
	public void setAllSelectionCriteria(boolean selectState) {
		...
		if (hasParameterFeature && parameterCheckBoxField.isVisible())
			parameterCheckBox.setSelected(selectState);
