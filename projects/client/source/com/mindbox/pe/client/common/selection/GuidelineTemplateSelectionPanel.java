...
public class GuidelineTemplateSelectionPanel extends PanelBase {
		...
		public void valueChanged(TreeSelectionEvent e) {
			...
			if (node == null) {
				...
				if (node instanceof UsageTypeTreeNode) {
					...
					try {
						// populate detail panel
						fireUsageGroupdSelected(((UsageGroupTreeNode) node).getGuidelineTabConfig());
					}
					catch (CanceledException e1) {
						if (lastSelectedPath == null) {
							tree.clearSelection();
	...
	private void fireUsageGroupdSelected(GuidelineTabConfig config) throws CanceledException {
		if (guidelineSelectionListener != null) {
