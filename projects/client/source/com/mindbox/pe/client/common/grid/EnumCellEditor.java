...
public class EnumCellEditor extends DefaultCellEditor {
	...
	public static final List<EnumValue> fetchSortedEnumValueListIfConfigured(ColumnDataSpecDigest columnDataSpecDigest) {
		...
		if (!(columnDataSpecDigest.getEnumSourceType() == EnumSourceType.EXTERNAL && columnDataSpecDigest.isEnumSelectorColumnSet())) {
			enumValueList.addAll(EnumValuesDataHelper.getAllEnumValues(
	...
	public static final List<EnumValue> fetchSortedApplicableEnumValueListIfConfigured(ColumnDataSpecDigest columnDataSpecDigest,
			String selectorValue) {
		List<EnumValue> enumValueList = new ArrayList<EnumValue>();
		if (columnDataSpecDigest.getEnumSourceType() == EnumSourceType.EXTERNAL && columnDataSpecDigest.isEnumSelectorColumnSet()) {
			enumValueList.addAll(ClientUtil.getEnumerationSourceProxy().getApplicableEnumValues(
					columnDataSpecDigest.getEnumSourceName(),
					selectorValue));
		}
		if (columnDataSpecDigest.isEnumValueNeedSorted()) {
			Collections.sort(enumValueList);
	...
	public EnumCellEditor(ColumnDataSpecDigest columnDataSpecDigest, String columnName, boolean viewOnly,
			AbstractGridTableModel<?> tableModel) {
	...
	private void resetListModelForSelectionIfNeeded(int row) {
		...
		if (columnDataSpecDigest.getEnumSourceType() == EnumSourceType.EXTERNAL && columnDataSpecDigest.isEnumSelectorColumnSet()) {
			...
			String selectorValue = (cellValue == null ? null : (cellValue instanceof EnumValue
					? ((EnumValue) cellValue).getDeployValue()
					: cellValue.toString()));
	...
	public boolean shouldSelectCell(EventObject anEvent) {
		if ((comboModel.getSize() == 0 || (comboModel.getSize() == 1 && columnDataSpecDigest.isBlankAllowed()))
				&& columnDataSpecDigest.isEnumSelectorColumnSet()) {
	...
	private class ActionL implements ActionListener {
		...
		public synchronized void actionPerformed(ActionEvent e) {
			if (!UtilBase.isSame(tableModel.getCellValueAt(currentRow, columnName), getCellEditorValue())) {
				if (!UtilBase.isEmptyCellValue(tableModel.getCellValueAt(currentRow, columnName))
						&& tableModel.hasNonEmptyDepedentCell(currentRow, columnName)) {
