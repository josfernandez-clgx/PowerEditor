...
class DateRangeCellEditor extends AbstractCellEditor {
		...
		if (eventobject instanceof MouseEvent) {
			...
			if (mouseevent.getClickCount() > 1) {
				if (JOptionPane.showConfirmDialog(
					...
					else if (CellValidator.validateValue(min, max, columnDataSpecDigest)
	...
	private void initEditor() {
		...
		if (min != null) {
			lowerLimitLabel.setText("(> " + UIConfiguration.FORMAT_DATE.format(min) + ")");
		...
		if (max != null) {
			upperLimitLabel.setText("(< " + UIConfiguration.FORMAT_DATE.format(max) + ")");
