...
public class GridValidator implements IClientConstants {
				...
				if (i != j) {
					...
					if (flag2 || flag3) {
						...
						if (flag) {
							...
							k = JOptionPane.showConfirmDialog(ClientUtil.getApplet(), s, ClientUtil.getInstance().getMessage(
									"ValidationErrorMsgTitle"), 0);
						}
						if (k == 0)
							flag1 = false;
						else
	...
	private static String getActivationString(AbstractGrid<?> abstractgrid) {
		...
		if (abstractgrid.getSunrise() != null)
			s += ClientUtil.getInstance().getLabel("label.from") + " " + UtilBase.format(abstractgrid.getSunrise());
		String s1 = "";
		if (abstractgrid.getSunset() != null)
			s1 = ClientUtil.getInstance().getLabel("label.until") + UtilBase.format(abstractgrid.getSunset());
		return s + " " + s1;
	}
			...
			if (abstractgrid.getStatus().equalsIgnoreCase(Constants.DRAFT_STATUS) == false) {
				if (abstractgrid.getSunset() != null) {
					if (abstractgrid.getSunset().before(new Date())) {
						String msg = "Activation : " + abstractgrid.getSunrise() + " Expiration : " + abstractgrid.getSunset()
								+ " already expired with status: " + abstractgrid.getStatus();
						msg += ".\nPlease confirm to continue saving?";
						int j = JOptionPane.showConfirmDialog(ClientUtil.getApplet(), msg, ClientUtil.getInstance().getMessage(
								"ErrorMsgTitle"), 0);
						if (j != 0) flag1 = false;
					}
				}
			...
			for (int i = 0; i < gridtemplate.getNumColumns(); i++) {
				...
				for (int j = 0; j < abstractgrid.getNumRows(); j++) {
					...
					if (!(columnDataSpecDigest.getType().equals(ColumnDataSpecDigest.TYPE_BOOLEAN))
							&& !columnDataSpecDigest.isBlankAllowed() && (value1 == null || value1.equals("Not Found"))) {
						String msg = "Blank values in some fields are not allowed.\nPlease fill them in, or data will not be saved.";
						JOptionPane.showMessageDialog(ClientUtil.getApplet(), msg, ClientUtil.getInstance().getMessage(
								"ValidationErrorMsgTitle"), 0);
						return false;
					}
				}
