...
class DynamicStringCellEditor extends AbstractCellEditor implements TableCellRenderer, TableCellEditor {
		...
		public void performAction(ActionEvent e) {
			if (textArea.getText() != null || textArea.getText().length() > 0) {
				...
				try {
					...
					Validator.validateDynamicString(
							attributeItemList,
							textArea.getText(),
							templateColumnNames.length,
							consumer,
							infoConsumer);
	...
	private final class FindAttrL extends AbstractThreadedActionAdapter {
		...
		public void performAction(ActionEvent e) {
			...
			try {
				String value = (String) JOptionPane.showInputDialog(ClientUtil.getApplet(), null, ClientUtil.getInstance().getLabel(
						"d.title.select.item"), JOptionPane.PLAIN_MESSAGE, null, getItems(), null);
	...
	DynamicStringCellEditor(String[] templateColumnNames, String columnName, ColumnDataSpecDigest columnDataSpecDigest, boolean viewOnly) {
		...
		validateButton = UIFactory.createButton(
				ClientUtil.getInstance().getLabel("button.validate"),
				"image.btn.validate.message",
				new ValidateL(),
				null);
		...
		if (columnDataSpecDigest.getAllAttributeItems() == null || columnDataSpecDigest.getAllAttributeItems().isEmpty()) {
			...
			findAttributeButton = UIFactory.createButton(
					ClientUtil.getInstance().getLabel("button.insert.attribute"),
					"image.btn.find.attribute",
					new FindAttrL(),
					null);
			initAttributeItemList(columnDataSpecDigest);
		}
		findColumnButton = UIFactory.createButton(
				ClientUtil.getInstance().getLabel("button.insert.column"),
				"image.btn.find.column",
				new FindColL(),
				null);
	...
	private void init() {
		button.addActionListener(new EditL());
