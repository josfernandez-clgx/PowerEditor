...
public abstract class AbstractGridPanel extends JPanel implements PowerEditorTabPanel, CellValueChangeListener {
		...
		if (context != null) {
			for (int i = 0; i < context.length; i++) {
				if (context[i].getGenericEntityType() != null) {
					...
					productGrid.setGenericCategoryIDs(ClientUtil.getEntityConfiguration().findEntityTypeForCategoryType(
	...
	private boolean isSame(ProductGrid serverGrid, ProductGrid selectedGrid) {
		if (UtilBase.isSame(serverGrid.getEffectiveDate(), selectedGrid.getEffectiveDate())
				&& UtilBase.isSame(serverGrid.getExpirationDate(), selectedGrid.getExpirationDate())
				&& UtilBase.isSame(serverGrid.getCreationDate(), selectedGrid.getCreationDate())
				&& serverGrid.getStatus().equals(selectedGrid.getStatus()) && serverGrid.getNumRows() == selectedGrid.getNumRows()) {
			return true;
		}
		else {
			return false;
	...
	boolean adjustActivationDates(ProductGrid source) {
		...
		if (gridList != null && !gridList.isEmpty()) {
			for (int i = 0; i < gridList.size(); i++) {
				...
				if (ClientUtil.hasProductionRestrictions(gridToCheck.getStatus())
						&& (effDate == null || effDate.getDate().before(new Date()))) {
					break;
				}
	...
	private void initActivationsPanel() {
		addActivationBtn = new JButton(ClientUtil.getInstance().getLabel("button.add"), ClientUtil.getInstance().makeImageIcon(
				"image.btn.small.new"));
		// This is a copy button as of PowerEditor 4.2.0.
		cloneActivationBtn = new JButton(ClientUtil.getInstance().getLabel("button.copy"), ClientUtil.getInstance().makeImageIcon(
				"image.btn.small.copy"));
		removeActivationBtn = new JButton(ClientUtil.getInstance().getLabel("button.remove"), ClientUtil.getInstance().makeImageIcon(
				"image.btn.small.delete"));
		editActivationBtn = new JButton(ClientUtil.getInstance().getLabel("button.edit.date"), ClientUtil.getInstance().makeImageIcon(
	...
	private String getActivationString(ProductGrid abstractgrid) {
		...
		synchronized (dateNameCheckbox) {
			if (abstractgrid.getSunrise() != null) {
				buff.append((dateNameCheckbox.isSelected()
						? abstractgrid.getEffectiveDate().getName()
						: UIConfiguration.FORMAT_DATE_TIME_SEC.format(abstractgrid.getEffectiveDate().getDate())));
			}
			buff.append(" - ");
			if (abstractgrid.getSunset() != null) {
				buff.append((dateNameCheckbox.isSelected()
						? abstractgrid.getExpirationDate().getName()
						: UIConfiguration.FORMAT_DATE_TIME_SEC.format(abstractgrid.getExpirationDate().getDate())));
			}
		}
	...
	public void populate(GuidelineContext[] contexts, List<ProductGrid> list, GridTemplate gridTemplate, boolean isSubcontext,
			AbstractGuidelineGrid gridToSelect) {
	...
	private boolean isGridActive() {
		return currentGrid != null
				&& (currentGrid.getEffectiveDate() == null || currentGrid.getEffectiveDate().getDate().before(new Date()));
	}

	private boolean isGridExpired() {
		return currentGrid != null && currentGrid.getExpirationDate() != null
				&& currentGrid.getExpirationDate().getDate().before(new Date());
	}
	...
	private void initButtonPanel() {
		saveGridButton = UIFactory.createButton(
		...
		JButton validateButton = UIFactory.createButton(
				ClientUtil.getInstance().getLabel("button.validate"),
				"image.btn.small.validate",
				null,
				"button.tooltip.validate");
		...
		toggleButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				toggleButton.setText((topPanel.isVisible()
						? ClientUtil.getInstance().getLabel("button.hide.context")
						: ClientUtil.getInstance().getLabel("button.show.context")));
			}
		});
		...
		ruleIDToggleButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				toggleRuleIDColumns();
				ruleIDToggleButton.setText(ruleIDToggleButton.getText().equals(showLabel) ? ClientUtil.getInstance().getLabel(
						"button.hide.rule.id") : showLabel);
			}
		});
		jpanel.add(new JLabel("  "));
