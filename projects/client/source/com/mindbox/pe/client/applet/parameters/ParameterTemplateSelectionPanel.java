...
public class ParameterTemplateSelectionPanel extends PanelBase {
		...
		public void valueChanged(ListSelectionEvent arg0) {
			...
			try {
				if (templateList.getSelectedIndex() != -1) {
					setSelection(
						(ParameterTemplate) templateList.getModel().getElementAt(templateList.getSelectedIndex()));
				}
				else {
					clearSelection();
				}
	...
	private void initPanel() {
		...
		setBorder(
			UIFactory.createTitledBorder(
				ClientUtil.getInstance().getLabel("label.title.parameter.template.selection")));
