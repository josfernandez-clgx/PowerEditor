...
public class GenericEntityToCategoryEditDialog extends JPanel {
    ...
    private GenericEntityToCategoryEditDialog(GenericEntity entity, MutableTimedAssociationKey key) {
        ...
        EntityTypeDefinition entityTypeDef = ClientUtil
                .getEntityConfiguration().findEntityTypeDefinition(
                        entity.getType());
        entityCanBelongToMultipleCategories = entityTypeDef
                .canBelongToMultipleCategories();

        GenericCategory category = EntityModelCacheFactory.getInstance()
                .getGenericCategory(entity.getType().getCategoryType(),
                        key.getAssociableID());
    ...
    public static MutableTimedAssociationKey editParentCategory(GenericEntity entity, MutableTimedAssociationKey key) {
        ...
        int option = JOptionPane.showConfirmDialog(ClientUtil.getApplet(),
                dialog, ClientUtil.getInstance().getLabel(
                        "d.title.edit.categorytoentity"),
                JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);

        while ((option != JOptionPane.CANCEL_OPTION)
                && (!GenericEntityToCategoryEditDialog.isValid(dialog.effDateEntryField.getValue(),
                        dialog.expDateEntryField.getValue()) || (!dialog.entityCanBelongToMultipleCategories && dialog
                        .hasOverlappingParentCategory()))) {
            option = JOptionPane.showConfirmDialog(ClientUtil.getApplet(),
                    dialog, ClientUtil.getInstance().getLabel(
                            "d.title.edit.categorytocategory"),
                    JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
        }

        if (option == JOptionPane.CANCEL_OPTION) {
            return null;
        } else if (!UtilBase.isSame(key.getEffectiveDate(),
                dialog.effDateEntryField.getValue())
                || !UtilBase.isSame(key.getExpirationDate(),
                        dialog.expDateEntryField.getValue())) {
            MutableTimedAssociationKey newKey = new DefaultMutableTimedAssociationKey(
                    dialog.associableID, dialog.effDateEntryField.getValue(),
                    dialog.expDateEntryField.getValue());
    ...
    private boolean hasOverlappingParentCategory() {
		...
		for (Iterator<MutableTimedAssociationKey> i = entity.getCategoryIterator(); i.hasNext();) {
			...
			if ((key.getAssociableID() != associableID) && newkey.overlapsWith(key)) {
				GenericCategory parentCat = EntityModelCacheFactory.getInstance().getGenericCategory(
						entity.getType(),
						key.getAssociableID());
				ClientUtil.getInstance().showErrorDialog(
						"msg.error.category.overlaps",
						new Object[] { entity.getType().getName(), entity.getName(), parentCat.getName() });
				overlaps = true;
