...
public class ParameterDetailPanel extends PanelBase implements EntityDeleteListener, PowerEditorTabPanel {
	...
	private final class CloneL extends AbstractThreadedActionAdapter {
		...
		public void performAction(ActionEvent e) {
			...
			if (grid != null) {
				try {
					if (((Boolean) values[1]).booleanValue()
							&& (grid.getEffectiveDate() == null || grid.getEffectiveDate().after(originalGrid.getEffectiveDate()))) {
	...
	private final class DeleteL extends AbstractThreadedActionAdapter {
		...
		public void performAction(ActionEvent e) {
			try {
				...
				ClientUtil.getCommunicator().delete(grid.getID(), EntityType.PARAMETER_GRID);
	...
	protected ParameterDetailPanel(ParameterManagerTab pmTab, boolean readOnly) {
		...
		deleteButton = UIFactory.createButton(
				ClientUtil.getInstance().getLabel("button.delete"),
		...
		editParamContextButton = UIFactory.createButton(
				ClientUtil.getInstance().getLabel("button.edit.param.context"),
				"image.btn.small.edit",
				new EditParamContextL(),
				null);
	...
	private boolean saveChanges_internal() throws ServerException {
		...
		if (hasProductionRestrictions() && !UtilBase.isSame(actField.getValue(), parameterGrid.getEffectiveDate())
				&& (actField.getValue() == null || actField.getValue().getDate().before(new Date()))) {
			ClientUtil.getInstance().showErrorDialog(
					"msg.error.statuschange.activate",
					new Object[] { ClientUtil.getStatusDisplayLabel(statusField.getSelectedEnumValueValue()) });
			return false;
		}

		String errMessageKey = Validator.validateActivationDateRange(
				actField.getValue(),
				expField.getValue(),
				statusField.getSelectedEnumValueValue());
	...
	public void lockGrid(ParameterGrid grid) throws ServerException {
		ClientUtil.getCommunicator().lock(grid.getTemplateID(), EntityType.PARAMETER_TEMPLATE);
	}

	private void lockSelectedGrid() throws ServerException {
		ClientUtil.getCommunicator().lock(getSelectedGrid().getTemplateID(), EntityType.PARAMETER_TEMPLATE);
	}

	private void unlockSelectedGrid() throws ServerException {
		ClientUtil.getCommunicator().unlock(getSelectedGrid().getTemplateID(), EntityType.PARAMETER_TEMPLATE);
	}

	private void unlockGrid(ParameterGrid grid) throws ServerException {
		ClientUtil.getCommunicator().unlock(grid.getTemplateID(), EntityType.PARAMETER_TEMPLATE);
	}
	...
	private void unlockIfLocked() {
		if (this.template != null && saveButton.isEnabled()) {
			try {
				ClientUtil.getCommunicator().unlock(template.getID(), EntityType.PARAMETER_TEMPLATE);
			}
			catch (Exception ex) {
				ClientUtil.handleRuntimeException(ex);
			}
	...
	private class StatusComboL implements ActionListener {
		...
		public void actionPerformed(ActionEvent event) {
			...
			if (!ClientUtil.checkPermissionByPrivilegeName(PrivilegeConstants.PRIV_EDIT_PRODUCTION_DATA)
					&& ClientUtil.isHighestStatus(newStatus)) {
				if (isGridActive()) {
					ClientUtil.getInstance().showErrorDialog(
							"msg.error.statuschange.activate",
					...
					setSelectedStatus(getSelectedGrid().getStatus());
						...
						"msg.confirm.statuschange.parameter.hightest",
						new Object[] { ClientUtil.getHighestStatusDisplayLabel() })) {
					setSelectedStatus(getSelectedGrid().getStatus());
				}
			}
	...
	private boolean rowHasProductionRestrictions() {
		ParameterGrid grid = getSelectedGrid();
		return grid != null && !ClientUtil.checkPermissionByPrivilegeName(PrivilegeConstants.PRIV_EDIT_PRODUCTION_DATA)
	...
	private boolean hasProductionRestrictions() {
		return !ClientUtil.checkPermissionByPrivilegeName(PrivilegeConstants.PRIV_EDIT_PRODUCTION_DATA)
				&& ClientUtil.isHighestStatus(statusField.getSelectedEnumValueValue());
	}
