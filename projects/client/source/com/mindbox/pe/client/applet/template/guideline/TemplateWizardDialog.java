...
public class TemplateWizardDialog extends JDialog {
	...
	private String updateFromFields(boolean fromFinish) throws CanceledException {
		...
		if (currentStep == 1) {
			...
			try {
				if (!ClientUtil.getCommunicator().checkNameForUniqueness(EntityType.TEMPLATE, nameField.getText().trim())) {
			...
			for (int i = 0; i < attributeList.getModel().getSize(); i++) {
				...
				template.addGridTemplateColumn(TemplateUtil.generateColumnsFromAttribute(
