 ...
 *
 */
public class CBRAttributeDetailPanel extends AbstractDetailPanel<CBRAttribute, EntityManagementButtonPanel<CBRAttribute>> implements
	...
	public CBRAttributeDetailPanel(CBRCaseBase cb) {
		super(EntityType.CBR_ATTRIBUTE);
		caseBase = cb;
	}
	...
	protected void validateFields() throws InputValidationException {
		...
		if (UtilBase.trim(this.nameField.getText()).length() == 0)
			throw new InputValidationException("An Attribute name may not be blank.");
		CBRAttribute att = currentObject;
		try {
			List<?> attList = ClientUtil.getCommunicator().search(
					new CBRExactNameSearchFilter(EntityType.CBR_ATTRIBUTE, nameField.getText(), caseBase.getID()));
		...
		CBRAttributeType at = attributeTypeCombo.getSelectedCBRAttributeType();
		if (at != null && at.getAskForNumericRange().booleanValue() && this.lowestValueField.getValue() != null
				&& this.highestValueField.getValue() != null
					...
					while (it2.hasNext()) {
						...
						if (av.getAttribute().getID() == att.getID()) {
							if (!newVR.isConforming(av.getName(), valuePanel.getDataList()))
								throw new InputValidationException("Case \"" + c.getName() + " \" has the Attribute/Value pair \""
										+ av.getAttribute().getName() + "/" + av.getName() + "\" not conforming to the Value Range \""
										+ newVR.getName() + "\"");
						}
					}
				}
	...
	private CBRValueRange getDefaultValueRangeObject(CBRAttributeType at) {
		NameSearchFilter<CBRValueRange> nsf = new NameSearchFilter<CBRValueRange>(EntityType.CBR_VALUE_RANGE);
