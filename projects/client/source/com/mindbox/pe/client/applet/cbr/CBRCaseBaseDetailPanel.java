 ...
 */
public class CBRCaseBaseDetailPanel extends IDNameDescriptionObjectDetailPanel<CBRCaseBase, EntityManagementButtonPanel<CBRCaseBase>>
	...
	public CBRCaseBaseDetailPanel(CBRCaseBase cb) throws ServerException {
		super(EntityType.CBR_CASE_BASE);
	...
	protected void addComponents(GridBagLayout bag, GridBagConstraints c) {
		matchThresholdField = new NumberTextField(
				10,
				Constants.CBR_NULL_DATA_EQUIVALENT_VALUE,
				Constants.CBR_NULL_DATA_EQUIVALENT_VALUE,
				true);
		maximumMatchesField = new NumberTextField(
				10,
				Constants.CBR_NULL_DATA_EQUIVALENT_VALUE,
				Constants.CBR_NULL_DATA_EQUIVALENT_VALUE,
				false);
	...
	protected void populateDetails(CBRCaseBase object) {
		...
		if (caseBase.getMatchThreshold() != Constants.CBR_NULL_DATA_EQUIVALENT_VALUE)
			this.matchThresholdField.setValue(caseBase.getMatchThreshold());
		if (caseBase.getMaximumMatches() != Constants.CBR_NULL_DATA_EQUIVALENT_VALUE)
			this.maximumMatchesField.setValue(caseBase.getMaximumMatches());
	...
	protected void validateFields() throws InputValidationException {
		...
		try {
			List<CBRCaseBase> caseBaseList = ClientUtil.getCommunicator().search(new AllSearchFilter<CBRCaseBase>(EntityType.CBR_CASE_BASE));
