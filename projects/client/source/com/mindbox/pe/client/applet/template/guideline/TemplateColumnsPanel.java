...
class TemplateColumnsPanel extends PanelBase {
	...
	private final NumberTextField minField = new NumberTextField(10);
	private final NumberTextField maxField = new NumberTextField(10);
	private final FloatTextField minFloatField = new FloatTextField(10, false);
	private final FloatTextField maxFloatField = new FloatTextField(10, false);
	...
	private void initPanel() {
		...
		c.gridwidth = 1;
		c.weightx = 0.0;
		addComponent(floatRangeDetailPanel, bag, c, new JLabel("Min:"));

		c.weightx = 0.5;
		addComponent(floatRangeDetailPanel, bag, c, minFloatField);

		c.weightx = 0.0;
		addComponent(floatRangeDetailPanel, bag, c, new JLabel(" Max:"));

		c.weightx = 0.5;
		addComponent(floatRangeDetailPanel, bag, c, maxFloatField);

		c.weightx = 0.0;
		addComponent(floatRangeDetailPanel, bag, c, new JLabel(" Display Decimals:"));
	...
	private void refreshSelectorColumnComboItems(GridTemplateColumn currentColumn) {
		...
		if (currentColumn != null && enumSourceDetailCombo.getItemCount() > 0) {
			...
			for (int i = 1; i <= template.getColumnCount(); i++) {
				if (template.getColumn(i).getColumnDataSpecDigest().canBeSelector()
						&& !template.getColumn(i).getName().equals(currentColumn.getName())) {
	...
	private void setEnabledColumnSelectionAwareButtons(boolean enabled) {
		...
		downColumnButton.setEnabled(enabled && (columnTable.getSelectedRow() < (template.getNumColumns() - 1))
				&& (columnTable.getSelectedRow() >= 0));
	}
	...
	private void updateDataTypeDetails(int index) throws ValidationException {
		...
		case 9: // float range
			...
			currentColumn.getColumnDataSpecDigest().setPrecision(
					(precisionField.hasValue() ? precisionField.getValue().intValue() : FloatFormatter.NO_PRECISION));
			break;
		...
		case 7: // enumlist
			if (enumSourceTypeCombo.getSelectedIndex() < 0)
				throw new ValidationException("msg.warning.enum.column.select.type", new Object[0]);
			...
			if (sourceType == EnumSourceType.COLUMN) {
				...
				if (enumSourceDetailCombo.getSelectedIndex() < 0)
					throw new ValidationException("msg.warning.enum.column.select.source", new Object[0]);
	...
	private final class DataTypeComboL implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			if (selection != null) {
				...
				multiSelectCheckBox.setVisible(selection.equals(ColumnDataSpecDigest.TYPE_ENUM_LIST)
						|| selection.equals(ColumnDataSpecDigest.TYPE_ENTITY));
				sortEnumCheckBox.setVisible(selection.equals(ColumnDataSpecDigest.TYPE_ENUM_LIST)
						|| selection.equals(ColumnDataSpecDigest.TYPE_ENTITY));
				showLHSAttrCheckBox.setVisible(selection.equals(ColumnDataSpecDigest.TYPE_DYNAMIC_STRING));
				if (sortEnumCheckBox.isVisible()) {
					sortEnumCheckBox.setText(ClientUtil.getInstance().getLabel(
							selection.equals(ColumnDataSpecDigest.TYPE_ENUM_LIST) ? "checkbox.sort.enum" : "checkbox.sort.entity"));
				}
			}
			else {
				columnDTDetailCard.show(columnDTDetailPanel, "EMPTY");
	...
	private class ColumnSelectionL implements ListSelectionListener {
		...
		private void cancelEvent() {
			...
			try {
				columnTable.getSelectionModel().setSelectionInterval(
						currentColumn.getColumnNumber() - 1,
						currentColumn.getColumnNumber() - 1);
			}
			finally {
	...
	private class ColumnActionL extends Action3Adapter {
		public void deletePerformed(ActionEvent e) {
			if (columnTable.getSelectedRow() != -1) {
				...
				int option = JOptionPane.showConfirmDialog(ClientUtil.getApplet(), "Are you sure you want to delete Column "
						+ column.getID() + " '" + column.getTitle() + "'?", "Confirm Delete", JOptionPane.YES_NO_OPTION);
