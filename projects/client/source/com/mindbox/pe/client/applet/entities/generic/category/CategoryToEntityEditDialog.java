...
public class CategoryToEntityEditDialog extends JPanel {
    ...
    private CategoryToEntityEditDialog(GenericCategory category,
        CategoryToEntityAssociationData data) {
        super();
        ...
        EntityTypeDefinition entityTypeDef = ClientUtil.getEntityConfiguration().findEntityTypeDefinition(data.getEntity().getType());
    ...
    private CategoryToEntityEditDialog(GenericCategory category) {
        ...
        EntityTypeDefinition entityTypeDef = ClientUtil.getEntityConfiguration().findEntityTypeDefinition(
                ClientUtil.getEntityConfiguration().findEntityTypeForCategoryType(category.getType()));
        entityCanBelongToMultipleCategories = entityTypeDef.canBelongToMultipleCategories();
    }
     ...
     */
    public static CategoryToEntityAssociationData newAssociationData(
        ...
        if (option == JOptionPane.CANCEL_OPTION) {
            ...
            GenericEntity entity = EntityModelCacheFactory.getInstance()
                                                          .getGenericEntity(GenericEntityType.forCategoryType(
                        category.getType()),
                    dialog.entityCombo.getSelectedGenericEntityID());
            MutableTimedAssociationKey key = new DefaultMutableTimedAssociationKey(category.getId(),
                    dialog.effDateEntryField.getValue(),
                    dialog.expDateEntryField.getValue());
            CategoryToEntityAssociationData data = new CategoryToEntityAssociationData(entity,
                    key);
    ...
    private boolean hasOverlappingParentCategory() {
        ...
        GenericEntity entity = EntityModelCacheFactory.getInstance()
            .getGenericEntity(GenericEntityType.forCategoryType(category.getType()), entityCombo.getSelectedGenericEntityID());
        MutableTimedAssociationKey newkey = new DefaultMutableTimedAssociationKey(category.getId(),
        ...
        for (Iterator<MutableTimedAssociationKey> i = entity.getCategoryIterator(); i.hasNext();) {
            ...
            if (newkey.overlapsWith(key)) {
                GenericCategory parentCat = EntityModelCacheFactory.getInstance().getGenericCategory(entity.getType(), 
                        key.getAssociableID());
                ClientUtil.getInstance().showErrorDialog("msg.error.entitycategory.overlaps",
                        new Object[] { entity.getType().getName(), entityCombo.getSelectedGenericEntity().getName(),
                        parentCat.getName() });                
                overlaps = true;
                break;
            }
     ...
     * otherwise
     */
    public static CategoryToEntityAssociationData editAssociationData(
        GenericCategory category, CategoryToEntityAssociationData data) {
        CategoryToEntityEditDialog dialog = new CategoryToEntityEditDialog(category,
                data);
        int option = JOptionPane.showConfirmDialog(ClientUtil.getApplet(),
                dialog,
    ...
    private void initDialog() {
        entityCombo = new GenericEntityComboBox(GenericEntityType.forCategoryType(
                    category.getType()), false, null, true);
    ...
    private class FindL extends AbstractThreadedActionAdapter {
        public void performAction(ActionEvent event) throws Exception {
            GenericEntity[] entities = GenericEntitySearchDialog.findGenericEntity(false,
