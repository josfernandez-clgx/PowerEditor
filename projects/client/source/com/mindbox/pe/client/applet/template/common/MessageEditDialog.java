...
public class MessageEditDialog extends JPanel {
    private static final MessageFormat validateMessageFormat = new MessageFormat(
            "* Validation Failed *" + System.getProperty("line.separator") +
            System.getProperty("line.separator") + "Error Type   : {0}" +
            System.getProperty("line.separator") + "Error Message: {1}" +
    ...
    private MessageEditDialog(JDialog dialog, boolean forTemplate,
        GridTemplate template, TemplateMessageDigest digest,
        ...
        this.entityCombo = ((ClientUtil.getEntityTypeForMessageContext() == null)
            ? null
            : new GenericEntityComboBox(ClientUtil.getEntityTypeForMessageContext(),
                true, null));
        ...
        validateMessageButton = UIFactory.createButton("",
                "image.btn.small.validate", new ValidateMessageL(), null);
        validateMessageButton.setToolTipText("Validate Message");

        browseMessageElemButton = UIFactory.createButton("",
                "image.btn.find.attribute",
                new BrowseAttributeL(messageText, true), null);
        browseMessageElemButton.setToolTipText(
            "Paste Attribute Reference for Message...");
        browseMessageColButton = UIFactory.createButton("",
                "image.btn.find.column", new BrowseColumnL(messageText), null);
        browseMessageColButton.setToolTipText(
            "Paste Column Reference for Rule...");
        insertColMessagesButton = UIFactory.createButton(ClientUtil.getInstance()
                                                                   .getLabel("button.insert.column.messages"),
                null, new InsertColMessagesL(messageText), null);
    ...
    public static TemplateMessageDigest editTemplateMessageDigest(Frame owner,
        boolean forTemplate, GridTemplate template,
        TemplateMessageDigest digest, boolean displayConditionalDelims) {
        JDialog dialog = new JDialog(owner, true);
        dialog.setTitle("Edit Message");

        MessageEditDialog panel = new MessageEditDialog(dialog, forTemplate,
                template, digest, displayConditionalDelims);
    ...
    public static TemplateMessageDigest createTemplateMessageDigest(
        Frame owner, boolean forTemplate, GridTemplate template,
        boolean displayConditionalDelims) {
        JDialog dialog = new JDialog(owner, true);
        dialog.setTitle("New Message");

        MessageEditDialog panel = new MessageEditDialog(dialog, forTemplate,
                template, null, displayConditionalDelims);
        UIFactory.addToDialog(dialog, panel);
    ...
    private void initPanel() {
        JPanel messageMsgButtonPanel = new JPanel(new FlowLayout(
                    FlowLayout.LEFT));
        ...
        if (entityCombo != null) {
            c.gridwidth = 1;
            c.weightx = 0.0;
            PanelBase.addComponent(this, bag, c,
                UIFactory.createFormLabel(entityCombo.getGenericEntityType()));
        ...
        c.weightx = 0.0;
        PanelBase.addComponent(this, bag, c,
            UIFactory.createFormLabel("label.delim.cond"));
        ...
        c.weightx = 0.0;
        PanelBase.addComponent(this, bag, c,
            UIFactory.createFormLabel("label.delim.cond.final"));
        ...
        c.weightx = 0.0;
        PanelBase.addComponent(this, bag, c,
            UIFactory.createFormLabel("label.message.text"));
        ...
        JPanel buttonPanel = UIFactory.createJPanel(new FlowLayout(
                    FlowLayout.CENTER));
    ...
    private void populateFields() {
        if (digest != null) {
            ...
            MessageConfiguration msgConfig = ((MainPanel) ClientUtil.getParent()).getUserSession()
                                              .getDefaultCondMsg(template.getUsageType());
    ...
    private boolean updateMessage() {
        if ((messageText.getText() == null) ||
            ...
            ClientUtil.getInstance().showWarning("msg.warning.empty.field",
                new Object[] { "message text" });
    ...
    private class BrowseAttributeL implements ActionListener {
        ...
        void pasteValue(String val) {
            if ((val != null) && (val.length() > 0)) {
                ...
                if (forMessage) {
                    textArea.setText(prevText.substring(0, pos) + "|" + val +
                        "|" + prevText.substring(pos));
    ...
    private class BrowseColumnL extends BrowseAttributeL {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            for (int i = 1; i <= columnValues.length; i++) {
                columnValues[i - 1] = i + " " +
                    template.getColumn(i).getTitle();
            }

            String value = (String) JOptionPane.showInputDialog(ClientUtil.getApplet(),
                    null, "Select Column", JOptionPane.PLAIN_MESSAGE, null,
                    columnValues, null);

            if (value != null) {
                int colIndex = Integer.parseInt(value.substring(0,
                            value.indexOf(" ")));
    ...
    private class InsertColMessagesL extends BrowseAttributeL {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            int option = JOptionPane.showConfirmDialog(ClientUtil.getApplet(),
                    selectionPanel, "Select Column",
            ...
            if (option == JOptionPane.OK_OPTION) {
                ...
                if ((objects != null) && (objects.length > 0)) {
                    ...
                    for (int idx = 0; idx < objects.length; idx++) {
                        ...
                        int colIndex = Integer.parseInt(str.substring(0,
                                    str.indexOf(" ")));
                        buf.append(colIndex);
    ...
    private class ValidateRuleL implements ActionListener, WarningConsumer {
		...
		String toErrorMessage(Throwable ex) {
            String errorName = ex.getClass().getName().substring(ex.getClass()
                                                                   .getName()
                                                                   .lastIndexOf(".") +
                    1);

            return validateMessageFormat.format(new Object[] {
