...
public class CategoryToCategoryEditDialog extends JPanel {
	...
	private CategoryToCategoryEditDialog(GenericCategory childCategory, MutableTimedAssociationKey key, boolean addMode) {
		...
		if (key != null) {
			GenericCategory parentCategory = EntityModelCacheFactory.getInstance().getGenericCategory(
	 ...
	 */
	public static MutableTimedAssociationKey newParentCategory(GenericCategory childCategory, MutableBoolean autoExpire,
			MutableTimedAssociationKey data) {
		...
		int option = JOptionPane.showConfirmDialog(ClientUtil.getApplet(), dialog, ClientUtil.getInstance().getLabel(
				"d.title.new.categorytocategory"), JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);

		while ((option != JOptionPane.CANCEL_OPTION)
				&& !dialog.isValid(dialog.effDateEntryField.getValue(), dialog.expDateEntryField.getValue(), null)) {
			option = JOptionPane.showConfirmDialog(ClientUtil.getApplet(), dialog, ClientUtil.getInstance().getLabel(
					"d.title.new.categorytocategory"), JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
		}
	...
	public static MutableTimedAssociationKey editParentCategory(GenericCategory childCategory, MutableTimedAssociationKey key) {
		...
		int option = JOptionPane.showConfirmDialog(ClientUtil.getApplet(), dialog, ClientUtil.getInstance().getLabel(
				"d.title.edit.categorytocategory"), JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);

		while ((option != JOptionPane.CANCEL_OPTION)
				&& !dialog.isValid(dialog.effDateEntryField.getValue(), dialog.expDateEntryField.getValue(), key)) {
			option = JOptionPane.showConfirmDialog(ClientUtil.getApplet(), dialog, ClientUtil.getInstance().getLabel(
					"d.title.edit.categorytocategory"), JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
		}
		...
		else if ((key.getAssociableID() != dialog.parentCatField.getGenericCategoryID())
				|| !UtilBase.isSame(key.getEffectiveDate(), dialog.effDateEntryField.getValue())
