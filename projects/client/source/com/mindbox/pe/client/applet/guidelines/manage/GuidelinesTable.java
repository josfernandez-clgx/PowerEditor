...
final class GuidelinesTable extends AbstractSortableTable<GuidelinesTableModel,GridSummary> {
	...
	private class BooleanIconRenderer extends DefaultTableCellRenderer {
		...
		public BooleanIconRenderer(String s, String s1) {
			...
			if (s != null)
				mOnIcon = ClientUtil.getInstance().makeImageIcon(s);
			if (s1 != null)
				mOffIcon = ClientUtil.getInstance().makeImageIcon(s1);
		}
	}
	...
	public void initColumns(String[] columnNames) {
		...
		for (int i = 0; i < columnNames.length; i++) {
			...
			if (ClientUtil.getUserSession().showGuidelineTemplateID()) {
				switch (i) {
					case 0 : // '\0'
					...
					case 3 : // '\002'
						BooleanIconRenderer booleaniconrenderer =
							new BooleanIconRenderer("image.box.filled", "image.box.empty");
						tablecolumn.setCellRenderer(booleaniconrenderer);
						break;

					case 4 : // '\003'
						BooleanIconRenderer booleaniconrenderer1 =
							new BooleanIconRenderer("image.box.filled", "image.box.empty");
						tablecolumn.setCellRenderer(booleaniconrenderer1);
						break;

					case 5 : // '\004'
						BooleanIconRenderer booleaniconrenderer2 =
							new BooleanIconRenderer("image.box.filled", "image.box.empty");
						tablecolumn.setCellRenderer(booleaniconrenderer2);
						break;
					case 6 : // '\004'
						BooleanIconRenderer booleaniconrenderer3 =
							new BooleanIconRenderer("image.box.filled", "image.box.empty");
