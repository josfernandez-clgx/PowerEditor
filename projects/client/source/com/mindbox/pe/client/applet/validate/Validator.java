...
public class Validator {
	 ...
	 * @param columnCount
	 * @param warningConsumer
	 * @return <code>true</code> if <code>text</code> is valid;
	 <code>false</code>, otherwise
	 * @since PowerEditor 3.2.0
	 */
	public static boolean validateDynamicString(List<String> attributeItemList, String text, int columnCount,
			WarningConsumer warningConsumer, TextWarningConsumer infoConsumer) {
		...
		if (result) {
			...
			while (matcher.find(index)) {
				...
				if (valueList.contains(refName)) {
					String msg = ClientUtil.getInstance().getMessage("msg.info.valid.reference.attribute.item", new String[] { refName });
	...
	public static <G extends AbstractGrid<?>> String validateGapsAndOverlapsInDates(boolean allowGaps, G abstractgrid, List<G> gridList,
			DateSynonym dsEff, DateSynonym dsExp, boolean editFlag, boolean autoAdjustFlag) {
	...
	public static <G extends AbstractGrid<?>> String validateGapsAndOverlapsInDatesInList(boolean allowGaps, boolean checkNewOrChangedOnly,
			List<G> gridList, String gapMsgKey, String overlapMsgKey) {
		boolean allowOverlaps = false;
		boolean ignoreEnclosedRanges = false;
		List<DateSynonymRange> listToCheck = new LinkedList<DateSynonymRange>();
		for (G grid : gridList) {
	...
	private static String hasOverlapOrGap(boolean allowGaps, boolean allowOverlaps, boolean ignoreEnclosedRanges,
			boolean checkNewOrChangedOnly, List<DateSynonymRange> dsRangeList, String gapMsgKey, String overlapMsgKey) {
	 ...
	 * @return
	 */
	public static <G extends AbstractGrid<?>> String validateDuplicateDates(G abstractgrid, List<G> gridList, DateSynonym dsEff,
			DateSynonym dsExp, boolean cloneFlag) {
		String result = null;
		if (gridList != null && gridList.size() > 0) {
			for (G grid : gridList) {
				if (cloneFlag || !grid.equals(abstractgrid)) {
					boolean sameEffDate = (grid.getEffectiveDate() == null && dsEff == null)
							|| (grid.getEffectiveDate() != null && grid.getEffectiveDate().isSameDate(dsEff));
					if (sameEffDate) {
