...
public class GenericEntitySearchDialog extends JPanel {
    ...
    private GenericEntitySearchDialog(JDialog dialog, boolean forMultiSelect,
        GenericEntityType genericEntityType) {
        super();
        this.dialog = dialog;
        this.forMultiSelect = forMultiSelect;
        this.genericEntityType = genericEntityType;
        nameField = new JTextField(10);
        searchButton = UIFactory.createButton(ClientUtil.getInstance().getLabel("button.search"),
                "image.btn.small.new", new SearchL(), null);
     ...
     */
    public static GenericEntity[] findGenericEntity(boolean forMultiSelect,
            GenericEntityType genericEntityType) {
        JDialog dialog = new JDialog(JOptionPane.getFrameForComponent(
                ClientUtil.getApplet()), true);
        
        dialog.setTitle(ClientUtil.getInstance().getLabel("d.title.search.entity"));
        GenericEntitySearchDialog panel = new GenericEntitySearchDialog(dialog,
                forMultiSelect, genericEntityType);
    ...
    private void initDialog() {
        ...
        c.weightx = 0.25;
        UIFactory.addComponent(criteriaContentsPanel, bag, c,
            UIFactory.createFormLabel("label.name.contains"));
        ...
        c.gridwidth = 1;
        UIFactory.addComponent(criteriaContentsPanel, bag, c,
            UIFactory.createFormLabel("label.no.parent.asof"));
        c.weightx = 0.75;
        c.gridwidth = GridBagConstraints.REMAINDER;
        ...
        resultsPanel.setBorder(BorderFactory.createTitledBorder(
                ClientUtil.getInstance().getLabel("label.search.results")));
        ...
        criteriaPanel.setBorder(BorderFactory.createTitledBorder(
                ClientUtil.getInstance().getLabel("label.search.criteria")));

        criteriaPanel.add(criteriaContentsPanel, BorderLayout.NORTH);
        mainPanel.add(criteriaPanel);
        mainPanel.add(resultsPanel);

        final JPanel buttonsPanel = UIFactory.createJPanel(new FlowLayout(
                    FlowLayout.CENTER, 4, 4));

        final JButton okButton = UIFactory.createButton(ClientUtil.getInstance()
                                                                  .getLabel("button.ok"),
                null, new SelectL(), null);
        okButton.setEnabled(false);

        final JButton cancelButton = UIFactory.createButton(ClientUtil.getInstance()
                                                                      .getLabel("button.cancel"),
                null, new CancelL(), null);
        ...
        resultsList.setSelectionMode(forMultiSelect
            ? ListSelectionModel.MULTIPLE_INTERVAL_SELECTION
            : ListSelectionModel.SINGLE_SELECTION);

        resultsList.setCellRenderer(new IDNameObjectCellRenderer(
                "image.node.entity"));
    ...
    private GenericEntity[] getSelectedGenericEntities() {
        if (forMultiSelect) {
            ...
            return (resultsList.getSelectedValue() == null) ? null
                                                            : new GenericEntity[] {
                (GenericEntity) resultsList.getSelectedValue()
            };
        }
    }
    ...
    private class SearchL extends AbstractThreadedActionAdapter {
        public void performAction(ActionEvent event) {
            ...
            try {
                ...
                ClientUtil.getInstance().showErrorDialog("msg.warning.invalid.date",
                    new Object[] { UIConfiguration.FORMAT_STR_DATE_TIME_SEC.toString() });

                return;
            }

            GenericEntityFilterSpec searchSpec = new GenericEntityFilterSpec(genericEntityType,
                    null);
            ...
            try {
                ...
                ClientUtil.getInstance().showErrorDialog("msg.error.generic.service",
