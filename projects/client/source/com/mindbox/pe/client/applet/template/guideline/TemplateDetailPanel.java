...
public class TemplateDetailPanel extends PanelBase implements GuidelineTemplateSelectionListener, PowerEditorTabPanel {
	...
	private final class NewL extends AbstractThreadedActionAdapter {
		...
		public void performAction(ActionEvent e) {
			try {
				GridTemplate template = TemplateWizardDialog.createTemplate((lastSelectedUsage == null
						? templateSelectionPanel.getLastSelectedUsageType()
						: lastSelectedUsage));
	...
	private final class MakeVersionL extends AbstractThreadedActionAdapter {
		...
		public void performAction(ActionEvent e) {
			...
			DateSynonym cutoverDS = TemplateNewVersionDialog.newTemplateVersion(template.getID(), clonedTemplate);
			if (cutoverDS == null) {
				clonedTemplate = null;
			}
			else {
				try {
					int newID = ClientUtil.getCommunicator().makeNewVersion(template.getID(), clonedTemplate, cutoverDS);
	...
	private final class CopyL extends AbstractThreadedActionAdapter {
		...
		public void performAction(ActionEvent e) {
			if (template == null) return;
			String newName = JOptionPane.showInputDialog(ClientUtil.getApplet(), ClientUtil.getInstance().getMessage(
					"msg.inst.new.name.template")
					+ ":", ClientUtil.getInstance().getLabel("d.title.copy.template"), JOptionPane.PLAIN_MESSAGE);
			if (!ClientUtil.isEmpty(newName)) {
				try {
					if (ClientUtil.getCommunicator().checkNameForUniqueness(EntityType.TEMPLATE, newName)) {
						GridTemplate clonedTemplate = new GridTemplate();
						clonedTemplate.copyFrom(template);
						clonedTemplate.setID(-1);
	...
	private final class EditL extends AbstractThreadedActionAdapter {
		...
		public void performAction(ActionEvent e) {
			if (template != null) {
				try {
					ClientUtil.getCommunicator().lock(template.getID(), EntityType.TEMPLATE);
			...
			if (template != null) {
				try {
					...
					if (ClientUtil.getInstance().showConfirmation(
							(templateHasGuidelines
									? "msg.question.delete.template.with.guideline"
									: "msg.question.delete.template.guideline"),
	...
	public class FieldChangeListener implements ActionListener, DocumentListener, MChangeListener, ListSelectionListener,
			RuleChangeListener, TableModelListener, ChangeListener, ListDataListener {
	...
	public TemplateDetailPanel(GuidelineTemplateSelectionPanel templateSelectionPanel) throws ServerException {
		...
		versionButton = UIFactory.createButton(
				ClientUtil.getInstance().getLabel("button.new.version"),
				null,
				new MakeVersionL(),
				"button.tooltip.new.template.version");
		newButton = UIFactory.createButton(
				ClientUtil.getInstance().getLabel("button.new.template"),
				null,
				new NewL(),
				"button.tooltip.new.template");
	...
	private void unlockIfLocked() {
		if (this.template != null && this.template.getID() > 0 && !editButton.isEnabled()) {
			try {
				ClientUtil.getCommunicator().unlock(template.getID(), EntityType.TEMPLATE);
			}
			catch (Exception ex) {
				ClientUtil.handleRuntimeException(ex);
			}
	...
	public synchronized void usageGroupSelected(GuidelineTabConfig guidelineTabConfig) throws CanceledException {
		if (guidelineTabConfig.getUsageTypes() != null && guidelineTabConfig.getUsageTypes().length > 0) {
			usageSelected(guidelineTabConfig.getUsageTypes()[0]);
		}
	}
	...
	public synchronized void selectionCleared() {
		try {
			if (template != null && hasChanges) {
				Boolean result = ClientUtil.getInstance().showConfirmationWithCancel(
						"msg.question.unsaved.changes.template",
	...
	private synchronized void saveTemplate_internal() throws InputValidationException, ValidationException, ServerException {
		...
		if (nameChanged) {
			if (!ClientUtil.getCommunicator().checkNameForUniqueness(EntityType.TEMPLATE, template.getName())) {
		...
		if (!oldVersion.equals(template.getVersion())) {
			List<GridTemplate> templateList = ClientUtil.getCommunicator().search(
					new TemplateByNameVersionFilter(template.getName(), template.getVersion()));
					...
					for (int columnIndex = 0; columnIndex < template.getNumColumns(); columnIndex++) {
						...
						for (int rowIndex = 0; rowIndex < grid.getNumRows(); rowIndex++) {
							...
							if (!blankAllowed && isBlank(value)) {
								addError(errorReportDataList, context, grid, rowIndex, columnIndex, ClientUtil.getInstance().getMessage(
										"msg.warning.invalid.blank"));
							}
								...
								if (value instanceof EnumValues) {
									...
									if (enumValues.isSelectionExclusion()) {
										addError(
												...
												rowIndex,
												columnIndex,
												ClientUtil.getInstance().getMessage("msg.warning.invalid.select.excl"));
									}
										...
										addError(
												...
												rowIndex,
												columnIndex,
												ClientUtil.getInstance().getMessage("msg.warning.invalid.multiple"));
									}
								}
	...
	static {
		...
		NEW_COLUMN_TYPE_TO_COMPATIBLE_OLD_COLUMN_TYPES_MAP.put(ColumnDataSpecDigest.TYPE_DYNAMIC_STRING, new HashSet<String>(
				Arrays.asList(allTypes)));
		NEW_COLUMN_TYPE_TO_COMPATIBLE_OLD_COLUMN_TYPES_MAP.put(ColumnDataSpecDigest.TYPE_STRING, new HashSet<String>(
				Arrays.asList(allTypes)));

		// Types for which there are specific old type to new type compatibilities
		NEW_COLUMN_TYPE_TO_COMPATIBLE_OLD_COLUMN_TYPES_MAP.put(ColumnDataSpecDigest.TYPE_CURRENCY, new HashSet<String>(
				Arrays.asList(new String[] { ColumnDataSpecDigest.TYPE_FLOAT, ColumnDataSpecDigest.TYPE_INTEGER })));
		NEW_COLUMN_TYPE_TO_COMPATIBLE_OLD_COLUMN_TYPES_MAP.put(ColumnDataSpecDigest.TYPE_CURRENCY_RANGE, new HashSet<String>(
				Arrays.asList(new String[] { ColumnDataSpecDigest.TYPE_FLOAT_RANGE, ColumnDataSpecDigest.TYPE_INTEGER_RANGE })));
		NEW_COLUMN_TYPE_TO_COMPATIBLE_OLD_COLUMN_TYPES_MAP.put(ColumnDataSpecDigest.TYPE_FLOAT, new HashSet<String>(
				Arrays.asList(new String[] { ColumnDataSpecDigest.TYPE_CURRENCY, ColumnDataSpecDigest.TYPE_INTEGER })));
		NEW_COLUMN_TYPE_TO_COMPATIBLE_OLD_COLUMN_TYPES_MAP.put(ColumnDataSpecDigest.TYPE_FLOAT_RANGE, new HashSet<String>(
				Arrays.asList(new String[] { ColumnDataSpecDigest.TYPE_CURRENCY_RANGE, ColumnDataSpecDigest.TYPE_INTEGER_RANGE })));
		NEW_COLUMN_TYPE_TO_COMPATIBLE_OLD_COLUMN_TYPES_MAP.put(ColumnDataSpecDigest.TYPE_SYMBOL, new HashSet<String>(
	...
	private void addError(LinkedList<GuidelineReportData> errorReportDataList, GuidelineContext[] context, AbstractGuidelineGrid grid,
			int rowIndex, int columnIndex, String errorDesc) {
	...
	private boolean isNewGuideline(AbstractGuidelineGrid currentGrid, GuidelineContext[] currentContext,
			LinkedList<GuidelineReportData> errorReportDataList) {
		if (errorReportDataList.isEmpty()) {
			return true;
		}

		GuidelineReportData lastReportData = errorReportDataList.getLast();
		return lastReportData.getTemplateID() != currentGrid.getTemplateID()
				|| !UtilBase.nullSafeEquals(lastReportData.getActivationDate(), currentGrid.getEffectiveDate())
	...
	private boolean showErrorReport(LinkedList<GuidelineReportData> errorReportDataList) throws Exception {
		return TemplateValidationErrorReportDialog.showErrorReport(
				generateValidationErrorReport(errorReportDataList),
				errorReportDataList.size());
	}
