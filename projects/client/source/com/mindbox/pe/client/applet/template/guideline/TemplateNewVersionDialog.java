...
public class TemplateNewVersionDialog extends JPanel {
	...
	public static DateSynonym newTemplateVersion(int sourceTemplateID, GridTemplate template) {
		TemplateNewVersionDialog instance = new TemplateNewVersionDialog(sourceTemplateID, template);
		JDialog dialog = UIFactory.createAsModelDialog("d.title.new.template.version", instance);
	...
	private TemplateNewVersionDialog(int sourceTemplateID, GridTemplate template) {
	...
	private void showCorrectPanel() {
		if (currentStep == 2) {
			...
			try {
				...
				cutoverDateLabel.setText(UIConfiguration.FORMAT_DATE_TIME_SEC.format(dsField.getDate()));
	...
	private void initPanel() {
		...
		JSplitPane sp = UIFactory.createSplitPane(JSplitPane.VERTICAL_SPLIT);
		JScrollPane scrollPane = new JScrollPane(new TemplateReportTable(cutOverTableModel));
		...
		if (currentStep == 1) {
			...
			if (dateSynonymFromPanel==null || dateSynonymFromPanel.getName() == null || dateSynonymFromPanel.getName().length() == 0) {
				ClientUtil.getInstance().showWarning(
						"msg.warning.empty.field",
						new Object[] { ClientUtil.getInstance().getLabel("label.date.cutover")});
				return false;
			}
			...
			if (cutOverTableModel.getRowCount() < 1) {
				if (!ClientUtil.getInstance().showConfirmation("msg.question.empty.cutover.guidelines")) { return false; }
			}
		}
		return true;
	}
