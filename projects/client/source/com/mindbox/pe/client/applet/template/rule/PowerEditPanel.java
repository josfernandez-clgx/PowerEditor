...
public class PowerEditPanel extends JPanel {
		...
		public void actionPerformed(ActionEvent arg0) {
			if (currentNode instanceof ConditionTreeNode) {
				if (ClientUtil.getInstance().showConfirmation("msg.question.delete.rule.condition")) {
					...
					&& ((LogicalOpTreeNode) currentNode).getCompoundLHSElementType() == CompoundLHSElement.TYPE_NOT) {
				...
				if (currentNode.getChildCount() > 0) {
					ClientUtil.getInstance().showWarning(
							"msg.warning.not.empty.rule.element",
							new Object[] { currentNode.getRuleElement().toDisplayName()});
				}
				else if (ClientUtil.getInstance().showConfirmation(
						"msg.question.delete.rule.element",
						new Object[] { currentNode.getRuleElement().toDisplayName()})) {
		...
		public void actionPerformed(ActionEvent arg0) {
			synchronized (PowerEditPanel.this) {
				if (currentNode instanceof LogicalOpAttachable) {
					addToCurrentNode(
							-1,
							new LogicalOpTreeNode(currentNode, RuleElementFactory.getInstance().createOrCompoundCondition()),
							true,
							false);
				}
				else {
	...
	public PowerEditPanel(boolean columnRefValueEditOnly, RuleDefinition rule) {
		...
		tree = (columnRefValueEditOnly ? UIFactory.createTree(treeModel, colRefTreeRenderer) : UIFactory.createTree(
				treeModel,
				new RuleTreeRenderer2()));
		...
		if (currentNode instanceof ConditionTreeNode) {
			if (columnRefValueEditOnly) {
				...
				if (origCondition.getValue() instanceof ColumnReference) {
					...
					Condition convertedCond = RuleElementFactory.deepCopyCondition(origCondition);
					convertedCond.setValue(RuleElementFactory.getInstance().createValue(
			...
			TestCondition test = FunctionEditDialog.editTestCondition(
					JOptionPane.getFrameForComponent(ClientUtil.getApplet()),
					((TestTreeNode) currentNode).getTestCondition());
			if (test != null) {
			...
			if (columnRefValueEditOnly) {
				...
				if (origParam != null && origParam instanceof ColumnReference && currentNode.getParent() instanceof FunctionTreeNode) {
					...
					if (typeDef != null) {
						...
						FunctionParameter newParam = ParameterEditDialog.editFunctionParameter(
								template,
								typeDef.getParameterDefinitionAt(origParam.index()),
								convertedParam,
								true);
						if (newParam != null) {
				...
				if (prevParam != null && currentNode.getParent() instanceof FunctionTreeNode) {
					...
					if (typeDef != null) {
						FunctionParameter newParam = ParameterEditDialog.editFunctionParameter(
								template,
								typeDef.getParameterDefinitionAt(prevParam.index()),
								prevParam);
	...
	private AbstractRuleTreeNode deleteNode_internal(AbstractRuleTreeNode node) {
		...
		if ((parent.getParent() instanceof AbstractRuleTreeNode) && isNonEmptyNotNode((AbstractRuleTreeNode) parent.getParent())) {
			...
			if (notNode.getChildCount() == 1 && parent instanceof LogicalOpTreeNode
					&& ((LogicalOpTreeNode) parent).getCompoundLHSElementType() == CompoundLHSElement.TYPE_AND
	...
	private void refreshButtons() {
		if (columnRefValueEditOnly) {
			...
			if (currentNode instanceof IfTreeNode) {
				...
				boolean parentNot = parentNode instanceof LogicalOpTreeNode
						&& ((LogicalOpTreeNode) parentNode).getCompoundLHSElementType() == CompoundLHSElement.TYPE_NOT;
		...
		if (currentNode instanceof LogicalOpAttachable) {
			...
			if (element == null) {
				...
				if (currentNode instanceof IfTreeNode) {
					...
					if (compoundElement == null) {
						ClientUtil.getInstance().showWarning(
								"msg.warning.failure.paste.generic",
								new Object[] { "the selected node cannot contain another rule element."});
						return;
					}
				...
				if (nodeToPaste != null) {
					if (element instanceof CompoundLHSElement && currentNode instanceof IfTreeNode
							&& ((CompoundLHSElement) element).getType() == CompoundLHSElement.TYPE_AND) {
			...
			if (param == null) {
				...
				if (action.getActionType() == null) {
					...
					ClientUtil.getInstance().showWarning(
							"msg.warning.failure.paste.generic",
							new Object[] { "selected action does not have such parameter."});
				}
				else {
			...
			if (param == null) {
				...
				if (test.getTestType().parameterSize() < param.index()) {
					ClientUtil.getInstance().showWarning(
							"msg.warning.failure.paste.generic",
							new Object[] { "selected test condition does not have such parameter."});
				}
				else {
