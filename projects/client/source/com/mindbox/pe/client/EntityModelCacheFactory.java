...
public class EntityModelCacheFactory implements DatedCategoryTreeModel.DataProvider {
	...
	private void buildGuidelineTemplateSearchTreeModel() {
		...
		GuidelineTabConfig[] tabConfigs = ClientUtil.getUserSession().getGuidelineConfigurations();
		...
		for (int i = 0; i < tabConfigs.length; i++) {
			if (ClientUtil.checkViewOrEditGuidelinePermission(tabConfigs[i])) {
				UsageGroupTreeNode groupNode = new UsageGroupTreeNode(rootNode, tabConfigs[i]);

				// add usage children
				TemplateUsageType[] usages = tabConfigs[i].getUsageTypes();
				for (int j = 0; j < usages.length; j++) {
					UsageTypeTreeNode usageNode = getUsageTypeSearchTreeNode(groupNode, usages[j]);
	...
	private void buildGuidelineTemplateTreeModel() {
		...
		RootTreeNode rootNode = new RootTreeNode(null);
		for (int i = 0; i < tabConfigs.length; i++) {
			if (ClientUtil.checkViewOrEditTemplatePermission(tabConfigs[i])) {
				UsageGroupTreeNode groupNode = new UsageGroupTreeNode(rootNode, tabConfigs[i]);

				// add usage children
				TemplateUsageType[] usages = tabConfigs[i].getUsageTypes();
				for (int j = 0; j < usages.length; j++) {
					UsageTypeTreeNode usageNode = getUsageTypeTreeNode(groupNode, usages[j]);
	...
	public void cacheTemplates() {
		...
		try {
			List<GridTemplate> list = ClientUtil.getCommunicator().search(new AllSearchFilter<GridTemplate>(EntityType.TEMPLATE));
			if (list != null) {
		...
		if (category.isRoot()) {
			...
			for (Iterator<GenericCategory> i = path.iterator(); i.hasNext();) {
				...
				fullyQualifiedName = fullyQualifiedName == null ? pathCat.getName() : fullyQualifiedName
						+ Constants.CATEGORY_PATH_DELIMITER + pathCat.getName();
			}
		}
	...
	public DatedCategoryTreeModel createGenericCategoryTreeModel(int categoryType, Date date, boolean showEntities, boolean sort) {
		...
		if (!categoryList.isEmpty()) {
			...
			throw new NullPointerException("Category cannot be null. "
					+ "At least one category must exist int order to create a category tree model.");
		}
	...
	public void reloadCache() throws ServerException {
		...
		for (int i = 0; i < types.length; i++) {
			CategoryTypeDefinition categoryDef = ClientUtil.getEntityConfiguration().getCategoryDefinition(types[i]);
			if (categoryDef != null) {
				List<GenericCategory> categoryList = cacheInstance.lookupCategory(categoryDef.getTypeID());
				reloadGenericCategories(new Integer(categoryDef.getTypeID()), categoryList);
			}
		}

		// parameter templates
		try {
			List<ParameterTemplate> list = ClientUtil.getCommunicator().search(
					new AllSearchFilter<ParameterTemplate>(EntityType.PARAMETER_TEMPLATE));
	...
	public List<GenericEntity> getGenericEntitiesInCategory(GenericEntityType entityType, int categoryID, Date date,
			boolean includeDescendents) throws ServerException {
	...
	private ComboBoxModel copyGenericEntityComboModel_aux(GenericEntityType type, boolean hasEmpty) {
		...
		Map<GenericEntityType, List<DefaultComboBoxModel>> copyMap = (hasEmpty
				? genericEntityWithEmptyComboModelCopyMap
				: genericEntityComboModelCopyMap);
	...
	private ComboBoxModel getGenericEntityModel_aux(GenericEntityType type, boolean hasEmpty) {
		ComboBoxModel model = (ComboBoxModel) (hasEmpty
				? genericEntityWithEmptyComboModelMap.get(type)
				: genericEntityComboModelMap.get(type));
		if (model == null) {
