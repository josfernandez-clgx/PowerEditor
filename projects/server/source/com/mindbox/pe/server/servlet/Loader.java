...
public final class Loader {
	...
	private static void loadDomain(String[] domainFiles) throws InvalidDataException, IOException, SAXException {
		...
		for (int i = 0; i < domainFiles.length; i++) {
			logger.info("Reading " + domainFiles[i]);

			BufferedReader reader = new BufferedReader(new FileReader(domainFiles[i]));
			DomainXMLDigester.getInstance().digestDomainXML(reader);
			reader.close();

			logger.info(domainFiles[i] + " loaded");
		}
	...
	private static void loadParameterTemplate(String[] templateFiles) throws ParseException, SapphireException, SQLException, SAXException,
		...
		for (int i = 0; i < templateFiles.length; i++) {
			logger.info("Reading " + templateFiles[i]);

			BufferedReader reader = new BufferedReader(new FileReader(templateFiles[i]));
			digester.digestTemplateXML(reader);
			reader.close();

			logger.info(templateFiles[i] + " loaded");
		}
	...
	private static void loadTemplate(KnowledgeBaseFilterConfig knowledgeBaseFilterConfig) throws ParseException, SapphireException,
			SQLException, SAXException, IOException, ParserConfigurationException {
		Logger logger = Logger.getLogger(Loader.class);
		logger.info("Loading templates files...");
	...
	public static void loadToCache(boolean reloadEvent, boolean doNotLoadTypeEnumValues) throws ServerException {
		...
		Logger logger = Logger.getLogger(Loader.class);
		try {
			KnowledgeBaseFilterConfig knowledgeBaseFilterConfig = ConfigurationManager.getInstance().getKnowledgeBaseFilterConfig();
			logger.info("Using dateFilter " + knowledgeBaseFilterConfig.getDateFilterConfig());
			...
			if (!reloadEvent) {
				loadDomain(ConfigurationManager.getInstance().getDomainDefinitionFiles());
			...
			if (ConfigurationManager.getInstance().getFeatureConfiguration().isFeatureEnabled(FeatureConfiguration.PARAMETER_FEATURE)) {
				loadParameterTemplate(ConfigurationManager.getInstance().getTemplateDefinitionFiles());
			}
			...
			if (ConfigurationManager.getInstance().getFeatureConfiguration().isFeatureEnabled(FeatureConfiguration.PHASE_FEATURE)) {
			...
			if (ConfigurationManager.getInstance().getFeatureConfiguration().isFeatureEnabled(FeatureConfiguration.CBR_FEATURE)) {
