...
public class AERuleBuilder extends ObjectDepthFirst {
	...
	private Object process(OrExpression orExpression, AbstractAeCompoundCondition compoundCondition) {
		logger.debug(
			">>> process(OrExpression,AbstractAeCompoundCondition): " + orExpression + "," + compoundCondition);
		Object obj = null;
		if (orExpression.f1.present()) { // f1 = NodeListOptional
			logger.debug(
				"process(OrExpression,AbstractAeCompoundCondition): * Creating new OR AePatternSet for "
					+ orExpression);
	...
	private Object process(AndExpression andExpression, AbstractAeCompoundCondition compoundCondition) {
		logger.debug(
			">>> visit(AndExpression,AbstractAeCompoundCondition): " + andExpression + "," + compoundCondition);
		Object obj = null;
		if (andExpression.f1.present()) { // f1 - NodeListOptional
			logger.debug(
				"visit(AndExpression,AbstractAeCompoundCondition): * Creating new AND AePatternSet for "
					+ andExpression);
	...
	private Object process(ExistExpression existExpression, AbstractAeCompoundCondition compoundCondition) {
		logger.debug(
			">>> visit(ExistExpression,AbstractAeCompoundCondition): " + existExpression + "," + compoundCondition);
	...
	public Object visit(RHS node, Object obj) {
		...
		if (obj instanceof AeRule) {
			String functionName = node.f0.tokenImage; // f0 is the name of the function
			RuleActionMethod raMethod =
				RuleActionMethod.valueOf(
					functionName,
					!ConfigurationManager.getInstance().getRuleGenerationConfiguration(usageType).isPEActionOn());
