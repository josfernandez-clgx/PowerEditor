...
public class RuleGeneratorHelper {
	...
	public static final DecimalFormat floatFormatter = new DecimalFormat("############0.0###");
	public static final DecimalFormat percentFormatter = new DecimalFormat("############0.00");

	static final String YES = "Yes";
	static final String NO = "No";

	private static final SimpleDateFormat ruleDateFormat = new SimpleDateFormat("MM/dd/yyyy");
	private static final SimpleDateFormat ruleDateTimeFormat = new SimpleDateFormat("MM/dd/yyyy HH:mm:ss");
	...
	public final static String toARTScriptOpString(int op) {
		...
		case Condition.OP_GREATER:
			return Condition.OPSTR_GREATER; //RuleGeneratorHelper.NUMERIC_FUNCTION_GREATER;
		case Condition.OP_GREATER_EQUAL:
			return Condition.OPSTR_GREATER_EQUAL; //RuleGeneratorHelper.NUMERIC_FUNCTION_GREATER_EQUAL;
		case Condition.OP_LESS:
			return Condition.OPSTR_LESS; //RuleGeneratorHelper.NUMERIC_FUNCTION_LESS;
		case Condition.OP_LESS_EQUAL:
			return Condition.OPSTR_LESS_EQUAL; //RuleGeneratorHelper.NUMERIC_FUNCTION_LESS_EQUAL;
	 ...
	 * @return quoted value if <code>asString</code> is <code>true</code>; unquoted value, otherwise
	 */
	public static void appendFormattedForStringType(StringBuffer buff, String value, boolean asString) {
		if (value == null) {
			buff.append(AE_NIL);
		}
		else if (asString) {
	...
	public static String formatForSprintf(String unformattedString) {
		StringBuffer buff = new StringBuffer(unformattedString.length() * 2);
	...
	public static IRange asIRangeValue(String valueStr, DeployType deployType) throws InvalidDataException {
		...
		else if (deployType == DeployType.CURRENCY || deployType == DeployType.FLOAT || deployType == DeployType.PERCENT
	...
	public static final String replaceColumnReferenceInDynamicString(DynamicStringValue dsValue, GridTemplate template,
			GridValueContainable gridData, int row) throws InvalidDataException {
		...
		while (matcher.find()) {
			...
			if (columnValue == null) {
				...
				MessageProcessor.writeEnum(
						...
						buff,
						ConfigurationManager.getInstance().getRuleGenerationConfiguration(template.getUsageType()).getMessageConfig());
			}
	...
	public static DomainAttribute findDomainAttributeForContextElement(

			RuleGenerationConfiguration.ControlPatternConfig controlPatternConfig, DomainClass dc, String contextElementType)
	...
	public static String generateMessageText(GuidelineGenerateParams ruleParams, int entityID)
			throws com.mindbox.pe.server.parser.jtb.message.ParseException {
	...
	static void writeDateValue(AbstractBufferedGenerator generator, Date date) {
	...
	public static String formatDateValueForLHS(Date date) {
		...
		if (ConfigurationManager.getInstance().getRuleGenerationConfigurationDefault().getLhsDateFormat().equalsIgnoreCase("julian")) {
			return strValue;
		}
		else {
			StringBuffer buff = new StringBuffer();
	...
	public static Date parseToDate(String str) throws java.text.ParseException {
		return ruleDateFormat.parse(str);
	}
	...
	public static String toRuleDateString(Date date) {
		if (date != null) {
			if (ConfigurationManager.getInstance().getRuleGenerationConfigurationDefault().getLhsDateFormat().equalsIgnoreCase("julian")) {
				...
				return '"' + ruleDateFormat.format(date) + '"';
			}
		}
		else {
	...
	static String toRuleDateTimeString(Date date) {
		if (date != null) {
			if (ConfigurationManager.getInstance().getRuleGenerationConfigurationDefault().getLhsDateFormat().equalsIgnoreCase("julian")) {
				...
				return '"' + ruleDateTimeFormat.format(date) + '"';
			}
		}
		else {
			return RuleGeneratorHelper.AE_NIL;
	...
	static GenericEntityType getFirstDisallowedEntityInContext(ProductGrid productGrid) {
		...
		RuleGenerationConfiguration.ControlPatternConfig controlPatternConfig = ConfigurationManager.getInstance().getRuleGenerationConfiguration(
				usageType).getControlPatternConfig();
	...
	public static RuleDefinition toRuleDefinition(GridTemplate template, int columnNo, String ruleName, DeploymentRule deploymentRule,
			MutableBoolean wasActionCreated, User user) throws SapphireException, ServletActionException, ParseException {
	...
	private static ActionTypeDefinition findMatchingActionType(GridTemplate template, int columnNo, AeRule aeRule)
	 ...
	 */
	private static String generateActionTypeDeploymentRule(GridTemplate template, int columnNo, AeRule aeRule,
			boolean generateParamDefinitions, boolean generateRuleParams, ActionTypeDefinition actionDef, RuleAction ruleAction) {
		StringBuffer buff = new StringBuffer();
		...
		for (Iterator<AbstractAeValue> iter = aeRule.getActionParms().iterator(); iter.hasNext();) {
			...
			if (element instanceof AeColumnValue) {
				if (generateRuleParams)
					addActionParamDefinitionAndRuleParam(
							actionDef,
							ruleAction,
							paramID,
							template,
							((AeColumnValue) element).getColumnNumber(),
							generateParamDefinitions);
				buff.append("\"%parameter " + paramID + "%\"");
				++paramID;
			}
			else if (element instanceof AeCellValue) {
				if (generateRuleParams)
					addActionParamDefinitionAndRuleParam(actionDef, ruleAction, paramID, template, columnNo, generateParamDefinitions);
				buff.append("\"%parameter " + paramID + "%\"");
				++paramID;
			}
				...
				if (token.startsWith("\"%column ")) {
					...
					if (generateRuleParams)
						addActionParamDefinitionAndRuleParam(actionDef, ruleAction, paramID, template, col, generateParamDefinitions);
					buff.append("\"%parameter " + paramID + "%\"");
					++paramID;
				}
	...
	private static RuleAction toRuleAction(GridTemplate template, int columnNo, String ruleName, AeRule aeRule,
			MutableBoolean wasActionCreated, User user) throws ServletActionException, ParseException {
		Logger logger = Logger.getLogger(RuleGeneratorHelper.class);
		...
		String deploymentRule = generateActionTypeDeploymentRule(
				template,
				columnNo,
				aeRule,
				(actionDef.getID() < 0),
	...
	private static void addActionParamDefinitionAndRuleParam(FunctionTypeDefinition actionDef,
			CompoundRuleElement<FunctionParameter> ruleAction, int paramID, GridTemplate template, int columnNo,
	...
	private static class RuleDefProcessor {
		...
		private RuleDefProcessor(int columnNo, RuleDefinition ruleDefinition, GridTemplate template, User user) {
		...
		private TestTypeDefinition toTestTypeDefinition(AeTestFunctionPattern testPattern, TestCondition testCondition) {
			TestTypeDefinition def = new TestTypeDefinition(-1, testPattern.getFunctionName(), testPattern.getFunctionName()
					+ " - Created by PowerEditor");
			...
			for (int i = 0; i < testPattern.size(); i++) {
				...
				if (element instanceof AeColumnValue) {
					...
					addActionParamDefinitionAndRuleParam(def, testCondition, paramID, template, columnNo, true);
					buff.append("\"%parameter " + paramID + "%\"");
					++paramID;
				}
				else if (element == null) {
					...
					if (token.startsWith("\"%column ")) {
						int col = Integer.parseInt(token.substring(9, token.length() - 2));
						addActionParamDefinitionAndRuleParam(def, testCondition, paramID, template, col, true);
						buff.append("\"%parameter " + paramID + "%\"");
						++paramID;
					}
					else if (token.equals("\"%cellValue%\"")) {
		...
		private LHSElement toRuleElement(AeAttributePattern aeattributepattern) throws SapphireException {
			...
			if (aeattributepattern.getClassName() == null) {
				aeattributepattern.setClassName((aeattributepattern.getParentObjectPattern() == null
						? "ERROR_UNKNOWN"
						: aeattributepattern.getParentObjectPattern().getClassName()));
			}
			condition.setReference(RuleElementFactory.getInstance().createReference(
					aeattributepattern.getClassName(),
					aeattributepattern.getAttributeName()));
			...
			if (value instanceof AeCellValue) {
				...
				condition.setValue(RuleElementFactory.getInstance().createValue(
						RuleElementFactory.getInstance().createColumnReference(((AeColumnValue) value).getColumnNumber())));
			}
			...
			if (valueStr != null && valueStr.length() > 0 && valueStr.charAt(0) == '[' && valueStr.charAt(valueStr.length() - 1) == ']') {
				...
				if (values != null) {
					...
					for (int i = 0; i < values.length; i++) {
						...
						if (i > 0) buff.append(',');
						if (valueToProcess != null && valueToProcess.length() > 0 && valueToProcess.charAt(0) == '"'
								&& valueToProcess.charAt(valueToProcess.length() - 1) == '"') {
		...
		private LHSElement toRuleElement(AeObjectPattern aeobjectpattern) throws SapphireException {
			...
			if (abstractaecondition != null) {
				LHSElement ruleElement = toRuleElement(abstractaecondition);
				if (ruleElement instanceof CompoundLHSElement
						&& ((CompoundLHSElement) ruleElement).getType() == CompoundLHSElement.TYPE_AND) {
	...
	public static StringBuffer generateContextSequence(String type, String categoryOrEntity, int[] ids) {
