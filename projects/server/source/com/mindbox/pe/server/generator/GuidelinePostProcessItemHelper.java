...
final class GuidelinePostProcessItemHelper extends AbstractGuidelineHelper {
	...
	private class MessageVisitor extends ObjectDepthFirst {
		...
		public Object visit(CellValueLiteral n, Object argu) {
			...
			Object cellValue = (cellValueOverride == -1
					? ((AbstractGenerateParms) argu).getCellValue()
					: ((AbstractGenerateParms) argu).getColumnValue(cellValueOverride));
	...
	GuidelinePostProcessItemHelper(GuidelineRuleGenerator ruleGenerator) {
		super(ruleGenerator);
		...
		if (!postProcessingItemList.isEmpty()) {
			...
			for (Iterator<Reference> iter = postProcessingItemList.iterator(); iter.hasNext();) {
				...
				if (!guidelineRule.hasPatternForReference((Reference) item)) {
					...
					if (iter.hasNext()) ruleGenerator.nextLine();
				}
				else {
	...
	private void writeEmptyAttributePattern(Reference reference, TemplateUsageType usageType) throws RuleGenerationException {
		...
		ruleGenerator.print(AeMapper.makeAEVariable(className));
		ruleGenerator.nextLineIndent();
		ruleGenerator.openParan();
		ruleGenerator.print("instance-of ");
		ruleGenerator.print(mappedClassName);
		ruleGenerator.closeParan();
		ruleGenerator.nextLine();

		// if prefix matches, insert lineage static pattern
		ruleGenerator.writeLineagePatternIfMatch(mappedClassName, usageType);
		...
		if (reference.getAttributeName() != null && reference.getAttributeName().length() > 0) {
			...
			ruleGenerator.openParan();
			ruleGenerator.print(attribName);

			// append '$' only if configured to do so
			if (ConfigurationManager.getInstance().getRuleGenerationConfiguration(usageType).getEmptyPatternConfig() != null
					&& ConfigurationManager.getInstance().getRuleGenerationConfiguration(usageType).getEmptyPatternConfig().generateAsSequence()) {
				ruleGenerator.print(" $");
			}
			else {
				ruleGenerator.print(" ");
			}
			ruleGenerator.print(attribVarName);
			ruleGenerator.closeParan();
		}
	...
	synchronized void processForAttributeAndClassReferencesInRHS(FunctionCallPattern functionCallPattern, GuidelineGenerateParams ruleParams)
	...
	private String addToPostProcessingItem_Columns(int column, DynamicStringValue dsValue, GuidelineGenerateParams ruleParams) throws RuleGenerationException {
		...
		while (matcher.find()) {
			...
			if (columnValue == null) {
				...
				MessageProcessor.writeVal(ruleParams.getTemplate().getColumn(col), columnValue, buff, ConfigurationManager.getInstance().getRuleGenerationConfiguration(
						ruleParams.getUsage()).getMessageConfig(), columnValue.toString());
			}
		}
	 ...
	 * @throws RuleGenerationException
	 */
	private List<String> addToPostProcessingItem_ClassAttribute(int column, DynamicStringValue dsValue, GuidelineGenerateParams ruleParams, String strAfterColumns)
			throws RuleGenerationException {
		logger.debug(">>> addToPostProcessingItem_ClassAttribute: column=" + column + ", DynamicStringValue=" + dsValue + ", ruleParams=" + ruleParams
				+ ", strAfterColumns=" + strAfterColumns);
	 ...
	 * @param varList
	 * @param buff
	 * @return boolean value indicating wether we matched with {@link com.mindbox.pe.model.DomainClass },
	 *  {@link com.mindbox.pe.model.DomainTranslation} or {@link com.mindbox.pe.model.DomainAttribute}
	 * @throws RuleGenerationException
	 */
	private boolean addToPostProcessingItem_aux(String className, String subName, List<String> varList, StringBuffer buff) throws RuleGenerationException {
