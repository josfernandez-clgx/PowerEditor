...
import com.mindbox.pe.server.config.RuleGenerationConfiguration.RequestPatternConfig;
...
public final class ObjectPatternFactory {
	...
	public ObjectPattern createControlPattern(TemplateUsageType usageType) throws RuleGenerationException {
		if (helper.getRuleGenerationConfiguration(usageType).getControlPatternConfig().isPatternOn()) {
			...
			String className = helper.getDeployLabelForClass(controlPatternConfig.getPatternClassName());
			String variableName = helper.asVariableName(controlPatternConfig.getPatternClassName());
			...
			for (int i = 0; i < entityTypes.length; i++) {
				if (entityTypes[i].isUsedInContext() && !controlPatternConfig.isDisallowed(entityTypes[i])) {
					...
					objectPattern.add(new ContextElementAttributePattern(
							attributeName,
							attrVarName,
							entityTypes[i],
							da.getDeployType().equals(DeployType.STRING)));
				}
			}
	...
	public ObjectPattern createRequestPattern(TemplateUsageType usageType) throws RuleGenerationException {
		RequestPatternConfig requestPatternConfig = helper.getRuleGenerationConfiguration(usageType).getRequestPatternConfig();
		if (requestPatternConfig != null && requestPatternConfig.isPatternOn()) {
			String className = requestPatternConfig.getPrefix() + requestPatternConfig.getPatternClassName();
			String variableName = "?" + requestPatternConfig.getPatternClassName();
			ObjectPattern objectPattern = OptimizingObjectPattern.createShouldBeFirstInstance(className, variableName);
			objectPattern.setCanBeSkipped(false);
			objectPattern.add(new FocusOfAttentionAttributePattern(requestPatternConfig.getPrefix() + "focus-of-attention"));
			objectPattern.add(new TimeSliceAttributePattern(
					requestPatternConfig.getPrefix() + "current-time-slice",
					RuleGeneratorHelper.TIME_SLICE_VARIABLE));
			return objectPattern;
		}
		else {
			return null;
	...
	public ObjectPattern createEmptyObjectPattern(String className) throws RuleGenerationException {
		...
		ObjectPattern objectPattern = OptimizingObjectPattern.createNormalInstance(
				helper.getDeployLabelForClass(className),
				helper.asVariableName(className));
		objectPattern.setCanBeSkipped(false);
		return objectPattern;
	}
	...
	public ObjectPattern createEmptyObjectPattern(ExistExpression existExpression) throws RuleGenerationException {
		...
		ObjectPattern objectPattern =  OptimizingObjectPattern.createNormalInstance(
				helper.getDeployLabelForClass(existExpression.getClassName()),
				existObjectNameVar);
		objectPattern.setCanBeSkipped(false);
		return objectPattern;
	}
	...
	public ObjectPattern createObjectPattern(Condition condition, TemplateUsageType usageType, String objectVarOverride)
			throws RuleGenerationException {
		...
		ObjectPattern objectPattern = OptimizingObjectPattern.createNormalInstance(
				...
				helper.asVariableName(condition.getReference().getClassName(), (UtilBase.isEmpty(condition.getObjectName())
	...
	public ObjectPattern createLinkObjectPattern(DomainClassLinkPattern linkPattern, TemplateUsageType usageType)
			throws RuleGenerationException {
	...
	public ObjectPattern createSingleAttrbiuteObjectPattern(Reference reference) throws RuleGenerationException {
		...
		if (!UtilBase.isEmpty(reference.getAttributeName())) {
			objectPattern.add(new StaticTextAttributePattern(
					helper.getDeployLabelForAttribute(reference),
					helper.asVariableName(reference.getAttributeName())));
		}
		return objectPattern;
	}
}
