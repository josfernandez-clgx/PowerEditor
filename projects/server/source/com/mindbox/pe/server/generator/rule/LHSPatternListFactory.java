...
public final class LHSPatternListFactory {
	...
	private void process(LHSPatternList parentPatternList, CompoundLHSElement elements, String objectVarOverride)
	...
	private void process_aux(LHSPatternList patternListToAddElements, CompoundLHSElement elements, String objectVarOverride)
			throws RuleGenerationException {
	...
	private void process(LHSPatternList patternList, ExistExpression existExpression, LHSPatternList patternListToAddNextedElementsInExist)
			throws RuleGenerationException {
		...
		process(patternListToAddNextedElementsInExist, existExpression.getCompoundLHSElement(), (existExpression.getObjectName() == null
				? null
				: "?" + existExpression.getObjectName()));
	}

	private void processChildLinkPatterns(ObjectPattern objectPattern, String parentClassName, CompoundLHSElement childElements,
			LHSPatternList patternList) throws RuleGenerationException {
		for (int i = 0; i < childElements.size(); i++) {
			...
			if (element instanceof Condition) {
				...
				processChildLinkPatterns(
						objectPattern,
						parentClassName,
						((Condition) element).getReference().getClassName(),
						null,
						patternList);
				if (((Condition) element).getValue() instanceof Reference) {
					...
					processChildLinkPatterns(
							objectPattern,
							parentClassName,
							((Reference) ((Condition) element).getValue()).getClassName(),
							null,
							patternList);
				}
			}
				...
				processChildLinkPatterns(
						objectPattern,
						parentClassName,
						((ExistExpression) element).getClassName(),
						((ExistExpression) element).getObjectName(),
						patternList);
			}
	...
	private void processChildLinkPatterns(ObjectPattern objectPattern, String parentClassName, String childClassName, String objectName,
			LHSPatternList patternList) throws RuleGenerationException {
		...
		if (dcLinks == null || dcLinks.length == 0) {
			...
			if (helper.getDeploymentConfig().isReportMissingLinkOn()) {
				helper.reportError("WARNING: No linkage generated for child " + childClassName + " from " + parentClassName
						+ " - NO LINKAGE EXISTS");
			}
		}
		else {
			logger.debug("... processChildLinkPatterns_aux: dcLinks.length = " + dcLinks.length);
			AttributePattern attributePattern = attributePatternFactory.createLinkAttributePattern(dcLinks[0], (dcLinks.length == 1
					? objectName
					: null), usageType);
