...
class ContextElementValueSlotHelper extends AbstractLHSValueSlotHelper {
	...
	ContextElementValueSlotHelper(RuleGenerationConfiguration ruleGenerationConfiguration) {
		super(ruleGenerationConfiguration);
	}

	void appendContextMatchFunctionPatternAsOf(StringBuilder valueBuff, StringBuilder commentBuff, String varName, GenericEntityType entityType,
			int[] categoryIDs, int[] entityIDs, boolean exclude, Date date) {
	...
	private void appendControlPatternLineForEntityType(StringBuilder valueBuff, StringBuilder commentBuff, String varName,
			TimeSlice timeSlice, GuidelineGenerateParams ruleParams, GenericEntityType type, boolean forceQuote)
			throws RuleGenerationException {
		// if the context contains categories only
		if (ruleParams.hasGenericCategoryContext(type)) {
			appendContextMatchFunctionPatternAsOf(
					valueBuff,
					commentBuff,
					varName,
					type,
					ruleParams.getGenericCategoryIDs(type),
					null,
					false,
	...
	private void appendControlPatternValueForSingleEntity(StringBuilder valueBuff, StringBuilder commentBuff, GenericEntityType type,
			int entityID, boolean forceQuote) throws RuleGenerationException {
		valueBuff.append(" & ");
	...
	public ValueAndComment generateValue(PatternValueSlot patternValueSlot, GuidelineGenerateParams generateParams, String attribVarName,
		...
		ContextElementPatternValueSlot cepValueSlot = (ContextElementPatternValueSlot) patternValueSlot;
		...
		if (messageContextType == null || cepValueSlot.getGenericEntityType() != messageContextType) {
			if (generateParams.hasGenericCategoryContext(cepValueSlot.getGenericEntityType())
					|| generateParams.hasGenericEntityContext(cepValueSlot.getGenericEntityType())) {
				// QN 2007-0817 always format as integer
				appendControlPatternLineForEntityType(
						valueBuff,
						commentBuff,
						attribVarName,
						timeSlices[0],
						generateParams,
						cepValueSlot.getGenericEntityType(),
						false);
			}
		}
		else if (messageContextType != null && cepValueSlot.getGenericEntityType() == messageContextType) {
			// QN 2007-0817 always format as integer
			appendControlPatternValueForSingleEntity(
					valueBuff,
					commentBuff,
					cepValueSlot.getGenericEntityType(),
