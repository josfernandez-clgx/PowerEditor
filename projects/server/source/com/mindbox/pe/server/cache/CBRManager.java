...
public class CBRManager extends AbstractCacheManager {
	...
	public CBRScoringFunction getCBRScoringFunction(int id) {
		CBRScoringFunction cbrScoringFunction =
			(CBRScoringFunction) cbrScoringFunctionMap.get(new Integer(id));
	...
	public synchronized void addCBRCaseClass(CBRCaseClass cbrCaseClass) {
		...
		if (cbrCaseClass.getSymbol() == null || cbrCaseClass.getSymbol().length()== 0)
			throw new IllegalArgumentException("Cannot add a CBR case class with no symbol");
	...
	public synchronized void addCBRCaseBase(CBRCaseBase cbrCaseBase) {
		...
		if (cbrCaseBase.getID() == 0)
			throw new IllegalArgumentException("Cannot add a CBR case base with no id");
	...
	public synchronized void addCBRCaseAction(CBRCaseAction cbrCaseAction) {
		...
		if (cbrCaseAction.getSymbol() == null || cbrCaseAction.getSymbol().length()== 0)
			throw new IllegalArgumentException("Cannot add a CBR case action with no symbol");
