...
public final class ParameterManager extends AbstractCacheManager {
	...
	public void addParameterGrid(int gridID, int templateID, String cellValue, int numRows, DateSynonym effDate, DateSynonym expDate,
			String status) {
	...
	public boolean addGridContext(int gridID, int categoryType, int categoryID) {
		...
		if (gridMap.containsKey(key)) {
			ParameterGrid grid = gridMap.get(key);
			grid.addGenericCategoryID(ConfigurationManager.getInstance().getEntityConfiguration().findEntityTypeForCategoryType(
	...
	public void insertIntoCache(int gridID, int templateID, String cellValue, int numRows, DateSynonym effDate, DateSynonym expDate,
			String status, GuidelineContext[] context) {
		ParameterGrid grid = new ParameterGrid(gridID, templateID, effDate, expDate);
		grid.setCellValues(cellValue);
		grid.setNumRows(numRows);
		grid.setStatus(status);
