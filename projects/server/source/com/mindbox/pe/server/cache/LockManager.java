...
public class LockManager extends AbstractCacheManager {
	...
	private final Map<EntityType, Map<Integer, LockInfo>> entityLockMap;
	private final Map<EntityType, Map<String, LockInfo>> namedEntityLockMap;

	private LockManager() {
		geneicEntityLockMap = Collections.synchronizedMap(new HashMap<GenericEntityType, Map<Integer, LockInfo>>());
		entityLockMap = Collections.synchronizedMap(new HashMap<EntityType, Map<Integer, LockInfo>>());
		namedEntityLockMap = Collections.synchronizedMap(new HashMap<EntityType, Map<String, LockInfo>>());
	...
	public LockInfo lock(EntityType entityType, int entityID, User user) throws LockException {
		if (entityID < 0) throw new IllegalArgumentException("Invalid entity id: " + entityID);
		if (user == null) throw new NullPointerException("User cannot be null");
	...
	public LockInfo lock(EntityType entityType, String name, User user) throws LockException {
	...
	public LockInfo getExistingLock(EntityType entityType, int id, User user) throws LockException {
		if (id < 0) throw new IllegalArgumentException("Invalid entity id: " + id);
		return handleExistingLock((LockInfo) getEntityLockMap(entityType).get(new Integer(id)), user);
	}

	public LockInfo getExistingLock(EntityType entityType, String name, User user) throws LockException {
	...
	public boolean unlock(EntityType entityType, int entityID, User user) throws LockException {
		LockInfo lockInfo = getExistingLock(entityType, entityID, user);
		if (lockInfo != null) {
	...
	public boolean unlock(EntityType entityType, String name, User user) throws LockException {
	...
	public void unlockAll(User user) {
		...
		for (EntityType entityType : entityLockMap.keySet()) {
			removeAllLocks(entityLockMap.get(entityType), user);
		}
		for (EntityType entityType : namedEntityLockMap.keySet()) {
	...
	private Map<Integer, LockInfo> getEntityLockMap(EntityType entityType) {
	...
	private Map<String, LockInfo> getNamedEntityLockMap(EntityType entityType) {
