...
import com.mindbox.pe.model.GridCellCoordinates;
import com.mindbox.pe.model.GridCellSet;
...
public class ReportGenerator {
	...
	public static String toElementName(CategoryTypeDefinition categoryTypeDefinition) {
			...
			for (int col = 1; col <= columnNames.length; col++) {
				try {
					...
					if ((cellValueObject instanceof EnumValues || cellValueObject instanceof String)
						...
						EnumValues selectedEnumValues = cellValueObject instanceof EnumValues
								? (EnumValues) cellValueObject
								: EnumValues.parseValue((String) cellValueObject);
	...
	private static String enumValuesToDisplayString(EnumValues<EnumValue> selectedEnumValues) {
		...
		String prefix = enumDisplayValues.isSelectionExclusion() && !enumDisplayValues.isEmpty() ? "<font color='red'>"
				+ EnumValues.OLD_EXCLUSION_PREFIX + "</font>" : "";
	...
	//	private static EnumValue findMatchingEnumVal(Object obj, List<?> enumVals) {
	//		EnumValue retVal = null;
	//		for (Iterator<?> it = enumVals.iterator(); it.hasNext();) {
	//			Object enumObj = it.next();
	//			if (enumObj instanceof EnumValue) { // for enums came from attribute map
	//				EnumValue val = (EnumValue) enumObj;
	//
	//				if (val.getDisplayLabel().equals(obj.toString())
	//						|| (val.hasDeployID() && val.getDeployID().toString().equals(obj.toString()))
	//						|| (!val.hasDeployID() && val.getDeployValue().equals(obj.toString()))) {
	//					retVal = val;
	//				}
	//			}
	//			else if (enumObj instanceof String && Util.isSame(obj, enumObj)) { // for enums defined in the column
	//				retVal = new EnumValue();
	//				retVal.setDisplayLabel((String) enumObj);
	//				retVal.setDeployValue((String) enumObj);
	//				retVal.setDeployID(new Integer(enumObj.hashCode()));
	//			}
	//		}
	//		return retVal;
	//	}
	...
	public static int[] extractMessageContextEntityIDs(GuidelineContext[] context) {
		...
		if (etDef != null) {
			for (int i = 0; i < context.length; i++) {
				if (etDef.getTypeID() == context[i].getGenericCategoryType()) {
	...
	public static GenericEntityType getMessageContextEntityType() {
		EntityTypeDefinition etDef = ConfigurationManager.getInstance().getEntityConfiguration().getEntityTypeForMessageContext();
		return (etDef == null ? null : GenericEntityType.forID(etDef.getTypeID()));
	}
	 ...
	 * @since PowerEditor 4.4.0
	 */
	public static final String extractContext(GuidelineContext[] contexts, EntityTypeDefinition typeDef) {
		StringBuffer buff = new StringBuffer();
		boolean hasText = false;
		for (int i = 0; i < contexts.length; i++) {
			if (contexts[i].getGenericEntityType() != null && contexts[i].getGenericEntityType().getID() == typeDef.getTypeID()) {
	 ...
	 * @since PowerEditor 4.4.0
	 */
	public static final String extractContext(GuidelineContext[] contexts, CategoryTypeDefinition typeDef) {
		StringBuffer buff = new StringBuffer();
		boolean hasText = false;
		for (int i = 0; i < contexts.length; i++) {
			int[] ids = contexts[i].getIDs();
			if (contexts[i] != null && contexts[i].getGenericCategoryType() == typeDef.getTypeID()) {
				for (int j = 0; j < ids.length; j++) {
					GenericCategory obj = EntityManager.getInstance().getGenericCategory(typeDef.getTypeID(), ids[j]);
					if (obj != null) {
						if (hasText) buff.append(",");
						buff.append(obj.getName());
						if (!hasText) hasText = true;
		...
		if (contexts != null && contexts.length > 0) {
			for (int i = 0; i < contexts.length; i++) {
				...
				if (contexts[i].getGenericEntityType() != null) {
					...
					// handle generic entity types
					CategoryTypeDefinition catTypeDef = ConfigurationManager.getInstance().getEntityConfiguration().findCategoryTypeDefinition(
	...
	private static String generateError(String errorStr, String detail) {
		StringBuffer buff = new StringBuffer("<br/><span class='error'>");
	...
	private static String asGridCellValue(CategoryOrEntityValue categoryOrEntityValue) {
		if (categoryOrEntityValue.isForEntity()) {
			GenericEntity entity = EntityManager.getInstance().getEntity(
					categoryOrEntityValue.getEntityType(),
					categoryOrEntityValue.getId());
			return (entity == null
					? categoryOrEntityValue.getEntityType() + "[id=" + categoryOrEntityValue.getId() + "]"
					: entity.getName());
		}
		else {
			GenericCategory category = EntityManager.getInstance().getGenericCategory(
					categoryOrEntityValue.getEntityType().getCategoryType(),
					categoryOrEntityValue.getId());
			return (category == null
					? categoryOrEntityValue.getEntityType() + "-Category[id=" + categoryOrEntityValue.getId() + "]"
					: category.getName());
		}
	}
	...
	private static String toActivationReportString(ProductGrid grid) {
		...
		if (grid.getSunrise() != null) {
			buff.append(UIConfiguration.FORMAT_DATE_TIME_SEC.format(grid.getSunrise()));
		}
		buff.append(" - ");
		if (grid.getSunset() != null) {
			buff.append(UIConfiguration.FORMAT_DATE_TIME_SEC.format(grid.getSunset()));
		}
		return buff.toString();
	}
	...
	private static void appendEntityLabelName(StringBuffer buff, GenericEntityType entityType, int entityID) {
	 ...
	 */
	private static boolean appendMessageStringForReport(StringBuffer buff, GridTemplate template, GenericEntityType entityType,
			int entityID, int column, GridValueContainable grid, boolean replaceColumnRef, int row) throws InvalidDataException {
		try {
			...
			Logger.getLogger(ReportGenerator.class).warn(
					"Message for Template:" + template + ",Column" + column + ",entityID:" + entityID + " has syntax error",
					ex);
	...
	private static void appendMessageString(StringBuffer buff, GridTemplate template, GenericEntityType entityType, int entityID,
			int column, GridValueContainable grid, boolean replaceColumnRef, int row) throws InvalidDataException {
		try {
			Message messageObj = findMessage(template, column, entityID);
			if (messageObj != null) {
				buff.append("<li>");
	...
	public synchronized void generateGuidelineReport(GuidelineReportSpec reportSpec, List<GuidelineReportData> guidelineList)
		...
		try {
			...
			for (Map.Entry<TemplateUsageType, List<GridTemplate>> entry : usageTemplateMap.entrySet()) {
				...
				// write usage info
				writer.println("<a name='usage_" + usage.toString() + "'><span class='usage'>Guideline Type:  " + usage.getDisplayName()
						+ "</span></a><br/>");
				writer.flush();
	...
	private synchronized void writeGuidelineReport(GuidelineReportSpec reportSpec, List<GridTemplate> templateList,
			List<GuidelineReportData> guidelineList) throws ServletActionException, ReportException, InvalidDataException, IOException,
			ParseException {
		logger.debug(">>> writeGuidelineReport(templateList): spec=" + reportSpec + ",templateList.size()" + templateList.size()
				+ ",guidelineList.size()=" + guidelineList.size());
		for (Iterator<GridTemplate> iter = templateList.iterator(); iter.hasNext();) {
			GridTemplate template = iter.next();

			// write template info
			writer.println("<a name='template_" + template.getID() + "'><span class='template'>Template:  " + template.getName()
					+ "   Version " + template.getVersion() + "</span></a><br/>");
			writer.flush();
	...
	private synchronized void writeGuidelineReport(GuidelineReportSpec reportSpec, GridTemplate template, GuidelineReportData guidelineData)
		...
		if (cachedGrid == null) {
			...
			ProductGrid grid = ProductGrid.copyOf(
					cachedGrid,
					(GridTemplate) cachedGrid.getTemplate(),
					cachedGrid.getEffectiveDate(),
					cachedGrid.getExpirationDate());

			GenericEntityType entityType = (ConfigurationManager.getInstance().getEntityConfiguration().getEntityTypeForMessageContext() == null
					? null
					: GenericEntityType.forID(ConfigurationManager.getInstance().getEntityConfiguration().getEntityTypeForMessageContext().getTypeID()));

			int rowID = 1;
			// template and context are common among all grid in gridList
			rowID = writeAs2ColumnTableRow(rowID, ResourceUtil.getInstance().getResource("label.template"), template.getName() + " ("
					+ template.getID() + ')' + "  Version " + template.getVersion());
			rowID = writeAs2ColumnTableRow(
					rowID,
					ResourceUtil.getInstance().getResource("label.context"),
					toHTMLString(guidelineData.getContext()));

			rowID = writeAs2ColumnTableRow(
					rowID,
					ResourceUtil.getInstance().getResource("label.activation"),
					toActivationReportString(grid));

			if (reportSpec.isCreatedDateOn()) {
				rowID = writeAs2ColumnTableRow(
						rowID,
						ResourceUtil.getInstance().getResource("label.date.creation"),
						UIConfiguration.FORMAT_DATE_TIME_SEC.format(guidelineData.getCreationDate()));
			}
			...
			if (reportSpec.isStatusChangeDateOn()) {
				rowID = writeAs2ColumnTableRow(
						...
						UIConfiguration.FORMAT_DATE_TIME_SEC.format(grid.getStatusChangeDate()));
			}
			if (reportSpec.isCommentsOn()) {
				rowID = writeAs2ColumnTableRow(rowID, ResourceUtil.getInstance().getResource("label.comments"), "<pre>"
						+ asGridCellValue(grid.getComments()) + "</pre>");
			}
			...
			if (reportSpec.isGridOn()) {
				rowID = writeAs2ColumnTableRow(rowID, ResourceUtil.getInstance().getResource("label.view.grid"), generateGridView(
						template,
						grid,
						entityType,
						(entityType == null ? null : grid.getGenericEntityIDs(entityType)),
						guidelineData.getCellSubset()));
			}
			if (reportSpec.isRowOn()) {
				rowID = writeAs2ColumnTableRow(rowID, ResourceUtil.getInstance().getResource("label.view.rule"), generateRuleView(
						template,
						grid,
						entityType,
						(entityType == null ? null : grid.getGenericEntityIDs(entityType))));
	...
	private String generateRuleView(GridTemplate template, ProductGrid grid, GenericEntityType type, int[] entityIDs)
	...
	private String generateGridView(GridTemplate template, ProductGrid grid, GenericEntityType entityType, int[] entityIDs,
			GridCellSet cells) throws ReportException, InvalidDataException, ParseException {
		StringBuffer buff = new StringBuffer();
		if (cells.isEmpty()) {
	...
	public static String generateRowRuleStringForReport(int row, RuleDefinition ruleDefinition, GridTemplate template,
			GridValueContainable grid) throws ReportException {
	...
	public static String generateRowMessageStringForReport(int row, int columnNo, GridTemplate template, GenericEntityType entityType,
			int[] entityIDs, GridValueContainable grid) throws InvalidDataException, ParseException {
		StringBuffer buff = new StringBuffer();
	...
	public static String generateRowMessageStringForReport(int row, GridTemplate template, GenericEntityType entityType, int[] entityIDs,
			GridValueContainable grid) throws InvalidDataException, ParseException {
		StringBuffer buff = new StringBuffer();
	...
	private static String generateRowMessageString(int row, GridTemplate template, GenericEntityType entityType, int[] entityIDs,
			GridValueContainable grid) throws InvalidDataException, ParseException {
		StringBuffer buff = new StringBuffer();
	...
	public static String generateMessageStringForReport(GridTemplate template, int[] entityIDs, GridValueContainable grid)
	...
	private static String generateMessageString(GridTemplate template, GenericEntityType entityType, int[] entityIDs,
			GridValueContainable grid) throws InvalidDataException, ParseException {
		StringBuffer buff = new StringBuffer();
		if (hasRule(template)) {
			buff.append("<ul>");
