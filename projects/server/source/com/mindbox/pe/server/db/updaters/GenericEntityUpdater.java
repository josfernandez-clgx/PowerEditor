...
public class GenericEntityUpdater extends DateSynonymReferenceUpdater implements GenericEntityDataUpdater {
	...
	private static final String Q_DELETE_ENTITY_COMPATIBILITY = "delete from MB_ENTITY_COMPATIBILITY"
			+ " where entity1_id=? and entity1_type=? and entity2_id=? and entity2_type=?";
	...
	public void addCategory(int id, String name, int categoryType, MutableTimedAssociationKey[] parentAssocations) throws SQLException {
		...
		try {
			...
			if (count < 1) { throw new SQLException("Insert failed: no rows inserted"); }
	...
	public void updateCategoryName(int id, int categoryType, String newName) throws SQLException {
		...
		try {
			...
			int count = ps.executeUpdate();
			if (count < 1) { throw new SQLException("updateCategoryName failed: no rows updated"); }
			conn.commit();
	...
	public void updateCategory(int id, String name, int categoryType, MutableTimedAssociationKey[] parentAssocations) throws SQLException {
		...
		try {
			...
			int count = ps.executeUpdate();
			if (count < 1) { throw new SQLException("Update failed: no rows updated"); }
	...
	public void insertGenericEntity(int id, int type, String name, int parentID, Map<String,Object> propertyMap, int categoryType, MutableTimedAssociationKey[] categoryAssociations)
		...
		try {
			...
			for (Map.Entry<String,Object> element : propertyMap.entrySet()) {
				...
				count = ps.executeUpdate();
				if (count < 1) { throw new SQLException("no generic entity rows inserted"); }
		...
		if (categoryAssociations != null && categoryAssociations.length > 0 && categoryType != -1) {
			...
			try {
				...
				for (int i = 0; i < categoryAssociations.length; i++) {
					...
					int count = ps.executeUpdate();
					if (count < 1) { throw new SQLException("no entity-to-category rows inserted"); }
				}
			}
			finally {
				if (ps != null) ps.close();
	...
	public void insertEntityCompatibility(int entityType1, int entityID1, int entityType2, int entityID2, DateSynonym effectiveDate, DateSynonym expirationDate)
			throws SQLException {
	...
	public void updateEntityCompatibility(int entityType1, int entityID1, int entityType2, int entityID2, DateSynonym effectiveDate, DateSynonym expirationDate)
			throws SQLException {
	...
	public void replaceDateSynonymReferences(DateSynonym[] toBeReplaced, DateSynonym replacement) throws SQLException {
		replaceDateSynonymReferencesInIntersectionTable(
				toBeReplaced,
				replacement,
				Q_REPLACE_ALL_COMPATIBILITY_EFFECTIVE_DATE_SYNONYM,
				Q_REPLACE_ALL_COMPATIBILITY_EXPIRATION_DATE_SYNONYM);
	}
