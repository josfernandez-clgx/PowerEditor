...
public class DBIdGenerator {
	...
	public static final String REGEX_VALID_ID_TYPES = 
		"^(" + SEQUENTIAL_ID + "|" + FILTER_ID + "|" + GRID_ID + "|" + AUDIT_ID + "|" + RULE_ID + ")$";
	...
	private static Hashtable<String, DBCachedId> idMap = new Hashtable<String, DBCachedId>();

	private class DBCachedId {

		public String toString() {
			String s = "NextId=" + mNextId + " MaxId=" + mMaxId + " CachedIds=" + mIdCacheSize;
			return s;
		}
	...
	public String toString() {
		String s = "";
		s += "IdGenerator managing " + idMap.size() + " Id objects! ";
		s += idMap.toString();
		return s;
	}
	...
	private synchronized int next(String s) throws SapphireException {
		int i = -1;
		DBCachedId dbcachedid = idMap.get(s);
		if (dbcachedid == null) throw new SapphireException("Invalid Type: " + s);
		boolean flag = true;
		while (flag) {
			try {
				i = dbcachedid.next();
