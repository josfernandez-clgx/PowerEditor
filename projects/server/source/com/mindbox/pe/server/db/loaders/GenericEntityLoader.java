				...
				if (dateSysnonymsPassFilter(effID, expID, knowledgeBaseFilterConfig.getDateFilterConfig())) {
					...
					if (GenericEntityType.hasTypeFor(type1) && GenericEntityType.hasTypeFor(type2)) {
						...
						logger.info(String.format(
								"Loaded Compatibility: id1=%s,type1=%s,id2=%d,type2=%s,effDateID=%d,expDateID=%d",
								id1,
								type1,
								id2,
								type2,
					...
					logger.info(String.format(
							"*SKIPPED* Compatibility Filtered out by KBDateFilter: id1=%s,type1=%s,id2=%d,type2=%s,effDateID=%d,expDateID=%d",
							id1,
							type1,
							id2,
							type2,
			...
			while (rs.next()) {
				...
				if (dateSysnonymsPassFilter(effID, expID, knowledgeBaseFilterConfig.getDateFilterConfig())) {
					try {
						...
						logger.info("Generic category parent: cat=" + catID + ",type=" + type + ",parent=" + parentID + ", effID=" + effID
								+ ",expID=" + expID);
					}
					catch (Exception ex) {
						logger.warn("Ignore invalid association: cat=" + catID + ",type=" + type + ",parent=" + parentID + ", effID="
								+ effID + ",expID=" + expID, ex);
					}
				}
				else {
					logger.info(String.format(
							"*SKIPPED* Generic Category Assoc. Filtered out by KBDateFilter: cat=%d,type=%s,parent=%d,effID=%d,expID=%d",
							catID,
							type,
							parentID,
							effID,
		...
		try {
			...
			while (rs.next()) {
				...
				if (dateSysnonymsPassFilter(effID, expID, knowledgeBaseFilterConfig.getDateFilterConfig())) {
					...
					logger.info("Generic entity-category: eid=" + entityID + ",etype=" + entityType + ",cid=" + catID + ",ctype=" + catType
							+ ",eff=" + effID + ",exp=" + expID);
				}
				else {
					logger.info(String.format(
