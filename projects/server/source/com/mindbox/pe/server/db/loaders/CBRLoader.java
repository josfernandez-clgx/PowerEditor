...
public class CBRLoader extends AbstractLoader {
	...
	private static final String Q_LOAD_CBR_ATTRIBUTE_TYPES = "select attribute_type_id, display_name, symbol,  default_match_contribution, default_mismatch_penalty, "
			+ "default_absence_penalty, default_value_range_id, ask_for_match_interval, ask_for_numeric_range "
			+ "from MB_CBR_ATTRIBUTE_TYPE ORDER BY attribute_type_id";
	...
	private void dbLoadCBRCases() throws SQLException {
		...
		try {
			...
			while (rs.next()) {
				...
				while (rs2.next()) {
					CBRAttributeValue newVal = new CBRAttributeValue(
							rs2.getInt(1),
							UtilBase.trim(rs2.getString(4)),
							UtilBase.trim(rs2.getString(7)));
