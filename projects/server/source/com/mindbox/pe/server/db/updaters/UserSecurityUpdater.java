...
public class UserSecurityUpdater implements UserDataUpdater {
	...
	public void updateUnknownPrivileges(int roleID, List<String> unknownPrivsForRole) throws SQLException {
		...
		PreparedStatement ps = null;
		try {
			if (unknownPrivsForRole.contains(PrivilegeConstants.PRIV_EDIT_ENTITIES)
					&& unknownPrivsForRole.contains(PrivilegeConstants.PRIV_VIEW_ENTITIES)) {
	 ...
	 */
	public void insertUser(String userID, String name, String status, boolean reset_password, int failedLoginCounter, int[] roleIDs,
	...
	/* (non-Javadoc)
	 * @see com.mindbox.pe.server.spi.db.UserDataUpdater#updateRoleWithUnknownPrivileges(int, java.lang.String, int[], java.util.List)
	 */
	...
	public void updateUser(String userID, String name, String status, boolean passwordChangeRequired, int failedLoginCounter,
		...
		logger.debug(">>> updateUser: " + userID + ",name=" + name + ",status=" + status + ",passwordChangeRequired="
	 ...
	 * @since 5.0.0
	 */
	public void loadRoles(Connection connection) throws SQLException {
		...
		try {
			...
			while (rs.next()) {
				roleId = rs.getInt(1);
				roleName = UtilBase.trim(rs.getString(2));
	 ...
	 * @throws SQLException
	 */
	public void insertPrivilegeRoles(Connection connection, Privilege priv) throws SQLException {
		PreparedStatement ps = null;
		try {
			for (Iterator<Role> itr = roles.iterator(); itr.hasNext();) {
				ps = connection.prepareStatement(ROLE_PRIVILEGE_INSERT_QUERY);
				Role role = itr.next();
