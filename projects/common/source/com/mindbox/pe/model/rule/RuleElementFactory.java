...
public class RuleElementFactory {
	...
	private static final long serialVersionUID = -2127830274911958932L;
	...
	private static void appendCopyString(StringBuffer buff, CompoundLHSElement element) {
	...
	private static void appendCopyString(StringBuffer buff, Condition element) {
		buff.append("C[");
	...
	private static void appendCopyString(StringBuffer buff, ExistExpression element) {
		buff.append("E[");
	...
	public static void appendCopyString(StringBuffer buff, TestCondition element) {
		buff.append("T[");
		...
		if (copyString.charAt(0) == 'P') {
			...
			if (tokens.length > 2) {
				if (ColumnRefParameterImpl.isValueString(tokens[2])) {
					return getInstance().createFunctionParameter(
							Integer.parseInt(tokens[0]),
							tokens[1],
							ColumnRefParameterImpl.extractColumnNumber(tokens[2]));
				}
				else {
	...
	public static ExistExpression deepCopyExistExpression(ExistExpression source) {
		ExistExpression expression = new ExistExpressionImpl(
				source.getClassName(),
				deepCopyCompoundLHSElement(source.getCompoundLHSElement()));
	...
	public static FunctionParameter deepCopyFunctionParameter(FunctionParameter param) {
		...
		if (param instanceof ColumnReference) {
			...
			copy = getInstance().createAttributeRefParameter(
					param.index(),
					param.toDisplayName(),
					((Reference) param).getAttributeName(),
					((Reference) param).getClassName());
		}
	...
	public FunctionParameter createFunctionParameterFromXMLStr(int index, String name, String value) {
		...
		if (ColumnRefParameterImpl.isValueString(value)) {
			...
			return new AttributeRefParameterImpl(
					index,
					name,
					AttributeRefParameterImpl.extractAttributeName(value),
					AttributeRefParameterImpl.extractClassName(value));
		}
		else {
	...
	public void createParametersForFunctionCall(FunctionCall function, FunctionTypeDefinition typeDef, List<FunctionParameter> oldParams,
			FunctionTypeDefinition oldTypeDef) {
