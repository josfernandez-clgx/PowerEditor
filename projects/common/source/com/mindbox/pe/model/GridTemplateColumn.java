...
public class GridTemplateColumn extends AbstractTemplateColumn implements RuleMessageContainer, ColumnReferenceContainer {
	...
	public GridTemplateColumn(GridTemplateColumn source) {
		...
		for (ColumnMessageFragmentDigest element : source.messageFragmentDigestList) {
			if (element != null) messageFragmentDigestList.add(new ColumnMessageFragmentDigest(element));
		}
	...
	public void adjustChangedColumnReferences(int originalColNo, int newColNo) {
		...
		for (ColumnMessageFragmentDigest element : messageFragmentDigestList) {
			element.adjustChangedColumnReferences(originalColNo, newColNo);
			try {
				if (element.getText() != null) messageFragmentTextList.addText(AbstractMessageKeyList.getKey(element), element.getText());
			}
	...
	public void addColumnMessageFragment(ColumnMessageFragmentDigest msgDigest) throws ParseException {
		...
		if (msgDigest.getText() != null) messageFragmentTextList.addText(key, msgDigest.getText());
		messageConfig.addMessageDigest(msgDigest);
	}
	...
	public void updateColumnMessageFragmentText(ColumnMessageFragmentDigest msgDigest) throws ParseException {
		String key = AbstractMessageKeyList.getKey(msgDigest);
		messageFragmentTextList.updateText(key, msgDigest.getText());
		...
		for (ColumnMessageFragmentDigest columnMessageFragmentDigest : messageFragmentDigestList) {
			if (key.equals(AbstractMessageKeyList.getKey(columnMessageFragmentDigest))) {
	...
	private ColumnMessageFragmentDigest getAnyMessageFragmentDigest() {
		for (ColumnMessageFragmentDigest element : messageFragmentDigestList) {
			if (element.getType().equals(AbstractMessageKeyList.TYPE_ANY_KEY)) {
				return element;
			}
		}
		return null;
	...
	private ColumnMessageFragmentDigest getMessageFragmentDigest(EnumValues<?> enumVal) {
		...
		for (ColumnMessageFragmentDigest element : messageFragmentDigestList) {
			if (element.getType().equals(AbstractMessageKeyList.ENUM_KEY) && element.getCellSelection() != null
					&& element.getCellSelection().equals(AbstractMessageKeyList.getEnumKey(isExclusion, isMultiSelect))) {
				return element;
			}
		}
		...
		for (ColumnMessageFragmentDigest element : messageFragmentDigestList) {
			if (element.getType().equals(AbstractMessageKeyList.ENUM_KEY) && element.getCellSelection() != null
					&& element.getCellSelection().equals(AbstractMessageKeyList.TYPE_DEFAULT_KEY)) {
				return element;
			}
		}
		return null;
	...
	private ColumnMessageFragmentDigest getMessageFragmentDigestForRange() {
		for (ColumnMessageFragmentDigest element : messageFragmentDigestList) {
			if (element.getType().equals(AbstractMessageKeyList.TYPE_RANGE_KEY)) {
				return element;
			}
		}
		return null;
