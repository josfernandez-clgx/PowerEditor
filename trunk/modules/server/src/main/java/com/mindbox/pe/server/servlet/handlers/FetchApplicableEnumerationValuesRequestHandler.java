...
public class FetchApplicableEnumerationValuesRequestHandler extends AbstractSessionRequestHandler<FetchApplicableEnumerationValuesRequest> {
	...
	@Override
	protected ResponseComm handleRequest(FetchApplicableEnumerationValuesRequest requestcomm, HttpServletRequest httpservletrequest) throws DataValidationFailedException, LockException,
			ServletActionException, ServerException {
		List<EnumValue> list = new LinkedList<EnumValue>();
		list.addAll(ConfigurationManager.getInstance().getEnumerationSourceConfigHelper().getApplicableEnumValues(requestcomm.getSourceName(), requestcomm.getSelectorValue()));
		ListResponse<EnumValue> response = new ListResponse<EnumValue>(list);
		if (logger.isDebugEnabled()) logger.debug(String.format("# of applicable enum values for %s = %d", requestcomm.getSelectorValue(), list.size()));
		return response;
	}
}
