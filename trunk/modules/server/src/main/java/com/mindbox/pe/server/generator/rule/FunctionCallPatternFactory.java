...
public final class FunctionCallPatternFactory {
	...
	public FunctionCallPattern createFunctionCallPattern(IDNameObject templateIDName, RuleDefinition ruleDefinition, LHSPatternList patternList) throws RuleGenerationException {
		...
		processActionArgs(templateIDName, functionCallPattern, new RuleDefinitionParamContainer(ruleDefinition), aeRule.getActionParms().iterator(), ruleDefinition.getUsageType(), patternList);
		return functionCallPattern;
	}

	public FunctionCallPattern createFunctionCallPattern(TestCondition testCondition, TemplateUsageType usageType, LHSPatternList patternList) throws RuleGenerationException {
		TestTypeDefinition testTypeDef = testCondition.getTestType();
		if (testTypeDef == null) {
			throw new RuleGenerationException("No type defined for Test Condition: " + testCondition);
		}
	...
	private void processActionArgs(IDNameObject templateIDName, FunctionCallPattern functionCallPattern, FunctionParameterContainer functionContainer,
		...
		while (argIterator.hasNext()) {
			...
			// To fix TT 1042, don't use indexOf(<literal>) >= 0. Use more accurate tests.
				...
				final LineagePatternConfigHelper lineagePatternSet = helper.getRuleGenerationConfiguration(usageType).getLineagePatternConfigSet();
				if (lineagePatternSet.size() > 0) {
					final List<RuleGenerationLHS.Pattern> configs = lineagePatternSet.getLineagePatternConfigs(lineagePatternSet.getPrefix().get(0));
					if (configs.size() > 0) {
						functionCallPattern.add(new StaticFunctionArgument("?" + configs.get(0).getVariable()));
					}
				}
			}
