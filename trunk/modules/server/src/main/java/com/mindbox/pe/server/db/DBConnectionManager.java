...
import com.mindbox.pe.server.config.ServerConfigHelper;
...
public final class DBConnectionManager {
	...
	private DBConnectionManager() {
		connectionPool = new GenericObjectPool<Object>(null);
		init();
		// start db connection monitor 
		new ConnectionMonitor(ConfigurationManager.getInstance().getServerConfigHelper().getDatabaseConfig().getMonitorIntervalInSeconds());
	}
	...
	private void init() {
		final ServerConfigHelper.DBConfig dbConfig = ConfigurationManager.getInstance().getServerConfigHelper().getDatabaseConfig();
		try {
		...
		PoolableConnectionFactory poolableConnectionFactory = new PoolableConnectionFactory(connectionFactory, connectionPool, null, (UtilBase.isEmpty(dbConfig.getValidationQuery())
				? null
				: dbConfig.getValidationQuery()), false, false);
