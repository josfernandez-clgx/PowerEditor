...
public class DefaultUserTrackingDao implements UserTrackingDao {
	...
	private void insertUserTrackingData(Connection connection, String userId, String modifiedBy) throws SQLException {
		...
		try {
			...
			logDebug(LOG, "Inserted new tracking data for %s", userId);
		}
		finally {
	...
	@Override
	public void insertUserTrackingData(final String userId, final String modifiedBy) throws SQLException {
		Connection connection = DBConnectionManager.getInstance().getConnection();
		try {
			connection.setAutoCommit(false);
	...
	public boolean isUserDisabled(String userId) throws SQLException {
		...
		try {
			...
			logDebug(LOG, "Determined disabled status for %s: %b", userId, result);
	...
	private void updateLastLoginDate(final Connection connection, final String userId, final Date lastLoginDate) throws SQLException {
		PreparedStatement ps = null;
		try {
			if (isTrackingRecordThere(connection, userId)) {
				...
				logDebug(LOG, "Updated last login for %s", userId);
			}
		}
		catch (Exception e) {
			connection.rollback();
	...
	public void updateLastLoginDate(String userId, Date lastLoginDate) throws SQLException {
		final Connection connection = DBConnectionManager.getInstance().getConnection();
		try {
			connection.setAutoCommit(false);
			updateLastLoginDate(connection, userId, lastLoginDate);
		}
		finally {
			DBConnectionManager.getInstance().freeConnection(connection);
		}
	...
	public void updateLastModifiedDetails(String userId, Date lastModifiedDate, String modifiedBy) throws SQLException {
		...
		try {
			...
			logDebug(LOG, "Updated last modified details for %s", userId);
		}
		catch (Exception e) {
			connection.rollback();
			if (SQLException.class.isInstance(e)) {
