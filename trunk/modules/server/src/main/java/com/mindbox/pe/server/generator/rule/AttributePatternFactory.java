...
public final class AttributePatternFactory {
	...
	public AttributePattern createAttributePattern(Condition condition, String variableOverride, TemplateUsageType usageType) throws RuleGenerationException {
		// if parent is NOT or OR and condition's value is attribute ref, wrap patterns with AND
		switch (condition.getOp()) {
		case Condition.OP_ANY_VALUE:
			return new StaticTextAttributePattern(helper.getDeployLabelForAttribute(condition.getReference()), helper.asVariableName(condition.getReference().getAttributeName(), variableOverride));
		case Condition.OP_IS_EMPTY:
		case Condition.OP_IS_NOT_EMPTY: {
			com.mindbox.pe.xsd.config.RuleGenerationLHS.Value valueConfig = helper.getRuleGenerationConfiguration(usageType).getLHSValueConfig(RuleGenerationConfigHelper.VAUE_TYPE_UNSPECIFIED);
			if (valueConfig == null) {
			...
			RuleGeneratorHelper.appendFormattedForStringType(buff, valueConfig.getDeployValue(), UtilBase.asBoolean(valueConfig.isValueAsString(), false));
			if (condition.getOp() == Condition.OP_IS_NOT_EMPTY) {
				buff.append(")");
			}
			return new StaticTextAttributePattern(helper.getDeployLabelForAttribute(condition.getReference()), attribVarName, buff.toString());
		}
		default:
			Value value = condition.getValue();
			if (value instanceof ColumnReference) {
				return new ColumnRefValueSlotAttributePattern(helper.getDeployLabelForAttribute(condition.getReference()), helper.asVariableName(
						condition.getReference().getAttributeName(),
						variableOverride), condition.getReference(), condition.getOp(), ((ColumnReference) value).getColumnNo());
			}
				...
				if (((MathExpressionValue) value).getColumnReference() != null) {
					return new ColumnRefValueSlotAttributePattern(helper.getDeployLabelForAttribute(condition.getReference()), helper.asVariableName(
							condition.getReference().getAttributeName(),
							variableOverride), condition.getReference(), condition.getOp(), ((MathExpressionValue) value).getColumnReference().getColumnNo(), slotText);
				}
				else {
					return new StringValueSlotAttributePattern(helper.getDeployLabelForAttribute(condition.getReference()), helper.asVariableName(
							condition.getReference().getAttributeName(),
							variableOverride), condition.getReference(), condition.getOp(), MessageFormat.format(slotText, new Object[] { meValue.getValue() }));
				}
			}
				...
				return new StaticTextAttributePattern(helper.getDeployLabelForAttribute(condition.getReference()), attrVariable, linkFunctionFormat.format(new Object[] {
						RuleGeneratorHelper.toARTScriptOpString(condition.getOp()),
						attrVariable,
						helper.asVariableName(((Reference) value).getAttributeName()) }));
			}
			else {
				...
				return new StringValueSlotAttributePattern(helper.getDeployLabelForAttribute(condition.getReference()), helper.asVariableName(
						condition.getReference().getAttributeName(),
						variableOverride), condition.getReference(), condition.getOp(), strToUse);
			}
		} // switch
	}
	...
	public AttributePattern createLinkAttributePattern(DomainClassLink dcLink, String objectName, TemplateUsageType usageType) {
		...
		if (dcLink.getDeployValueName() == null || dcLink.getDeployValueName().trim().length() == 0) {
			logger.warn("Domain class link " + dcLink + " has no deploy value set; using '" + dcLink.getChildName() + DEFAULT_DOMAINCLASSLINK_ATTRIBUTE_SUFFIX + "'.");
		...
		String variableName;
		String text;
		LinkPatternConfigHelper linkPatternConfig = helper.getRuleGenerationConfiguration(usageType).getLinkPatternConfig();
		if (linkPatternConfig != null && linkPatternConfig.useTestFunction()) {
			...
			variableName = (objectName == null || objectName.length() == 0 ? helper.asVariableName(dcLink.getChildName()) : "?" + objectName);
			text = null;
		}
