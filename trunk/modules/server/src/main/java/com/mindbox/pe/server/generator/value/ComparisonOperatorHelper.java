...
class ComparisonOperatorHelper extends AbstractOperatorHelper {
	...
	ComparisonOperatorHelper(RuleGenerationConfigHelper ruleGenerationConfiguration) {
			...
			if (value != null && !UtilBase.isEmpty(value.toString())) {
				...
				if (value instanceof String) {
					...
					// TT 1991
						...
						appendFormattedNumericValue(buff, reference, number, precision);
					}
				}
				// TT 1991
					...
					appendFormattedNumericValue(buff, reference, (Number) value, precision);
				}
				else {
					throw new RuleGenerationException("number or reference variable expected for " + attribVarName + "; " + value + " is neither");
				}
