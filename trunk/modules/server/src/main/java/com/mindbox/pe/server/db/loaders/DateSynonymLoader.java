...
public class DateSynonymLoader extends AbstractLoader {
	...
	private void loadDateSynonyms(KnowledgeBaseFilter knowledgeBaseFilterConfig) throws SQLException, ParseException {
		...
		try {
			...
			while (rs.next()) {
				...
				// Mark as not-in-use if the date is filtered out
				if (knowledgeBaseFilterConfig != null && knowledgeBaseFilterConfig.getDateFilter() != null) {
					DateFilterConfigHelper dateFilterConfigHelper = new DateFilterConfigHelper(knowledgeBaseFilterConfig.getDateFilter());
					if (date != null && !dateFilterConfigHelper.isInRange(date)) {
