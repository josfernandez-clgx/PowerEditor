...
public class DefaultPEDBCProvider implements PEDBCProvider {
	...
	public UserManagementProvider getUserManagementProvider() {
		if (userManagementProvider == null) {
			String classname = ConfigurationManager.getInstance().getServerConfigHelper().getDatabaseConfig().getUserManagementProviderClassName();
			if (classname == null || classname.length() == 0) {
				...
				try {
					...
					if (!(UserManagementProvider.class.isAssignableFrom(providerClass))) {
						throw new IllegalArgumentException("The <Database><UserManagementProviderClass> class " + providerClass.getName()
					...
					throw new IllegalArgumentException("Failed to initialize <Server><Database><UserManagementProviderClass> " + classname + ": " + ex.toString());
				}
			}
		}
	...
	public GuidelineRuleProvider getGuidelineRuleProvider() {
		if (ruleProvider == null) {
			String classname = ConfigurationManager.getInstance().getServerConfigHelper().getDatabaseConfig().getGuidelineRuleProviderClassName();
			if (classname == null || classname.length() == 0) {
				...
				try {
					...
					if (!(GuidelineRuleProvider.class.isAssignableFrom(providerClass))) {
						throw new IllegalArgumentException("The <Database><GuidelineRuleProviderClass> class " + providerClass.getName()
					...
					throw new IllegalArgumentException("Failed to initialize <Server><Database><GuidelineRuleProviderClass> " + classname + ": " + ex.toString());
				}
			}
		}
		return ruleProvider;
