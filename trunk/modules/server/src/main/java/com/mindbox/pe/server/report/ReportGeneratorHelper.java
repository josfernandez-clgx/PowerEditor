...
public class ReportGeneratorHelper implements ErrorMessageStorage {
	...
	public ReportGeneratorHelper(String templateName, String templateID, String usageType, String columns, String contextElements, String includeChildren, String includeParents,
	...
	public Map<String, String> getEntityContextValuesMap() {
		...
		final EntityConfigHelper entityConfig = ConfigurationManager.getInstance().getEntityConfigHelper();
		for (final GenericEntityType genericEntityType : GenericEntityType.getAllGenericEntityTypes()) {
			if (genericEntityType.isUsedInContext()) {
				String key = ReportGenerator.toElementName(genericEntityType);
				String value = ReportGenerator.extractContext(currentGrid.extractGuidelineContext(), genericEntityType);
				map.put(key, value);

				if (genericEntityType.hasCategory()) {
					final CategoryType categoryDef = entityConfig.findCategoryTypeDefinition(genericEntityType.getCategoryType());
					if (categoryDef != null) {
	...
	public String getRowRuleStringForReport() {
		...
		if ((currentTemplate.getRuleDefinition() != null) && !currentTemplate.getRuleDefinition().isEmpty()) {
			try {
				rowRuleString = ReportGenerator.generateRowRuleStringForReport(currentRowNumber, currentTemplate.getRuleDefinition(), currentTemplate, currentGrid);
			}
			catch (ReportException e) {
	...
	public String getRowMessageStringForReport() {
		...
		if ((currentTemplate.getRuleDefinition() != null) && !currentTemplate.getRuleDefinition().isEmpty()) {
			try {
				...
				rowMsgString = ReportGenerator.generateRowMessageStringForReport(currentRowNumber, -1, currentTemplate, ReportGenerator.getMessageContextEntityType(), entityIDs, currentGrid);
			}
			catch (Exception e) {
	...
	public Map<String, String[]> getRowColumnRuleAndMessageMap() {
		...
		for (Iterator<Integer> keyIter = containerMap.keySet().iterator(); keyIter.hasNext();) {
			...
			try {
				colRuleAndMsg[0] = ReportGenerator.generateRowRuleStringForReport(currentRowNumber, rmContainer.getRuleDefinition(), currentTemplate, currentGrid);
			}
			catch (ReportException e) {
	...
	private List<AbstractGuidelineGrid> filterActivations(List<ProductGrid> grids) {
		...
		for (Iterator<ProductGrid> i = grids.iterator(); i.hasNext();) {
			...
			if (date != null) {
				if (grid.getEffectiveDate() != null && grid.getEffectiveDate().getDate() != null && date.before(grid.getEffectiveDate().getDate())) {
					continue;
				}
				else if (grid.getExpirationDate() != null && grid.getExpirationDate().getDate() != null && date.after(grid.getExpirationDate().getDate())) {
					continue;
				}
			}
			// Check context
			if (!reportFilterDataHolder.isContextSpecified()) {
				...
				// check for context match in entity columns
					...
					ProductGrid gridCopy = ProductGrid.copyOf((ProductGrid) grid, currentTemplate, grid.getEffectiveDate(), grid.getExpirationDate());
