...
public class SearchCooridinator {
	...
	public <T extends Persistent> List<T> process(SearchFilter<T> searchFilter, String userID) throws ServerException {
		...
		PeDataType entityType = searchFilter.getEntityType();
		...
		if (searchFilter instanceof AbstractGenericCategorySearchFilter) {
			...
			if (entityType == PeDataType.PRIVILEGE) {
			...
			else if (entityType == PeDataType.TEMPLATE) {
			...
			else if (entityType == PeDataType.GUIDELINE_REPORT) {
			...
			else if (entityType == PeDataType.CBR_CASE_CLASS) {
				iter = CBRManager.getInstance().getCBRCaseClasses().iterator();
			}
			else if (entityType == PeDataType.CBR_SCORING_FUNCTION) {
				iter = CBRManager.getInstance().getCBRScoringFunctions().iterator();
			}
			else if (entityType == PeDataType.CBR_CASE_ACTION) {
				iter = CBRManager.getInstance().getCBRCaseActions().iterator();
			}
			else if (entityType == PeDataType.CBR_ATTRIBUTE_TYPE) {
			...
			else if (entityType == PeDataType.CBR_VALUE_RANGE) {
	...
	public Persistent retrieveEntity(PeDataType entityType, int entityID) {
		if (entityType == PeDataType.GUIDELINE_ACTION) {
			return GuidelineFunctionManager.getInstance().getActionTypeDefinition(entityID);
		}
		if (entityType == PeDataType.GUIDELINE_TEST_CONDITION) {
	...
	public Persistent retrieveEntity(PeDataType entityType, String name) {
