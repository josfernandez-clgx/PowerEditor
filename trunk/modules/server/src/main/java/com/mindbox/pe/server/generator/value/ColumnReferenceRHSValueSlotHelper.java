...
class ColumnReferenceRHSValueSlotHelper extends AbstractRHSValueSlotHelper {
	...
	final String toPrintableRHSColumnValue(int column, AbstractGenerateParms ruleParams, boolean addQuotesIfNecessary) throws RuleGenerationException {
		...
		if (colValue != null) {
			return writeVal(
					...
					ConfigurationManager.getInstance().getRuleGenerationConfigHelper(ruleParams.getUsage()).generateMultiEnumAsSequenceInRHS());
		}
		else {
	...
	private String writeVal(AbstractTemplateColumn column, Object obj, boolean addQuotes, boolean multiEnumAsSequence) throws RuleGenerationException {
