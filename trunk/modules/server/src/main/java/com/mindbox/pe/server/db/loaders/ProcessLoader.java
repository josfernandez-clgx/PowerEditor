		...
		try {
			...
			while (rs.next()) {
				...
				if (phaseID > 0) {
					...
					if (phase == null) {
						logger.warn("Process-Request: phase not set - invalid phaseID: id=" + id + ",name=" + name + ",type=" + type + ",func=" + initFunction + ",purpose=" + purpose + ",phaseID="
								+ phaseID);
					}
				}
				...
				logger.warn("Process-Request: id=" + id + ",name=" + name + ",type=" + type + ",func=" + initFunction + ",purpose=" + purpose + ",phaseID=" + phaseID);
			}
		}
		finally {
			DBUtil.closeLocallyManagedResources(rs, ps);
