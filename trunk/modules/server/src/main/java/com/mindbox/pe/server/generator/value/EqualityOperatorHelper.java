...
class EqualityOperatorHelper extends AbstractOperatorHelper {
	...
	private static String getEqualifyOperatorString(DeployType deployType) {
		if (deployType == DeployType.CURRENCY || deployType == DeployType.DATE || deployType == DeployType.FLOAT || deployType == DeployType.INTEGER || deployType == DeployType.PERCENT) {
	...
	EqualityOperatorHelper(RuleGenerationConfigHelper ruleGenerationConfiguration) {
	...
	public ValueAndComment formatForPattern(Object valueObj, int op, String attribVarName, boolean asString, Reference reference, TimeSlice timeSlice, final Integer precision)
		...
		if (valueObj != null && !UtilBase.isEmpty(valueObj.toString())) {
			...
			if (valueObj instanceof CategoryOrEntityValue || valueObj instanceof CategoryOrEntityValues) {
				...
				// TT 1991
				...
				else if (valueObj instanceof String && ((String) valueObj).length() > 0 && (((String) valueObj).charAt(0) == '?' || ((String) valueObj).charAt(0) == '(')) {
					...
					appendAsEnumValueIfAppropriate(buff, deployType, reference, valueObj, asString, precision);
				}
				else {
					appendFormattedValueForReference(buff, deployType, reference, valueObj, asString, precision);
				}
	...
	private void appendAsEnumValueIfAppropriate(StringBuilder buff, DeployType deployType, Reference reference, Object value, boolean asString, final Integer precision) throws RuleGenerationException {
		// getEnumAttributeIfApplicable quotes the returned value if necessary
		String valueStr = AeMapper.getEnumAttributeIfApplicable(reference.getClassName(), reference.getAttributeName(), value.toString(), true);
		if (valueStr == null) {
			appendFormattedValueForReference(buff, deployType, reference, value, asString, precision);
		}
		else {
	 ...
	 * @throws RuleGenerationException
	 */
	private void appendFormattedValueForReference(final StringBuilder buff, final DeployType deployType, final Reference reference, final Object obj, final boolean asString, final Integer precision)
			throws RuleGenerationException {
		try {
			if (deployType == null) {
				...
				Boolean boolValue = (obj.toString().equalsIgnoreCase("YES") || obj.toString().equalsIgnoreCase(RuleGeneratorHelper.AE_TRUE) ? Boolean.TRUE : Boolean.valueOf(obj.toString()));
			...
			else if (deployType == DeployType.CURRENCY || deployType == DeployType.FLOAT || deployType == DeployType.PERCENT || deployType == DeployType.INTEGER) {
				appendFormattedNumericValue(buff, reference, Double.parseDouble(obj.toString()), precision);
			}
			else if (deployType == DeployType.SYMBOL || deployType == DeployType.CODE) {
