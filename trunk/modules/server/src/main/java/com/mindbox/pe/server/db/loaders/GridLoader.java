...
public class GridLoader extends AbstractLoader {
	...
	private static final String Q_LOAD_GRID = "select g.grid_id, g.template_id, g.deploy_status," + " g.last_status_change_on, g.clone_of, g.creation_date, g.num_rows, g.comments,"
			+ " d.effective_synonym_id,d.expiration_synonym_id" + " from MB_GRID g, MB_GRID_DATE_SYNONYM d" + " where g.grid_id = d.grid_id and g.deploy_status <> 'Retired'";
	...
	public void load(KnowledgeBaseFilter knowledgeBaseFilterConfig) throws SQLException {
	...
	private void setGridCellValue(ProductGrid grid, int rowID, GridTemplateColumn column, String value) throws InvalidDataException {
		...
		grid.setValue(rowID, column.getName(), column.convertToCellValue(valueToSet, DomainManager.getInstance(), ConfigurationManager.getInstance().getEnumerationSourceConfigHelper()));
	}

	private synchronized void dbLoadProductGrids(KnowledgeBaseFilter knowledgeBaseFilterConfig) throws SQLException {
		final DateFilterConfigHelper dateFilterConfigHelper = (knowledgeBaseFilterConfig == null || knowledgeBaseFilterConfig.getDateFilter() == null) ? null : new DateFilterConfigHelper(
				knowledgeBaseFilterConfig.getDateFilter());
		...
		try {
			...
			rs = ps.executeQuery();
			processResultSet(gridmanager, rs, dateFilterConfigHelper);
		}
	...
	private void processResultSet(GridManager gridmanager, ResultSet rs, final DateFilterConfigHelper dateFilterConfigHelper) throws SQLException, ParseException {
		int gridID;
		int templateID;
		String status;
		...
		while (rs.next()) {
			...
			if (gridID <= 0) {
				logger.warn("Guideline Grid Ignored - invalid grid ID (" + gridID + ";template=" + templateID + "; status=" + status + "; NumRows=" + numRows + ")");
			}
			else if (templateID < 0) {
				logger.warn("Guideline Grid Ignored - invalid template ID (" + gridID + ";template=" + templateID + "; status=" + status + "; NumRows=" + numRows + ")");
			}
			else if (GuidelineTemplateManager.getInstance().getTemplate(templateID) == null) {
				logger.warn("Guideline Grid Ignored - template not found (" + gridID + ";template=" + templateID + "; status=" + status + "; NumRows=" + numRows + ")");
			}
			else {
				if (dateSysnonymsPassFilter(effID, expID, dateFilterConfigHelper)) {
					logger.info(gridID + ";template=" + templateID + "; status=" + status + "; NumRows=" + numRows + "; effID=" + effID + ",expID=" + expID);
		...
		try {
			...
			while (rs.next()) {
				...
				if (entityID > 0) {
					...
					if (genericEntityType == null) {
					...
					else if (!genericEntityType.isUsedInContext()) {
						logger.warn("Grid entity context ignored - entity type not configured for context: " + gridID + "," + entityID + ',' + entityType);
					}
						...
						if (added) {
							...
							logger.warn("Ignoring Grid EntityContext: grid=" + gridID + ",type=" + entityType + ",entityID=" + entityID + "; no grid " + gridID + " exist");
						}
					}
				}
				else if (categoryID > 0) {
					if (EntityManager.getInstance().getGenericCategory(categoryType, categoryID) == null) {
						logger.warn("Grid entity context ignored - generic category not found: " + gridID + "," + categoryType + ',' + categoryID);
					}
					else {
						...
						if (added) {
							...
							logger.warn("Ignoring Grid CategoryContext: grid=" + gridID + ",categoryType=" + categoryType + ",categoryID=" + categoryID + "; no grid " + gridID + " exist");
