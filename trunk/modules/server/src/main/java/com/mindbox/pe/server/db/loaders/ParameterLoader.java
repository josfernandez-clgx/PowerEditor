...
public class ParameterLoader extends AbstractLoader {
	...
	private void loadGrid(KnowledgeBaseFilter knowledgeBaseFilterConfig) throws SQLException {
		...
		try {
			...
			while (rs.next()) {
				...
				if (dateSysnonymsPassFilter(effID, expID, dateFilterConfigHelper)) {
					...
					logger.info("ParameterGrid: " + gridID + ",template=" + templateID + ",cellValues=" + cellValues + ",rowCount=" + rowCount + ",sunrise=" + effID + ",sunset=" + expID + ",status="
							+ status);
				}
				else {
			...
			while (rs.next()) {
				...
				if (entityID > 0) {
					if (EntityManager.getInstance().getEntity(GenericEntityType.forID(entityType), entityID) == null) {
						logger.warn("ParameterEntityContext ignored - generic entity not found (" + gridID + "," + entityID + ',' + entityType);
					}
					else {
						...
						if (added) {
							...
							logger.warn("Ignoring grid context: grid=" + gridID + ",type=" + entityType + ",entityID=" + entityID + "; no grid " + gridID + " exist");
						}
					}
				}
				else if (categoryID > 0) {
					if (EntityManager.getInstance().getGenericCategory(categoryType, categoryID) == null) {
						logger.warn("ParameterEntityContext ignored - generic category not found (" + gridID + "," + categoryType + ',' + categoryID);
					}
					else {
						...
						if (added) {
							...
							logger.warn("Ignoring grid context: grid=" + gridID + ",categoryType=" + categoryType + ",categoryID=" + categoryID + "; no grid " + gridID + " exist");
						}
					}
				}
			}
