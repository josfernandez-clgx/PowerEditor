...
class MessageReportGenerator extends ObjectDepthFirst {
	...
	private static String generateMessageText(DynamicStringValue dsValue, MessageReportGenerator source, GridValueContainable grid) throws ParseException {
		if (dsValue == null || dsValue.toString() == null || dsValue.toString().length() == 0) {
			return "";
		}
	...
	private String getConditionalDelimiter() {
		return ConfigurationManager.getInstance().getRuleGenerationConfigHelper(template.getUsageType()).getMessageConfig().getConditionalDelimiter();
	}

	private String getConditionalFinalDelimiter() {
		return ConfigurationManager.getInstance().getRuleGenerationConfigHelper(template.getUsageType()).getMessageConfig().getConditionalFinalDelimiter();
	}

	private MessageConfiguration getMessageConfiguration() {
		MessageConfiguration config = null;
		if (column > 0) {
			if (ConfigurationManager.getInstance().getRuleGenerationConfigHelper(this.template.getColumn(column).getUsageType()) != null) {
				config = ConfigurationManager.getInstance().getRuleGenerationConfigHelper(this.template.getColumn(column).getUsageType()).getMessageConfig();
			}
		}
		return (config == null ? ConfigurationManager.getInstance().getRuleGenerationConfigHelper(this.template.getUsageType()).getMessageConfig() : config);
	}
	...
	public Object visit(CellValueLiteral n, Object argu) {
		...
		if (replaceColumnReference) {
			try {
				...
				MessageProcessor.writeVal(template.getColumn(colToUse), ((GridValueContainable) argu).getCellValueObject(row, colToUse, null), buff, getMessageConfiguration(), "");
			}
	...
	public Object visit(ColumnMessagesLiteral n, Object argu) {
		...
		for (int i = 0; i < intArray.length; i++) {
			if (i > 0 && i < intArray.length - 1) {
				...
				buff.append((condDelim == null ? Constants.DEFAULT_CONDITIONAL_DELIMITER : condDelim));
			}
			else if (i == intArray.length - 1 && i != 0) {
				String condFinalDelim = getConditionalFinalDelimiter();
				buff.append((condFinalDelim == null ? Constants.DEFAULT_CONDITIONAL_FINAL_DELIMITER : condFinalDelim));
			}
			...
			try {
				...
				if (parsedMessage == null) {
					if (replaceColumnReference) {
						logger.warn("    visit(ColumnMessageLiteral): no column message found - adding column value: " + grid.getCellValueObject(row, colNum, null));
