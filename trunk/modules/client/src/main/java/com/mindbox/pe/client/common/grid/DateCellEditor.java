...
public class DateCellEditor extends AbstractCellEditor implements TableCellRenderer {
		...
		try {
			if (isNullEmptyString(value)) {
				...
				if (!(value instanceof Date)) {
					if (forDateTime) {
						dateEntryField.setValue(Constants.THREADLOCAL_FORMAT_DATE_TIME_SEC.get().parse(value.toString()));
					}
					else {
						dateEntryField.setValue(Constants.THREADLOCAL_FORMAT_DATE.get().parse(value.toString()));
					}
				}
				else
	...
	public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
		...
		if (value != null && value instanceof Date) {
			if (forDateTime) {
				label.setText(Constants.THREADLOCAL_FORMAT_DATE_TIME_SEC.get().format((Date) value));
			}
			else {
				label.setText(Constants.THREADLOCAL_FORMAT_DATE.get().format((Date) value));
			}
		}
		else {
			label.setText("");
