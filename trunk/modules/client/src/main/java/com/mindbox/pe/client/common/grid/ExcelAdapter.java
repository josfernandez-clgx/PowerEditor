...
public class ExcelAdapter implements ActionListener {
	 ...
	 */
	private static final CategoryOrEntityValue convertToCategoryOrCellValue(GenericEntityType entityType, String str, int columnNo) throws InputValidationException {
		...
		if (!categoryAndEntityHaveSameName && entities != null && !entities.isEmpty()) {
			if (entities.size() > 1) {
				ClientUtil.getInstance().showWarning("msg.warning.paste.entity.name.duplicate", new Object[] { entityType.getDisplayName(), str, columnNo });
			}
			return new CategoryOrEntityValue(entities.get(0));
		}
		else {
			// if there are category and entity with the same name
			EntityType typeDef = ClientUtil.getEntityConfigHelper().findEntityTypeDefinition(entityType);
	...
	private static final CategoryOrEntityValue convertToCategoryOrCellValue(String typeStr, String str, int columnNo) throws InputValidationException {
		GenericEntityType entityType = GenericEntityType.forName(typeStr);
		if (entityType == null) {
			throw new IllegalArgumentException("No entity type of '" + typeStr + "' found");
		}
		return convertToCategoryOrCellValue(entityType, str, columnNo);
	}

	private static final CategoryOrEntityValues convertToCategoryOrCellValues(String typeStr, String str, int columnNo) throws InputValidationException {
		GenericEntityType entityType = GenericEntityType.forName(typeStr);
		if (entityType == null) {
			throw new IllegalArgumentException("No entity type of '" + entityType + "' found");
		}
		if (UtilBase.isEmpty(str)) {
			return null;
		}
	...
	// TT 2105
	private static final void displayWarningPopup(String messageTag, EntityType typeDef, String cellValue, int columnNo) throws InputValidationException {
	...
	private static String getValueStringForClipboard(CategoryOrEntityValues values) {
		...
		for (Iterator<CategoryOrEntityValue> iter = values.iterator(); iter.hasNext();) {
			...
			if (element.isForEntity()) {
				...
				GenericCategory category = EntityModelCacheFactory.getInstance().getGenericCategory(element.getEntityType(), element.getId());
				// if this category has duplicated name with either another category or entity for the same type
				if (EntityModelCacheFactory.getInstance().hasDuplicatedCategoryOrEntityName(true, element.getEntityType(), category.getName())) {
	...
	public void setCutAndPasteEabled(boolean enabled) {
		this.isCutAndPasteEnabled = enabled;
	}
			...
			for (int pasteRowIndex = selectedRowIndex; rowtokenizer.hasMoreTokens(); pasteRowIndex++) {
				...
				for (int i = 0; i < strs.length; i++) {
					...
					if (columnIndex < gridTable.getColumnCount()) {
						try {
							boolean writeInfo = gridTable.getTemplateColumn(columnIndex + 1).getColumnDataSpecDigest().getType().equals(ColumnDataSpecDigest.TYPE_INTEGER_RANGE);
							...
							// (part of TT 2186)
							if (gridTable.getTemplateColumn(columnIndex + 1).getColumnDataSpecDigest().getType().equals(ColumnDataSpecDigest.TYPE_ENTITY)) {
								if (gridTable.getTemplateColumn(columnIndex + 1).getColumnDataSpecDigest().isMultiSelectAllowed()) {
									...
									cellValueToUse = convertToCategoryOrCellValue(gridTable.getTemplateColumn(columnIndex + 1).getColumnDataSpecDigest().getEntityType(), cellValueStr, columnIndex + 1);
								}
							}
							else {
								cellValueToUse = gridTable.getTemplateColumn(columnIndex + 1).convertToCellValue(cellValueStr, DomainModel.getInstance(), ClientUtil.getEnumerationSourceProxy());

								if (writeInfo) {
									logInfo(ClientUtil.getLogger(), "[paste] converted: cellValue [%s] to [%s] for row=%d,col=%d", cellValueStr, cellValueToUse, pasteRowIndex, columnIndex);
								}

								// TT-17: Validate enum values before pasting
								if (gridTable.getTemplateColumn(columnIndex + 1).getColumnDataSpecDigest().getType().equals(ColumnDataSpecDigest.TYPE_ENUM_LIST)) {
									if (!UtilBase.isEmpty(cellValueStr)) {
										if (String.class.equals(cellValueToUse.getClass()) || (EnumValues.class.equals(cellValueToUse.getClass()) && EnumValues.class.cast(cellValueToUse).isEmpty())) {
											throw new IllegalArgumentException(String.format("%s is not a valid enum value for column %d", cellValueStr, (columnIndex + 1)));
										}
									}
								}
							}
						...
						// TT 2105
							...
							ClientUtil.getLogger().warn("Error while pasting " + cellValueStr + " into " + pasteRowIndex + "," + columnIndex, ex);
