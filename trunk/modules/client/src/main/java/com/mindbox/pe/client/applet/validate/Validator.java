...
public class Validator {
	...
	public static <G extends AbstractGrid<?>> String validateGapsAndOverlapsInDates(boolean allowGaps, G abstractgrid, List<G> gridList, DateSynonym dsEff, DateSynonym dsExp, boolean editFlag,
			boolean autoAdjustFlag) {
	...
	public static <G extends AbstractGrid<?>> String validateGapsAndOverlapsInDatesInList(boolean allowGaps, boolean checkNewOrChangedOnly, List<G> gridList, String gapMsgKey, String overlapMsgKey) {
		boolean allowOverlaps = false;
		boolean ignoreEnclosedRanges = false;
		List<DateSynonymRange> listToCheck = new LinkedList<DateSynonymRange>();
		for (G grid : gridList) {
	...
	private static String hasOverlapOrGap(boolean allowGaps, boolean allowOverlaps, boolean ignoreEnclosedRanges, boolean checkNewOrChangedOnly, List<DateSynonymRange> dsRangeList, String gapMsgKey,
		...
		if (gridList != null && gridList.size() > 0) {
			for (G grid : gridList) {
				if (cloneFlag || !grid.equals(abstractgrid)) {
					...
					if (sameEffDate) {
						boolean sameExpDate = (grid.getExpirationDate() == null && dsExp == null) || (grid.getExpirationDate() != null && grid.getExpirationDate().isSameDate(dsExp));
