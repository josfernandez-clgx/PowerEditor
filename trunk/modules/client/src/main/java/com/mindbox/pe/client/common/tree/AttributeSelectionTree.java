...
public final class AttributeSelectionTree extends JTree {
	...
	public DomainAttribute getSelectedAttribute() {
		TreePath treePath = getSelectionPath();
		if (treePath != null) {
			if (treePath.getLastPathComponent() instanceof AttributeTreeNode) {
				return ((AttributeTreeNode) treePath.getLastPathComponent()).getDomainAttribute();
			}
		}
		return null;
	}
	...
	public DomainClass getClassOfSelectedAttribute() {
		...
		if (treePath != null) {
			if (treePath.getLastPathComponent() instanceof AttributeTreeNode) {
				return ((DomainWithAttributeTreeNode) ((AttributeTreeNode) treePath.getLastPathComponent()).getParent()).getDomainClass();
			}
		}
		return null;
	}
