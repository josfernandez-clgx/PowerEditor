...
public class RoleTabPanel extends AbstractFilterSelectEditPanel {
	...
	public RoleTabPanel(boolean readOnly) {
		...
		refreshButton = UIFactory.createJButton("button.reload", null, new ReloadL(), "button.tooltip.reload.role");
	...
	protected String getEditPermission() {
		return PrivilegeConstants.PRIV_MANAGE_ROLES;
	}
	...
	private boolean deleteRole(Role role) {
		try {
			ClientUtil.getCommunicator().delete(role.getID(), PeDataType.ROLE);
			return true;
		}
