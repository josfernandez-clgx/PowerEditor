...
public class ActivationEditorDialog extends JPanel {
					...
					if (hasProductionRestriction(data.getStatus())) {
						if (!UtilBase.isSame(dsExp, data.getExpirationDate())) {
							if (dsExp.getDate() != null && dsExp.getDate().before(new Date())) {
								ClientUtil.getInstance().showErrorDialog("msg.error.cannot.change.expdate.past", new Object[] { ClientUtil.getHighestStatusDisplayLabel() });
								return false;
							}
							else if (data.getExpirationDate() != null && data.getExpirationDate().getDate().before(new Date())) {
								ClientUtil.getInstance().showErrorDialog("msg.error.cannot.change.expdate.expired", new Object[] { ClientUtil.getHighestStatusDisplayLabel() });
								return false;
							}
						}
					}
	...
	public boolean edit(ProductGrid grid, Operation op) {
		...
		if (grid != null) {
			...
			if (!op.equals(ActivationEditorDialog.Operation.COPY) && !ClientUtil.checkPermissionByPrivilegeName(PrivilegeConstants.PRIV_EDIT_PRODUCTION_DATA)
	...
	private boolean validate(ActivationEditorDialog dialog, ProductGrid grid, Operation op) {
		...
		if (errorMessageKey == null) {
			errorMessageKey = Validator.validateDuplicateDates(grid, gridList, effDateField.getValue(), expDateField.getValue(), op != Operation.COPY); // not copy flag
		}
		if (errorMessageKey != null) {
		...
		// prompt for continue
		if (autoAdjustCheckBox.isSelected() && hasRelevantProductionRestrictionToAutoAdjust(((AbstractGridPanel) this.object).gridList, effDateField.getValue(), expDateField.getValue())
				&& !ClientUtil.getInstance().showConfirmation("msg.confirm.autoadjust.production", new Object[] { ClientUtil.getHighestStatusDisplayLabel() })) {
			return false;
		}
		return true;
	}
