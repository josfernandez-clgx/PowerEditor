...
public final class GuidelineListPanel extends PanelBase implements GuidelineGridEditor, EntityDeleteListener, PowerEditorTabPanel {
	...
	private final class ChangeStatusL extends AbstractThreadedActionAdapter {
		...
		public void performAction(ActionEvent e) {
			...
			int option = -1;
			do {
				option = JOptionPane.showConfirmDialog(ClientUtil.getApplet(), combo, "Select Status", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
			} while (option == JOptionPane.OK_OPTION && invalidProductionChange(combo.getSelectedEnumValueValue()));
			if (option == JOptionPane.OK_OPTION) {
				if (!ClientUtil.checkPermissionByPrivilegeName(PrivilegeConstants.PRIV_EDIT_PRODUCTION_DATA) && ClientUtil.isHighestStatus(combo.getSelectedEnumValueValue())
						&& !ClientUtil.getInstance().showConfirmation("msg.confirm.statuschange.hightest", new Object[] { ClientUtil.getHighestStatusDisplayLabel() })) {
					return;
				}
				try {
		...
		public void valueChanged(ListSelectionEvent arg0) {
			...
			if (atLeastOneSelection) {
				for (Iterator<GuidelineReportData> i = getAllSelectedRowsDataList().iterator(); i.hasNext();) {
					...
					if (!ClientUtil.checkPermissionByPrivilegeName(PrivilegeConstants.PRIV_EDIT_PRODUCTION_DATA) && ClientUtil.isHighestStatus(data.getStatus())
	...
	public GuidelineListPanel(SearchGuidelinesTab sgTab, boolean readOnly) {
		...
		this.viewButton = UIFactory.createButton(ClientUtil.getInstance().getLabel("button.view"), null, new ViewL(), "button.tooltip.view.guideline");
		viewButton.setEnabled(false);
		this.editButton = UIFactory.createButton(ClientUtil.getInstance().getLabel("button.edit"), null, new EditL(), "button.tooltip.edit.guideline");
		editButton.setEnabled(false);

		this.printButton = UIFactory.createButton(ClientUtil.getInstance().getLabel("button.report"), null, new PrintL(), "button.tooltip.report");
		printButton.setEnabled(false);
		this.changeStatusButton = UIFactory.createButton(ClientUtil.getInstance().getLabel("button.change.status"), null, new ChangeStatusL(), "button.tooltip.change.status");
		changeStatusButton.setEnabled(false);
		this.changeDatesButton = UIFactory.createButton(ClientUtil.getInstance().getLabel("button.change.activation.date"), null, new ChangeActivationDateL(), "button.tooltip.change.activation.date");
		...
		removeSelectedButton = UIFactory.createButton(ClientUtil.getInstance().getLabel("button.remove.selection"), null, new ActionListener() {
	...
	private boolean invalidProductionChange(String status) {
		if (ClientUtil.isHighestStatus(status)) {
			...
			for (Iterator<GuidelineReportData> i = allData.iterator(); i.hasNext();) {
				GuidelineReportData data = i.next();
				if (!data.getStatus().equals(status) && (data.getActivationDate() == null || data.getActivationDate().getDate().before(new Date()))) {
	...
	private void showGridPanel(boolean viewOnly) {
		...
		try {
			if ((data.getContext() != null) && (data.getDataType().equals("GuidelineGrid"))) { // If it's a guideline grid.
				...
				// set activation
				gridEditPanel.showActivation(data.getContext(), findActivation(response.getResultList(), data.getActivationDate(), data.getExpirationDate()));
