...
public class GridTablePanel extends PanelBase implements IGridDataCard {
	...
	private class GridAdapter extends AbstractThreadedActionAdapter {
		...
		public void performAction(ActionEvent actionevent) {
			...
			if (obj == appendRowButton) {
				...
				addRow(selectedRow);
				if (selectedRow == -1) {
					selectedRow = gridTable.getRowCount() - 1;
				}
	...
	public void cancelEdits() {
		if (gridTable.isEditing()) {
			gridTable.getCellEditor().stopCellEditing();
		}
	}
	...
	protected void setEditable(boolean flag) {
		...
		for (int i = 0; i < ajbutton.length; i++) {
			if (ajbutton[i] != null) {
				ajbutton[i].setEnabled(flag);
			}
		}
		gridTable.setEnabled(flag);
		excelAdapter.setCutAndPasteEabled(flag);
	}

	public void setEnabled(boolean flag) {
		JButton ajbutton[] = { appendRowButton, addRowButton };
		boolean flag1 = flag && !viewOnlyFlag;
		for (int i = 0; i < ajbutton.length; i++) {
			if (ajbutton[i] != null) {
				ajbutton[i].setEnabled(flag1);
			}
		}
	...
	public void setTemplate(GridTemplate gridtemplate) {
		...
		gridScrollPane.setHorizontalScrollBarPolicy((gridtemplate.fitToScreen() ? JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED : JScrollPane.HORIZONTAL_SCROLLBAR_ALWAYS));
	}
	...
	void updateButtonStates() {
		boolean flag = !viewOnlyFlag && (gridTable.getSelectedRowCount() > 0);
