...
public class ParameterContextDialog extends JPanel {
	...
	public static Object[] cloneParameterGrid(ParameterGrid originalGrid) {
		ParameterGrid newGrid = new ParameterGrid(originalGrid, originalGrid.getEffectiveDate(), originalGrid.getExpirationDate());
		...
		int option = -1;
		do {
			option = JOptionPane.showConfirmDialog(ClientUtil.getApplet(), dialog, "Copy Parameter", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
	...
	private ParameterContextDialog(ParameterGrid paramGrid, boolean allowEdit) {
		...
		if (paramContext != null) {
			...
			if (!allowEdit) contextPanel.setEditContextEnabled(false);
	...
	private boolean updateFromFields() {
		...
		if (errMessageKey != null) {
			ClientUtil.getInstance().showErrorDialog(errMessageKey, new Object[] { ClientUtil.getStatusDisplayLabel(status) });
			return false;
		}
