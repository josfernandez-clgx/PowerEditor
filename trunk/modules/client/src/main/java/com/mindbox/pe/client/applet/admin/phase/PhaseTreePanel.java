...
class PhaseTreePanel extends JPanel {
	...
	private class LeftL extends AbstractThreadedActionAdapter {
		...
		public void performAction(ActionEvent arg0) {
			if (currentNode.getParent() != null && currentNode.getParent().getParent() == rootNode && currentNode.getPhase().hasPrerequisites()) {
				ClientUtil.getInstance().showWarning("msg.warning.process.invalid.prereq", new Object[] { currentNode.getPhase().getDisplayName() });
			}
			else {
				outdentCurrentNode();
			}
	...
	private class DeleteL extends AbstractThreadedActionAdapter {
		...
		private boolean canDelete(Phase phase) {
			try {
				List<ProcessRequest> requestList = ClientUtil.getCommunicator().search(new AllSearchFilter<ProcessRequest>(PeDataType.PROCESS_REQUEST));
				for (Iterator<ProcessRequest> iter = requestList.iterator(); iter.hasNext();) {
					ProcessRequest element = iter.next();
					if (element.getPhase().getID() == phase.getID()) {
						return ClientUtil.getInstance().showConfirmation("msg.question.delete.phase.used.request", new Object[] { phase.getDisplayName(), element.getDisplayName() });
					}
				}
		...
		public void performAction(ActionEvent arg0) {
			...
			if (phase != null && canDelete(phase)) {
				// delete phase
				try {
					ClientUtil.getCommunicator().delete(phase.getID(), PeDataType.PROCESS_PHASE);
	...
	private class LoadL extends AbstractThreadedActionAdapter {
		...
		public void performAction(ActionEvent arg0) {
			try {
				refreshTree(ClientUtil.getCommunicator().search(new AllSearchFilter<Phase>(PeDataType.PROCESS_PHASE)));
			}
			catch (Exception ex) {
				ClientUtil.handleRuntimeException(ex);
			}
