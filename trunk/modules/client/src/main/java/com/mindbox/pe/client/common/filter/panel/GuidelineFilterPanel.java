...
public final class GuidelineFilterPanel extends PanelBase {
	...
	private GuidelineReportFilter getGuidelineReportFilter(String descPrefix) {
		...
		List<GuidelineTab> usageGroupList = usageTypeTemplateSelectionPanel.getSelectedUsageGroups();
		...
		if (!usageGroupList.isEmpty()) {
			...
			buff.append(": ");
			for (Iterator<GuidelineTab> iterator = usageGroupList.iterator(); iterator.hasNext();) {
				GuidelineTab tabConfig = iterator.next();
				for (GuidelineTab.UsageType usageType : tabConfig.getUsageType()) {
					filter.addUsageType(TemplateUsageType.valueOf(usageType.getName()));
				}
				buff.append(tabConfig.getDisplayName());
				if (iterator.hasNext()) {
					buff.append(",");
				}
			}
