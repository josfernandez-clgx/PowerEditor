...
public class DateRange extends AbstractRange {
	...
	public boolean isSubsumedBy(DateRange dateRange) {
		boolean flag = true;
		if ((lowerVal != null && dateRange.lowerVal != null && lowerVal.before(dateRange.lowerVal)) || (upperVal != null && dateRange.upperVal != null && upperVal.after(dateRange.upperVal))) {
			flag = false;
		}
		return flag;
	}
	...
	public String toString() {
		...
		if (lowerVal != null) {
			...
			buff.append(Constants.THREADLOCAL_FORMAT_DATE.get().format(lowerVal));
		}
		else {
			buff.append(" ");
		}
		buff.append("-");
		if (upperVal != null) {
			buff.append(Constants.THREADLOCAL_FORMAT_DATE.get().format(upperVal));
	...
	public Object valueOf(String str) throws Exception {
		return Constants.THREADLOCAL_FORMAT_DATE.get().parse(str);
	}

	public String formatValue(Object value) {
		if (value instanceof Date) {
			return Constants.THREADLOCAL_FORMAT_DATE.get().format((Date) value);
		}
		else {
