...
public abstract class AbstractTemplateCore<C extends AbstractTemplateColumn> extends AbstractIDNameDescriptionObject {
	...
	public final C getColumnByName(final String columnName) {
		for (C element : columnList) {
	...
	public boolean hasEntityTypeColumnFor(GenericEntityType type, boolean forEntity) {
		...
		for (C column : columnList) {
			if (column.getColumnDataSpecDigest().getType().equals(ColumnDataSpecDigest.TYPE_ENTITY) && column.getColumnDataSpecDigest().getEntityType().equals(type.getName())
					&& ((column.getColumnDataSpecDigest().isEntityAllowed() && forEntity) || column.getColumnDataSpecDigest().isCategoryAllowed() && !forEntity)) {
				return true;
			}
		}
		return false;
