...
public class DataTypeCompatibilityValidator {
	...
	static public int getGenericDataType(DeployType deployType) {
		if (deployType == DeployType.CURRENCY || deployType == DeployType.INTEGER || deployType == DeployType.FLOAT || deployType == DeployType.PERCENT)
	...
	static public int getGenericDataType(String type) {
		...
		else if (type.equals(ColumnDataSpecDigest.TYPE_TIME_RANGE) || type.equalsIgnoreCase(ColumnDataSpecDigest.TYPE_INTEGER_RANGE) || type.equalsIgnoreCase(ColumnDataSpecDigest.TYPE_FLOAT_RANGE)
				|| type.equalsIgnoreCase(ColumnDataSpecDigest.TYPE_CURRENCY_RANGE))
			return NUMERIC_RANGE_DATA_TYPE;
		else if (type.equals(ColumnDataSpecDigest.TYPE_INTEGER) || type.equalsIgnoreCase(ColumnDataSpecDigest.TYPE_FLOAT) || type.equalsIgnoreCase(ColumnDataSpecDigest.TYPE_CURRENCY)
				|| type.equalsIgnoreCase(ColumnDataSpecDigest.TYPE_PERCENT))
			return NUMERIC_VALUE_DATA_TYPE;
	...
	static int getPreferredValueDataType(int dataType, int opType) {
		if (opType == RANGE_OPERATOR_TYPE) {
			if (dataType == NUMERIC_VALUE_DATA_TYPE)
				return NUMERIC_RANGE_DATA_TYPE;
			else if (dataType == DATE_VALUE_DATA_TYPE) return DATE_RANGE_DATA_TYPE;
		}
		else if (opType == MEMBERSHIP_OPERATOR_TYPE) return ENUMERATED_VALUE_LIST_DATA_TYPE;
		return dataType;
	}
	...
	static int getGenericDataType(String value, int preference) {
		if (preference == NUMERIC_VALUE_DATA_TYPE) {
			...
			try {
				if (Constants.THREADLOCAL_FORMAT_DATE.get().parse(value) != null) {
					return preference;
				}
			}
			catch (Exception ex) {
	...
	static String getGenericDataTypeName(String value, int preference) {
		if (preference == DATE_VALUE_DATA_TYPE || preference == UNKNOWN_DATA_TYPE) {
			try {
				if (Constants.THREADLOCAL_FORMAT_DATE.get().parse(value) != null) return DATE_VALUE_DATA_TYPE_NAME;
			}
			catch (Exception ex) {
			}
		}
		if (preference == DATE_RANGE_DATA_TYPE || preference == UNKNOWN_DATA_TYPE) {
			DateRange dr = DateRange.parseValue(value);
			if (dr.getUpperValue() != null || dr.getLowerValue() != null) return DATE_RANGE_DATA_TYPE_NAME;
		}
		if (preference == NUMERIC_RANGE_DATA_TYPE || preference == UNKNOWN_DATA_TYPE) {
			FloatRange fr = FloatRange.parseValue(value);
			if (fr.getUpperValue() != null || fr.getLowerValue() != null) return NUMERIC_RANGE_DATA_TYPE_NAME;
		}
		...
		if (preference == BOOLEAN_VALUE_DATA_TYPE || preference == UNKNOWN_DATA_TYPE) {
			if (value.equalsIgnoreCase("true") || value.equalsIgnoreCase("false")) return BOOLEAN_VALUE_DATA_TYPE_NAME;
		}
	...
	public static int[] getLegalGenericDataTypes(DeployType dt, int opType) {
		...
		for (int i = 0; i < LEGAL_CONDITION_COMBINATIONS.length; i++) {
			if (LEGAL_CONDITION_COMBINATIONS[i][0] == lhsGenericDataType && LEGAL_CONDITION_COMBINATIONS[i][1] == genericOpType) legalTypes.add(new Integer(LEGAL_CONDITION_COMBINATIONS[i][2]));
		}
	...
	public static int[] getLegalGenericDataTypesForParameter(DeployType dt) {
		...
		for (int i = 0; i < LEGAL_PARAMETER_COMBINATIONS.length; i++) {
			if (LEGAL_PARAMETER_COMBINATIONS[i][0] == lhsGenericDataType) legalTypes.add(new Integer(LEGAL_PARAMETER_COMBINATIONS[i][1]));
		}
		if (legalTypes.size() == 0)
			return null;
		else {
	...
	public static String isValid(Reference lhs, int op, Value rhs, GridTemplate template, DomainClassProvider domain, boolean incompleteOK) {
		if (lhs == null) {
			if (incompleteOK)
				return null;
			else
		...
		if (dc == null) return "In condition \"" + conditionString + "\": Domain Class " + "\"" + lhs.getClassName() + "\" not found.";
		DomainAttribute da = dc.getDomainAttribute(lhs.getAttributeName());
		if (da == null) return "In condition \"" + conditionString + "\": Domain Attribute " + "\"" + lhs.getAttributeName() + "\" not found in class \"" + lhs.getClassName() + "\".";
		if (op == Condition.OP_ANY_VALUE || op == Condition.OP_IS_EMPTY || op == Condition.OP_IS_NOT_EMPTY) return null;
		...
		if (rhs instanceof ColumnReference) {
			...
			if (column == null) return "In condition \"" + conditionString + "\": Column " + colNo + " does not exist in template.";
			...
			if (dc2 == null) return "In condition \"" + conditionString + "\": Domain Class " + "\"" + rhsRef.getClassName() + "\" not found.";
			DomainAttribute da2 = dc2.getDomainAttribute(rhsRef.getAttributeName());
			if (da2 == null) return "In condition \"" + conditionString + "\": Domain Attribute " + "\"" + rhsRef.getAttributeName() + "\" not found in class \"" + rhsRef.getClassName() + "\".";
			...
			if (dc2 == null) return "In condition \"" + conditionString + "\": Domain Class " + "\"" + mathRef.getClassName() + "\" not found.";
			DomainAttribute da2 = dc2.getDomainAttribute(mathRef.getAttributeName());
			if (da2 == null) return "In condition \"" + conditionString + "\": Domain Attribute " + "\"" + mathRef.getAttributeName() + "\" not found in class \"" + mathRef.getClassName() + "\".";
			if (!UtilBase.isMember(getGenericDataType(da2.getDeployType()), DATA_TYPES_FOR_CALC)) { // != NUMERIC_VALUE_DATA_TYPE)
				return "In condition \"" + conditionString + "\": Domain Attribute " + "\"" + mathRef.getClassName() + "." + mathRef.getAttributeName() + "\" must be numeric.";
			}
			...
			if ((opType == EQUALITY_OPERATOR_TYPE || opType == NUMERIC_COMPARISON_OPERATOR_TYPE) && da.hasEnumValue()) {
				...
				if (enumValues != null) {
					for (EnumValue enumVal : enumValues) {
						...
						if (rhs.toString().equalsIgnoreCase(displayVal)) return null;
					}
				}
			}
		...
		for (int i = 0; i < LEGAL_CONDITION_COMBINATIONS.length; i++) {
			if (LEGAL_CONDITION_COMBINATIONS[i][0] == lhsGenericDataType && LEGAL_CONDITION_COMBINATIONS[i][1] == opType && LEGAL_CONDITION_COMBINATIONS[i][2] == rhsGenericDataType) return null;
		}
		String opString = Condition.Aux.toOpString(op);
		return "In condition \"" + conditionString + "\": illegal combination of reference, operation and value: " + lhsDeployType.getName() + " " + opString + " " + rhsDescription;

	}

	public static String isValid(FunctionParameter param, FunctionParameterDefinition paramDef, GridTemplate template, DomainClassProvider domain, boolean incompleteOK) {
		...
		if (param instanceof ColumnReference) {
			...
			AbstractTemplateColumn column = template.getColumn(colNo);
			if (column == null) return "Column " + colNo + " does not exist in template.";
			...
			if (dc2 == null) return "Domain Class " + "\"" + ref.getClassName() + "\" not found.";
			DomainAttribute da2 = dc2.getDomainAttribute(ref.getAttributeName());
			if (da2 == null) return "Domain Attribute " + "\"" + ref.getAttributeName() + "\" not found in class \"" + ref.getClassName() + "\".";
		...
		for (int i = 0; i < LEGAL_PARAMETER_COMBINATIONS.length; i++) {
			if (LEGAL_PARAMETER_COMBINATIONS[i][0] == expectedGenericDataType && LEGAL_PARAMETER_COMBINATIONS[i][1] == paramGenericDataType) return null;
		}
		return "Parameter value (\"" + param.toDisplayName() + "\", type = " + getGenericDataTypeName(paramGenericDataType) + ") is incompatible with the defined data type ("
				+ getGenericDataTypeName(expectedGenericDataType) + ").";
	}
	...
	public static String isValid(CompoundRuleElement<?> compound, GridTemplate template, DomainClassProvider domain, boolean incompleteOK) {
		if (compound != null) {
			...
			for (int i = 0; i < compound.size(); i++) {
				...
				if (error != null) errors.append((errors.length() == 0 ? "" : "\n") + error);
			}
	...
	public static String isValid(RuleElement element, CompoundRuleElement<?> parent, GridTemplate template, DomainClassProvider domain, boolean incompleteOK) {
		if (element != null) {
			if (element instanceof CompoundRuleElement)
				...
				if (paramNum <= funDef.getParameterDefinitions().length) paramDef = funDef.getParameterDefinitionAt(paramNum);
				...
				if (paramDef == null) {
					...
					if (error != null) error = "In " + typeString + " " + funDef.getName() + " parameter " + paramNum + "(" + paramDef.getName() + "): " + error;
					return error;
				}
			}
		}
